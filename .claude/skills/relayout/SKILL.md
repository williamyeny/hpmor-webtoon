...
In the case that it's an engine bug, definitely feel free to fix it. However, be mindful of how the change affects other panels -- you may need to relayout them. After an engine change, re-render everything with `work/render-all.sh` (episodes, cover and site) and look at what moved (`git status site/` lists the tiles whose pixels changed); `node work/tilesfor.mjs` helps find the tiles a pose change affects.
