...
await p.evaluate(() => localStorage.setItem('hpmor-state', JSON.stringify({ last: 'ep03', eps: { ep01: { done: true, p: 1 }, ep02: { done: true, p: 1 }, ep03: { done: true, p: 1 } } })));
await p.reload(); await p.waitForTimeout(400); await p.evaluate(() => scrollTo(0, 380)); await p.waitForTimeout(200);
