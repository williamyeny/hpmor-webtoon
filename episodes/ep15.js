...
import { comedCan, pouch, quill } from '../engine/props/props.js';
...
ep.panel(760, { cam: { on: ['harry'], fr: 'waist', dy: 0.1 }, bg: MO, blur: 2, actors: ROOM({}, { expr: 'think', pose: 'raiseHand' }) },
...
ep.panel(1150, { cam: TWO(1150), bg: MO, actors: ROOM({ expr: 'think' }, { expr: 'neutral', pose: 'shrug' }) },
  [say('McGonagall', 'Suppose I only awarded you fifty points, then?', 285, 50, { anchor: 'tc', w: 400, fixed: true }),
   say('Harry', 'It\'s not fair to the other children, if I earn lots of points for grown-up things they can\'t be part of. How is Terry Boot supposed to earn fifty points for reporting a whisper from the Sorting Hat?', 488, 230, { anchor: 'tc', w: 410, fixed: true })], { mood: 'warm' });
...
ep.panel(820, { cam: { x: 1230, y: 607, w: 620 }, bg: MO, actors: [MCG({ x: 1060, y: 1000, pose: 'present', seat: undefined, turn: 0.4, expr: 'stern', armB: { sh: 80, el: 10, hand: 'hold', prop: g({ transform: 'translate(0,40) scale(0.35)' }, P2.timeTurner(1)) } }), HP({ x: 1400, expr: 'bigGrin', pose: 'reach', armF: { sh: 70, el: 20, hand: 'open' } })] },
...
ep.panel(840, { cam: { on: ['harry'], fr: 'bust', dy: 0.05 }, bg: MO, blur: 2, actors: [HP({ x: 1400, expr: 'scheme', pose: 'hold', armF: { sh: 30, el: 80, hand: 'hold', prop: g({ transform: 'translate(0,24) scale(0.3)' }, P2.timeTurner(1, { chain: false })) } })] },
...
ep.panel(700, { cam: { x: 400, y: 330, w: 800 }, bg: () => rect(-100, -100, 1000, 1000, { fill: '#e9dcc0' }) + g({ transform: 'translate(400,380)' }, circle(0, 0, 220, { fill: '#f6efdc', stroke: C.ink, 'stroke-width': 4 }), ...[...Array(26).keys()].map((k) => { const a = k / 26 * Math.PI * 2; return line(Math.sin(a) * 200, -Math.cos(a) * 200, Math.sin(a) * 180, -Math.cos(a) * 180, { stroke: C.ink, 'stroke-width': 3 }); }), line(0, 0, 0, -150, { stroke: C.ink, 'stroke-width': 6 }), line(0, 0, 110, 60, { stroke: C.ink, 'stroke-width': 5 }), g({ transform: 'translate(170,-170) scale(0.5)' }, P2.timeTurner(1, { chain: false }))), over: (e) => FX.memoryEdge(e.w, e.h) },
  [cap('*(In Episode 2, she had promised him a solution "in time".)*', 44, 30, { w: 420, fixed: true })], { mood: 'sepia', alt: 'Memory: the 26-hour clock diagram from his first day, and the tiny hourglass beside it.' });
// the meltdown
...
ep.panel(560, { cam: { on: ['harry'], fr: 'eyes', zoom: 0.8 }, bg: MO, blur: 3, actors: [HP({ x: 1400, expr: { base: 'blank', eyes: { open: 1.2 } } })] },
...
ep.panel(700, { cam: { on: ['harry'], fr: 'eyes', zoom: 1.2 }, bg: MO, blur: 3, actors: [HP({ x: 1400, expr: { base: 'horror', eyes: { open: 1.3 } } })], over: (e) => FX.burst(e.w, e.h, e.w / 2, e.h / 2, { n: 30, op: 0.2 }) },
...
ep.panel(1250, { cam: { on: ['harry'], fr: 'bust', dy: -0.2, dx: -0.3 }, bg: MO, blur: 2, actors: [HP({ x: 1400, expr: 'laugh', pose: 'holdUp', armB: { sh: 110, el: 0, hand: 'hold', prop: g({ transform: 'translate(0,40) scale(0.3)' }, P2.timeTurner(1)) } })] },
...
ep.panel(1180, { cam: STAND2(1180), bg: MO, actors: [MCG({ x: 1000, y: 1000, pose: 'stand', seat: undefined, turn: 0.4, expr: 'smile' }), HP({ x: 1400, expr: 'rant', pose: 'holdUp', armB: { sh: 110, el: 0, hand: 'hold', prop: g({ transform: 'translate(0,40) scale(0.3)' }, P2.timeTurner(1)) } })] },
  [say('McGonagall', 'That can\'t possibly happen, Mr Potter. The Time-Turner cannot be used to go more than six hours back. And it can\'t be used more than six times in any day.', 300, 80, { anchor: 'tc', w: 380, fixed: true }),
   say('Harry', 'Oh, good, very good, that. And if someone bumps into me, it will *not* break and trap the whole castle of Hogwarts in an endlessly repeating loop of Thursdays?', 505, 470, { anchor: 'tc', w: 380, fixed: true })], { mood: 'warm' });
...
ep.panel(1350, { cam: STAND2(1350), bg: MO, actors: [MCG({ x: 1000, y: 1000, pose: 'stand', seat: undefined, turn: 0.4, expr: 'calm' }), HP({ x: 1400, expr: 'rant', pose: 'armsUp' })] },
  [say('McGonagall', 'Wizards *are* advised to avoid being seen by their past selves. The first version of you should step aside and close his eyes at a known time. It\'s all in the pamphlet.', 300, 76, { anchor: 'tc', w: 380, fixed: true }),
   say('Harry', 'And it doesn\'t, say, create a paradox that destroys the universe.', 530, 460, { anchor: 'tc', w: 330, fixed: true }),
   say('McGonagall', 'Mr Potter, I think I\'d remember hearing if *that* had ever happened.', 430, 738, { anchor: 'tc', w: 360, fixed: true })], { mood: 'warm' });
ep.bleed(1130, { cam: { on: ['harry'], fr: 'bust', zoom: 0.85, dy: -1.0 }, bg: MO, actors: [HP({ x: 1400, expr: 'yell', pose: 'panic' })], over: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.62, { n: 34, op: 0.25 }) },
  [shout('Harry', '*THAT IS NOT REASSURING! HAVEN\'T YOU PEOPLE EVER HEARD OF THE ANTHROPIC PRINCIPLE? AND WHAT IDIOT EVER BUILT ONE OF THESE THINGS FOR THE FIRST TIME?*', 400, 140, { anchor: 'tc', w: 440, size: 36, fixed: true })], { alt: 'Harry, arms flailing, at full volume.' });
...
// realisation 1
ep.bleed(820, { cam: { on: ['harry'], fr: 'close', zoom: 0.9 }, bg: MO, actors: [HP({ x: 1400, expr: { base: 'shock', eyes: { open: 1.3 } } })], over: (e) => K.glow(e.w / 2, e.h * 0.4, 500, '#ffe9a8', 0.4) + FX.burst(e.w, e.h, e.w / 2, e.h * 0.4, { n: 40, col: '#fff3c9', op: 0.35 }) },
...
ep.panel(900, { cam: { on: ['harry'], fr: 'bust', dy: -0.2 }, bg: MO, blur: 2, actors: [HP({ x: 1400, expr: 'rant', pose: 'facepalm' })] },
...
ep.panel(1000, { cam: { x: 820, y: 470, w: 900 }, bg: DAWN(), actors: [{ def: terry, id: 'terry', x: 500, y: 960, s: 1.05, turn: 0.4, pose: 'crossArms', expr: 'suspicious' }, { def: anthony, id: 'anthony', x: 680, y: 960, s: 1.05, turn: 0.4, expr: 'neutral' }, { def: harryRaven, id: 'harry', x: 1120, y: 1000, s: 1.1, turn: -0.5, pose: 'raiseHand', expr: 'calm' }] },
...
ep.panel(1150, { cam: { on: ['harry'], fr: 'waist', dy: -0.95 }, bg: DAWN(), actors: [{ def: harryRaven, id: 'harry', x: 1250, y: 1000, s: 1.1, turn: -0.5, pose: 'raiseHand', expr: 'calm' }] },
...
ep.panel(1000, { cam: { x: 600, y: 540, w: 1100 }, bg: DAWN(), actors: [{ def: terry, id: 'terry', x: 500, y: 960, s: 1.05, turn: 0.4, pose: 'point', expr: 'suspicious' }, { def: clearwater, id: 'penelope', x: 260, y: 960, turn: 0.4, pose: 'crossArms', expr: 'smile' }, { def: harryRaven, id: 'harry', x: 1050, y: 1000, s: 1.1, turn: -0.5, pose: 'stand', expr: 'neutral' }] },
  [say('Terry', 'Wait, is that…', 470, 64, { anchor: 'tc', w: 240, fixed: true }),
   say('Penelope', 'Don\'t worry. There were no loopholes. Well-worded, Potter. You should be a lawyer.', 280, 360, { w: 320, fixed: true }),
   say('Harry', 'Thank you. I think.', 600, 450, { anchor: 'tc', w: 240, fixed: true })], { mood: 'warm' });
...
ep.panel(1000, { cam: { x: 1180, y: 600, w: 900 }, bg: DAWN({ harryNote: true }), actors: [H1(), () => CS.dormBlanket({ lift: 40 }), { def: harryRaven, id: 'harry', x: 1380, y: 1010, s: 1.1, turn: -0.5, pose: 'reach', expr: 'scheme', armB: { sh: 90, el: 20, hand: 'hold' } }], over: (e) => { const a = e.anchors.harry1; return a ? g({ transform: `translate(${a.head[0] + 80},${a.head[1] + 110}) rotate(-12)` }, P2.cloak(220, 120, { fade: 0.9 })) : ''; } },
...
ep.multi(1500, [
  ...
  say('Harry', 'I was told it was spoken by a hollow voice that belled forth from a gap within the air itself…', 556, 590, { anchor: 'tc', w: 315, size: 28, fixed: true }),
...
ep.bleed(960, { cam: { x: 1090, y: 690, w: 1500 }, bg: COR, actors: [
  ...
  [cold('Harry', 'Hello. I\'m the Boy-Who-Lived.', 618, 620, { w: 300, fixed: true, tail: 'harry1' }),
