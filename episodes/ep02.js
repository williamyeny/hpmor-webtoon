...
const tableFg = () => O.dinnerTable();
ep.panel(760, { cam: { x: 700, y: 640, w: 1150 }, bg: () => O.kitchen(), fg: tableFg,
  actors: [{ def: dad, id: 'dad', x: 380, y: 1020, turn: 0.45, pose: 'sit', expr: 'deadpan' }, { def: mum, id: 'mum', x: 1020, y: 1010, turn: -0.45, pose: 'sit', expr: 'sad' }, { def: harry, id: 'harry', x: 700, y: 1000, s: 1.1, turn: 0, pose: 'sit', expr: 'unimpressed' }] },
  [cap('That evening. Dinner.', 44, 34, { w: 300 }), note('clink', 180, 540, { rot: -8, size: 30 }), note('scrape', 610, 600, { rot: 6, size: 30 })], { mood: 'candle', alt: 'Three people eat dinner in total silence.' });
ep.panel(420, { cam: { on: ['harry'], fr: 'close' }, bg: () => O.kitchen(), blur: 2, actors: [{ def: harry, id: 'harry', x: 700, y: 1000, s: 1.1, turn: 0, pose: 'sit', expr: { base: 'deadpan', eyes: { lookX: -0.8 } } }] },
  [inner('Harry', 'A jiffy or two, she\'d said. That was four hours ago.', 250, 330, { w: 380 })], { mood: 'candle' });
ep.beat(420, [], { over: (t) => FX.sfxText(400, 260, 'DING-DONG', { size: 110, rot: -4 }) });

// =============================================================== the doorstep
const DS = () => O.doorstep();
ep.panel(1000, { cam: { x: 900, y: 580, w: 1000 }, bg: DS,
...
// =============================================================== ground rules
ep.panel(820, { cam: { x: 1100, y: 660, w: 1300 }, bg: LR(),
  actors: [DAD({ pose: 'crossArms', expr: 'unimpressed' }), MCG({ pose: 'stand', expr: 'calm', turn: -0.2 }), MUM({ pose: 'hold', expr: 'worried' }), HAR({ pose: 'lecture', expr: 'focus', x: 1460, turn: -0.45 })] },
  [say('Harry', 'Now, just to be clear. If the Professor levitates you, Dad, and you know you haven\'t been attached to any wires—that\'s sufficient evidence.', 490, 90, { w: 440 }),
   say('Harry', 'No turning round afterwards and saying it was a magician\'s trick. If you feel that way, say so *now,* and we\'ll design a different experiment.', 470, 700, { w: 440 })], { mood: 'warm' });
ep.panel(560, { cam: { on: ['dad'], fr: 'close' }, bg: LR(), blur: 2, actors: [DAD({ expr: { base: 'exasperated' } })] },
  [say('Dad', 'Yes, Harry.', 560, 110, { w: 180 }), cap('The Professor rolled his eyes.', 44, 440, { w: 300 })], { mood: 'warm' });
ep.panel(640, { cam: { on: ['harry', 'mum'], fr: 'bust' }, bg: LR(), actors: [MUM({ expr: 'worried', pose: 'hold' }), HAR({ pose: 'point', expr: 'focus', x: 1480, turn: -0.5 })] },
  [say('Harry', 'And Mum—if it *doesn\'t* happen, you admit you were mistaken. No saying magic doesn\'t work when people are sceptical.', 300, 90, { w: 420 })], { mood: 'warm' });
ep.panel(700, { cam: { on: ['mcgonagall'], fr: 'bust' }, bg: LR(), blur: 1, actors: [MCG({ expr: { base: 'smile', mouth: { type: 'smirk' } }, pose: 'wand', armB: { sh: 30, el: 50, hand: 'hold', prop: WAND } })] },
...
ep.panel(620, { cam: { on: ['harry'], fr: 'bust' }, bg: LR(), blur: 2, actors: [HAR({ expr: 'think', pose: 'chin' })] },
  [say('Harry', '*Sufficient?* Probably not. But it will *help.*', 270, 90, { w: 320 }),
   say('Harry', 'Go ahead, Deputy Headmistress.', 290, 520, { w: 290 })], { mood: 'warm' });
...
ep.panel(640, { cam: { on: ['mcgonagall'], fr: 'waist', dx: 0.6 }, bg: LR(), actors: [MCG({ expr: 'focus', pose: 'wandUp', armB: { sh: 128, el: -12, hand: 'hold', prop: WAND } })], over: sparkTrail },
  [shout('McGonagall', '*Wingardium Leviosa.*', 560, 110, { w: 320, size: 34, weight: 400 })], { mood: 'warm' });
ep.bleed(1300, { cam: { x: 860, y: 560, w: 760 }, bg: LR(),
  actors: [{ def: dad, id: 'dad', x: 760, y: 720, turn: 0.2, pose: 'panic', expr: 'blank', armF: { sh: -60, el: -20, hand: 'open', prop: g({ transform: 'translate(0,30) rotate(160)' }, bookHeld('#43302a', { w: 60, h: 76 })) } }],
...
ep.panel(700, { cam: { on: ['harry'], fr: 'waist' }, bg: LR(), actors: [HAR({ expr: 'think', pose: 'think', turn: -0.3 })] },
  [say('Harry', 'That\'s a bit of an anticlimax. You\'d think there\'d be some more dramatic mental event associated with updating on an observation of infinitesimal probability—', 300, 110, { w: 440 })], { mood: 'warm' });
ep.panel(640, { cam: { on: ['dad', 'mcgonagall', 'mum'], fr: 'bust' }, bg: LR(), actors: [DAD({ expr: 'deadpan' }), MCG({ expr: 'deadpan', turn: 0.3 }), MUM({ expr: 'deadpan' })] },
  [cap('Mum, the witch, and even Dad were giving him *that look* again.', 44, 30, { w: 440 })], { mood: 'warm' });
...
ep.panel(700, { cam: { on: ['mcgonagall'], fr: 'bust' }, bg: LR(), actors: [MCG({ expr: 'warm' })] },
  [say('McGonagall', 'Would you like a further demonstration, Mr Potter?', 540, 110, { w: 320 })], { mood: 'warm' });
ep.panel(700, { cam: { on: ['harry'], fr: 'bust' }, bg: LR(), actors: [HAR({ expr: 'think', pose: 'crossArms', turn: -0.3 })] },
  [say('Harry', 'You don\'t have to. We\'ve performed a definitive experiment.', 260, 100, { w: 330 }),
   say('Harry', 'But…', 600, 380, { w: 120 })], { mood: 'warm' });
ep.panel(600, { cam: { on: ['harry'], fr: 'close' }, bg: LR(), blur: 2, actors: [HAR({ expr: 'delight', turn: -0.1 })], under: (e) => FX.burst(e.w, e.h, e.w / 2, e.h / 2, { col: '#f0c878', op: 0.5 }) },
...
ep.panel(760, { cam: { x: 1200, y: 830, w: 640 }, bg: LR(),
  actors: [g({ transform: 'translate(1010,1012) scale(0.75)' }, cat({ col: '#9b7a52', spectacles: true })),
    { def: harry, id: 'harry', x: 1380, y: 1050, s: 1.12, turn: -0.6, pose: 'fallBack', expr: 'horror' },
    g({ transform: 'translate(1480,850) rotate(30)' }, bookHeld('#274060', { w: 60, h: 80 })), g({ transform: 'translate(1300,800) rotate(-40)' }, bookHeld('#7b2433', { w: 50, h: 70 })), g({ transform: 'translate(1560,960) rotate(80)' }, bookHeld('#2f5a40', { w: 50, h: 66 }))],
...
ep.panel(520, { cam: { on: ['mcgonagall'], fr: 'close' }, bg: LR(), blur: 2, actors: [MCG({ expr: 'calm' })] },
...
ep.panel(640, { cam: { on: ['harry'], fr: 'bust' }, bg: LR(), actors: [{ def: harry, id: 'harry', x: 1380, y: 1050, s: 1.12, turn: -0.3, pose: 'fists', expr: 'yell' }] },
  [shout('Harry', 'Magic *isn\'t enough* to do that! You\'d have to be a *god!*', 360, 110, { w: 440, size: 34 })], { mood: 'warm' });
...
ep.bleed(1250, (ctx) => rect(0, 0, ctx.w, ctx.h, { fill: '#1a2238' }) + FX.physicsDrain(ctx.w, ctx.h, ctx.w * 0.5, ctx.h * 0.55, { n: 22 }),
  [dark('A blur came over Harry\'s vision as his brain began to comprehend what had just broken.', 400, 120, { w: 560 }),
   dark('Three thousand years of discovering that the music of the planets was the same tune as a falling apple. That the true laws were universal, with no exceptions anywhere. That the mind was the brain, and the brain was made of neurons—', 400, 950, { w: 600 })],
...
ep.panel(680, { cam: { on: ['harry'], fr: 'bust' }, bg: LR(), actors: [{ def: harry, id: 'harry', x: 1380, y: 1050, s: 1.12, turn: -0.35, pose: 'gesture', expr: 'suspicious' }] },
  [say('Harry', 'And—and what kind of incantation is *Wingardium Leviosa?* Who invents the words to these spells, nursery schoolers?', 300, 110, { w: 440 })], { mood: 'warm' });
ep.panel(700, { cam: { on: ['mcgonagall'], fr: 'bust' }, bg: LR(), actors: [MCG({ expr: 'stern' })] },
  [say('McGonagall', 'That will do, Mr Potter.', 540, 90, { w: 260 }),
   say('McGonagall', 'If you wish to learn about magic, I suggest that we finalise the paperwork so that you can go to Hogwarts.', 520, 560, { w: 360 })], { mood: 'warm' });
ep.panel(640, { cam: { on: ['harry'], fr: 'close' }, bg: LR(), blur: 2, actors: [{ def: harry, id: 'harry', x: 1380, y: 1050, s: 1.12, turn: -0.2, expr: 'determined' }] },
  [inner('Harry', 'Right. The March of Reason would just have to start over, that was all. They still had the experimental method—that was the important thing.', 400, 110, { w: 560 }),
   say('Harry', 'How do I get to Hogwarts, then?', 540, 540, { w: 280 })], { mood: 'warm' });
ep.panel(420, { cam: { on: ['mcgonagall'], fr: 'eyes' }, bg: LR(), blur: 2, actors: [MCG({ expr: 'laugh' })] },
  [cap('A choked laugh escaped Professor McGonagall, as if extracted from her by tweezers.', 44, 30, { w: 440 })], { mood: 'warm' });

// =============================================================== his condition
ep.panel(700, { cam: { on: ['dad', 'mcgonagall'], fr: 'bust' }, bg: LR(), actors: [DAD({ expr: 'worried', pose: 'gesture' }), MCG({ expr: 'suspicious', turn: -0.5 })] },
  [say('Dad', 'Hold on a moment, Harry. Remember why you haven\'t been going to school? What about your *condition?*', 250, 100, { w: 360 }),
   say('McGonagall', 'His condition? What\'s this?', 560, 580, { w: 260 })], { mood: 'warm' });
const sleepClock = (ctx) => {
  const cx = ctx.w / 2, cy = ctx.h * 0.55, r = Math.min(ctx.w, ctx.h) * 0.3;
  ...
  marks.forEach((h, i) => { const a = ((h % 12) / 12) * Math.PI * 2 - Math.PI / 2; const x = cx + Math.cos(a) * r * 0.55, y = cy + Math.sin(a) * r * 0.55; out += circle(x, y, 22, { fill: '#2f4f86', opacity: 0.2 + i * 0.15 }) + text(x, y + 8, ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'][i], { 'font-family': 'Caveat', 'font-size': 24, 'text-anchor': 'middle', fill: '#2d2a4a', 'font-weight': 700 }); });
...
ep.panel(760, sleepClock,
  [say('Harry', 'I don\'t sleep right. My sleep cycle is twenty-six hours long. I go to sleep two hours later, every day.', 400, 90, { w: 520, who: 'Harry', tail: null }),
   note('10, 12, 2, 4… all the way round the clock', 400, 700, { w: 560, size: 34 })], { alt: 'A clock diagram: Harry\'s bedtime creeps two hours later every day, all the way round the dial.' });
ep.panel(620, { cam: { on: ['harry', 'mum'], fr: 'bust' }, bg: LR(), actors: [MUM({ expr: 'smile', x: 1250 }), HAR({ expr: 'unimpressed', x: 1480 })] },
  [say('Harry', 'That\'s why I haven\'t been going to a normal school.', 560, 90, { w: 300 }),
   say('Mum', '*One* of the reasons.', 230, 520, { w: 220 })], { mood: 'warm' });
ep.panel(760, { cam: { on: ['mcgonagall'], fr: 'bust' }, bg: LR(), actors: [MCG({ expr: 'think', pose: 'chin' })],
  over: (e) => g({ transform: `translate(${e.w * 0.82},${e.h * 0.72})`, opacity: 0.8 }, path('M-14,-24 L14,-24 L2,0 L14,24 L-14,24 L-2,0Z', { fill: '#f1e6cc', stroke: '#b08d45', 'stroke-width': 3 }), path('M-6,-14 L6,-14 L0,-4Z M-8,20 L8,20 L0,10Z', { fill: '#d9b35c' })) },
  [say('McGonagall', 'Hmmmm. I can\'t recall hearing of such a condition. I\'ll ask Madam Pomfrey…', 300, 90, { w: 380 }),
   say('McGonagall', 'No—I\'m sure this won\'t be a problem. I\'ll find a solution *in time.*', 300, 600, { w: 360 })], { mood: 'warm' });
...
ep.panel(760, { cam: { on: ['harry'], fr: 'waist' }, bg: LR(), actors: [HAR({ expr: 'determined', pose: 'handsHips', turn: -0.3 })] },
  [say('Harry', 'I am a conscientious objector to child conscription,', 300, 100, { w: 360 }),
   say('Harry', 'on grounds that I should not have to suffer for a disintegrating school system\'s failure to provide teachers or study materials of even minimally adequate quality.', 310, 600, { w: 420 })], { mood: 'warm' });
...
ep.panel(560, { cam: { on: ['harry'], fr: 'close' }, bg: LR(), blur: 2, actors: [HAR({ expr: 'rant', turn: -0.1 })], under: (e) => FX.burst(e.w, e.h, e.w / 2, e.h / 2, { col: '#e8b4a0', op: 0.5 }) },
  [shout('Harry', 'SHE DIDN\'T KNOW WHAT A LOGARITHM WAS!', 400, 110, { w: 520, size: 38 })], { mood: 'warm' });
// flashback gag
...
ep.panel(760, { cam: { x: 850, y: 760, w: 760 }, bg: classroom,
  ...
  [cap('Age seven.', 44, 34, { w: 180 }), note('chomp', 620, 580, { size: 44, rot: -10 })], { mood: 'sepia', overlay: (ctx) => FX.memoryEdge(ctx.w, ctx.h), alt: 'Memory: a tiny seven-year-old Harry clamped onto a teacher\'s ankle beneath a blackboard.' });
ep.panel(760, { cam: { on: ['dad', 'mum'], fr: 'bust', padX: 1.1 }, bg: LR(), actors: [DAD({ expr: 'smug', x: 900 }), MUM({ expr: 'smile', x: 1150 })] },
  [say('Mum', 'Of course. Biting her was a very mature response to that.', 560, 90, { w: 300 }),
   say('Dad', 'A well-considered policy for addressing the problem of teachers who don\'t understand logarithms.', 250, 600, { w: 360 })], { mood: 'warm' });
ep.panel(620, { cam: { on: ['harry'], fr: 'bust' }, bg: LR(), actors: [HAR({ expr: 'yell', pose: 'armsUp' })] },
  [shout('Harry', 'I was *seven years old!* How long are you going to keep bringing that up?', 380, 110, { w: 440, size: 32 })], { mood: 'warm' });
ep.panel(560, { cam: { on: ['mum'], fr: 'close' }, bg: LR(), blur: 2, actors: [MUM({ expr: 'warm' })] },
  [say('Mum', 'I know. You bite *one* maths teacher, and they never let you forget it, do they?', 270, 100, { w: 380 })], { mood: 'warm' });
ep.panel(700, { cam: { on: ['harry', 'mcgonagall'], fr: 'bust' }, bg: LR(), actors: [MCG({ expr: 'twitch', x: 1150, turn: 0.3 }), HAR({ expr: 'cross', pose: 'point', x: 1420, turn: -0.5 })] },
...
ep.panel(620, { cam: { on: ['mum'], fr: 'full' }, bg: GDX(), actors: [{ def: mum, id: 'mum', x: 360, y: 1000, turn: 0.6, pose: 'run', expr: 'laugh' }] },
  [say('Mum', 'Excuse me—', 560, 110, { w: 200 })], { mood: 'dusk', alt: 'Mum flees out of the back door.', over: undefined });
...
// =============================================================== flaming zebras
ep.panel(700, { cam: { on: ['mcgonagall'], fr: 'bust' }, bg: LR(), actors: [MCG({ expr: 'exasperated' })] },
  [say('McGonagall', 'Well. I think, under the circumstances, I should avoid taking you to buy your school things until a day or two before term begins.', 520, 120, { w: 380 })], { mood: 'warm' });
ep.panel(640, { cam: { on: ['harry'], fr: 'bust' }, bg: LR(), actors: [HAR({ expr: 'shock', pose: 'shrug' })] },
  [shout('Harry', 'What? Why? The other children already know magic! I have to start catching up *right away!*', 380, 110, { w: 460, size: 30 })], { mood: 'warm' });
ep.panel(720, { cam: { on: ['mcgonagall'], fr: 'close' }, bg: LR(), blur: 2, actors: [MCG({ expr: { base: 'calm', eyes: { open: 0.6 } } })] },
  [say('McGonagall', 'Rest assured, Mr Potter, Hogwarts is quite capable of teaching the basics.', 520, 110, { w: 340 }),
   say('McGonagall', 'And I suspect that if I leave you alone for two months with your schoolbooks, even without a wand, I will return to this house to find…', 480, 560, { w: 420 })], { mood: 'warm' });
ep.bleed(1100, (ctx) => {
...
ep.panel(620, { cam: { on: ['dad', 'mum'], fr: 'bust', padX: 1.1 }, bg: LR(), actors: [DAD({ expr: 'calm', x: 900, turn: 0.2 }), MUM({ expr: 'calm', x: 1150, turn: -0.2 })] },
  [cap('Harry\'s mother and father nodded in perfect unison.', 44, 30, { w: 440 }), note('nod', 250, 330, { size: 40 }), note('nod', 560, 330, { size: 40 })], { mood: 'warm' });
ep.panel(560, { cam: { on: ['harry'], fr: 'close' }, bg: LR(), blur: 2, actors: [HAR({ expr: 'rant', turn: 0 })], under: (e) => FX.burst(e.w, e.h, e.w / 2, e.h / 2, { col: '#e8b4a0', op: 0.5 }) },
  [shout('Harry', 'MUM! DAD!', 400, 110, { w: 360, size: 50 })], { mood: 'warm' });

// =============================================================== goodbye at the door (ADDED beat: the thank-you)
ep.panel(820, { cam: { on: ['harry', 'mcgonagall'], fr: 'waist' }, bg: DS,
  ...
  [say('Harry', 'Professor?', 230, 90, { w: 160 }),
   say('Harry', 'Thank you. For coming. For *showing* us, instead of just telling us.', 250, 250, { w: 330 })], { mood: 'night' });
ep.panel(620, { cam: { on: ['mcgonagall'], fr: 'close' }, bg: DS, blur: 2, actors: [{ def: mcgonagall, id: 'mcgonagall', x: 1180, y: 1060, turn: -0.3, expr: 'warm' }] },
  [cap('Something in her face softened—just for a moment.', 44, 34, { w: 400 }),
   say('McGonagall', 'Goodnight, Mr Potter.', 540, 480, { w: 240 })], { mood: 'night' });
ep.panel(760, { cam: { x: 1220, y: 860, w: 620 }, bg: DS,
  actors: [g({ transform: 'translate(1320,1030) scale(0.8)' }, cat({ col: '#9b7a52', spectacles: true, flip: true }))] },
  [cap('She stepped out into the rain—', 44, 34, { w: 300 }), cap('—and by the bottom step, there was only a tabby cat, trotting briskly away down the wet street.', 360, 620, { w: 380 })], { mood: 'night', alt: 'A tabby cat trots off into the rain.' });
