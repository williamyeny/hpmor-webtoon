...
ep.panel(1000, { cam: { on: ['harry'], fr: 'close', dy: -0.05 }, bg: CH, blur: 3, actors: [HS({ expr: 'cold', turn: 0.2 })], over: (e) => FX.frost(e.w, e.h, 0.55, 71) },
  [cap('He had read a great many fantasy novels. And it was starting to look like he had a mysterious dark side. So he glanced around to make sure no-one was watching, took a deep breath, and made himself angry.', 44, 30, { w: 620, fixed: true }),
   cold('Harry', '*Frigideiro.*', 400, 940, { anchor: 'bc', w: 250, fixed: true })], { mood: 'cold', alt: 'Harry thinks of the bullies, of Draco, and lets the cold rise. Frost at the edges. He says the spell in icy tones.' });
...
ep.panel(820, { cam: { on: ['flitwick'], fr: 'waist', dy: -0.2 }, bg: CH, actors: [FL({ y: 820 })] },
  [shout('Flitwick', 'Oh, *very* good, Miss Granger! Excellent! *Stupendous!*', 400, 64, { anchor: 'tc', w: 480, fixed: true })], { mood: 'warm' });
...
ep.panel(760, { cam: { on: ['hermione'], fr: 'close', dy: 0.1 }, bg: CH, blur: 3, actors: [HE({ turn: -0.2, pose: 'stand', expr: { base: 'delight', eyes: { sparkle: true } } })], over: (e) => FX.sparkles([[e.w * 0.36, e.h * 0.44], [e.w * 0.62, e.h * 0.44]], { r: 30 }) },
  [cap('Hermione\'s eyes lit up with a terrible light of helpfulness. And something in the back of Harry\'s brain screamed in desperate humiliation.', 44, 30, { w: 620, fixed: true })], { mood: 'warm' });
...
ep.panel(1160, { cam: { on: ['harry'], fr: 'close', dy: 0.35 }, bg: CH, blur: 3, actors: [HS({ expr: 'angry', turn: 0, pose: 'stand', armB: {} })], over: (e) => { const a = e.anchors.harry; return a ? K.glow(a.head[0] - a.hr * 0.2, a.head[1] + a.hr * 2.2, 50, '#ffe9a8', 0.8) + g({ transform: `translate(${a.head[0] - a.hr * 0.2},${a.head[1] + a.hr * 2.2}) scale(${a.hr / 200})` }, P2.timeTurner(1, { chain: false })) : ''; } },
...
// explicit camera on McGonagall (feet at y 900, head centre y 443): tile height H, world width w, head at fraction `frac` of the panel height, shifted dx
const mgCam = (H, w, frac = 0.6, dx = 0, x = 1500) => ({ x: x - dx, y: 443 - (frac - 0.5) * w * (H - 36) / 752, w });
const CLASS = (o = {}) => [
...
ep.multi(700, [
  { x: M, y: 18, w: 368, h: 664, mood: 'warm', art: { cam: { x: 1110, y: 640, w: 560 }, bg: TR([]), actors: [MG({ x: 1290, pose: 'wand', expr: 'stern', turn: -0.4, armB: { hand: 'hold', prop: wand(110) } }), TDESK] } },
  { x: 408, y: 18, w: 368, h: 664, mood: 'warm', art: { cam: { x: 1110, y: 640, w: 560 }, bg: TR([]), actors: [MG({ x: 1290, pose: 'wand', expr: 'stern', turn: -0.4, armB: { hand: 'hold', prop: wand(110) } }), () => g({ transform: 'translate(960,930)' }, P2.pig(1.2, { confused: true }))] } },
], [cap('*Tap.*', 44, 34, { w: 120, fixed: true }), cap('Her desk became a pig.', 428, 34, { w: 280, fixed: true })],
...
ep.panel(760, { cam: mgCam(760, 480, 0.62, 120, 1400), bg: TR([]), blur: 2, actors: [MG({ x: 1400, pose: 'lecture', turn: -0.3 })] },
...
ep.panel(900, { cam: { x: 960, y: 680, w: 560 }, bg: TR([]), blur: 2, actors: CLASS({ h: { expr: 'think' } }) },
  [say('McGonagall', 'Can you tell me why this rule exists, Mr Potter?', 400, 64, { anchor: 'tc', w: 440, fixed: true, noTail: true }),
   inner('Harry', '*Because a single error in Transfiguration can be incredibly dangerous.*', 400, 290, { w: 440, fixed: true }),
...
ep.bleed(960, { cam: { x: 1160, y: 405, w: 1000 }, bg: TR(NOTPERM), actors: [MG({ x: 1560, pose: 'point', turn: -0.6, expr: 'yell' })] },
  [shout('McGonagall', 'Transfiguration is not permanent!', 290, 340, { w: 300, size: 28, fixed: true, noTail: true }),
   shout('McGonagall', 'Transfiguration is not permanent!', 320, 575, { w: 330, size: 33, fixed: true, noTail: true }),
   shout('McGonagall', '*Transfiguration is not permanent!*', 350, 900, { anchor: 'bc', w: 380, size: 40, fixed: true })],
...
ep.panel(860, { cam: mgCam(860, 600, 0.55, 150, 1560), bg: TR(NOTPERM), actors: [MG({ x: 1560, pose: 'gesture', turn: -0.4, expr: 'stern' })] },
  [say('McGonagall', 'Mr Potter, suppose a student Transfigured a block of wood into a cup of water, and you drank it. What do you imagine might happen to you when the Transfiguration wore off?', 310, 64, { anchor: 'tc', w: 420, fixed: true })], { mood: 'warm' });
...
ep.panel(940, { cam: mgCam(940, 620, 0.5, 150), bg: TR(NOTPERM), actors: [MG({ x: 1500, expr: 'stern', pose: 'present', turn: -0.4, armB: { sh: 70, el: 40, hand: 'hold', prop: bookOpen({ w: 150, h: 100, col: '#6a2a2a' }) } })] },
  [say('McGonagall', 'As Mr Potter has correctly reasoned, he would become extremely sick, and require immediate Flooing to St Mungo\'s if he was to have any chance of survival.', 320, 64, { anchor: 'tc', w: 440, fixed: true }),
   say('McGonagall', 'Please turn your textbooks to page five.', 195, 740, { w: 240, fixed: true })], { mood: 'warm' });
ep.panel(620, (ctx) => rect(0, 0, ctx.w, ctx.h, { fill: '#5a4030' }) + g({ transform: 'translate(30,32)' }, P2.textbookPage('victim', 350, 520)) + FX.burst(360, 380, 205, 195, { n: 18, op: 0.25 }),
  [cap('Even without any sound in the moving picture, you could tell the woman with the discoloured skin was screaming.', 428, 200, { w: 290, fixed: true })],
  { mood: 'warm', alt: 'A textbook page with a moving picture of a poisoned woman, screaming.' });
...
ep.panel(1060, (ctx) => rect(0, 0, ctx.w, ctx.h, { fill: '#5a4030' }) + g({ transform: `translate(${ctx.w / 2 - 200},290)` }, P2.textbookPage('dementor', 400, 500)) + FX.frost(ctx.w, ctx.h, 0.35, 16),
  [say('McGonagall', 'Page six. That is a Dementor. They are the guardians of Azkaban. They suck away at your magic, your life, and any happy thoughts you try to have.', 400, 64, { anchor: 'tc', w: 540, fixed: true, noTail: true }),
   say('McGonagall', 'The picture on page seven is of the criminal, ten years later, on his release. You will note that he is dead.', 400, 1010, { anchor: 'bc', w: 520, fixed: true, noTail: true })], { alt: 'Page six: a tall, hooded, cold shape. A Dementor.' });
ep.panel(700, { cam: { x: 1010, y: 690, w: 620 }, bg: TR(NOTPERM), actors: CLASS({ h: { pose: 'raiseHand', expr: 'worried', turn: 0.2, armF: { sh: -168, el: 12, hand: 'open' }, armB: { sh: 20, el: 60 } } }) },
...
ep.panel(780, { cam: mgCam(780, 540, 0.55, -150), bg: TR(NOTPERM), actors: [MG({ x: 1500, pose: 'gesture', turn: 0.3, expr: 'stern' })] },
...
ep.panel(1000, { cam: mgCam(1000, 470, 0.71, 50), bg: TR(NOTPERM), blur: 2, actors: [MG({ x: 1500, pose: 'point', expr: 'cross', turn: -0.3 })] },
...
ep.panel(900, { cam: { x: 880, y: 560, w: 940 }, bg: TR(NOTPERM), actors: [MG({ x: 520, y: 960, pose: 'point', expr: 'stern', turn: 0.5 }), ...CLASS({ h: { expr: 'worried' }, he: { expr: 'focus' }, c: [{}, { expr: 'shock' }, { expr: 'worried' }] }).filter((a) => a.id !== 'c0')] },
  [say('McGonagall', 'You will never Transfigure anything that could conceivably go inside anyone\'s body. No food. Nothing that *looks like* food. Not even as a funny little prank. Is that well understood by *every single student?*', 490, 64, { anchor: 'tc', w: 380, fixed: true })], { mood: 'warm', alt: 'McGonagall looms over the front row.' });
...
ep.panel(900, { cam: mgCam(900, 300, 0.74), bg: TR(NOTPERM), blur: 3, actors: [MG({ x: 1500, expr: 'yell', pose: 'fists', turn: 0 })], over: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.55, { n: 30, op: 0.2 }) },
  [shout('McGonagall', '*IS THAT WELL UNDERSTOOD BY EVERY SINGLE STUDENT?*', 400, 84, { anchor: 'tc', w: 470, fixed: true })], { mood: 'warm' });
// the chant
...
ep.panel(620, { cam: { x: 1035, y: 685, w: 600 }, bg: TR(NOTPERM), actors: CLASS({ ...CHORUS, h: { expr: 'yell' }, he: { expr: 'yell' } }) },
  [CHANT('I will never Transfigure anything that is to be burned, because it could make smoke.', 72, { size: 30 }),
   CHANT('I will never Transfigure anything that looks like money.', 192, { size: 32 })],
  { mood: 'warm', alt: 'Louder now.' });
ep.panel(760, { cam: mgCam(760, 560, 0.55, 150), bg: TR(NOTPERM), actors: [MG({ x: 1500, pose: 'lecture', turn: -0.3, expr: 'stern' })] },
  [say('McGonagall', 'Including Muggle money. The goblins have ways of finding out who did it. As a matter of recognised law, the goblin nation is in a permanent state of *war* with all magical counterfeiters.', 318, 70, { anchor: 'tc', w: 410, fixed: true })], { mood: 'warm' });
ep.panel(520, { cam: mgCam(520, 360, 0.5, -95), bg: TR(NOTPERM), blur: 3, actors: [MG({ x: 1500, pose: 'stand', turn: 0.1, expr: { base: 'stern', brows: { angle: -12 } } })] },
  [say('McGonagall', 'They will not send Aurors. They will send an *army.*', 540, 250, { w: 280, fixed: true })], { mood: 'warm' });
const RAISED = { pose: 'raiseHand', expr: 'focus', turn: 0.2, armF: { sh: -168, el: 12, hand: 'open' }, armB: { sh: 20, el: 60 } };
ep.panel(700, { cam: mgCam(700, 420, 0.62, 120), bg: TR(NOTPERM), blur: 2, actors: [MG({ x: 1500, pose: 'point', turn: -0.3, expr: 'cross' })] },
...
ep.multi(330, [0, 1, 2, 3].map((k) => ({ x: M + k * 190, y: 18, w: 178, h: 294, mood: 'warm', art: (ctx) => rect(0, 0, ctx.w, ctx.h, { fill: '#b89a6a' }) + g({ transform: `translate(${ctx.w / 2},${ctx.h * 0.42}) scale(1.5)` },
  ...
  [capC('wood', 113, 252, { w: 140, fixed: true }), capC('glass', 303, 252, { w: 140, fixed: true }), capC('*Crystferrium!* steel', 493, 240, { w: 128, size: 25, fixed: true }), capC('wood again', 683, 240, { w: 110, fixed: true })],
  { alt: 'A chunk of wood becomes a glass ball, then (Crystferrium!) a steel ball, then wood again.' });
ep.panel(800, { cam: mgCam(800, 580, 0.55, -150), bg: TR(NOTPERM), actors: [MG({ x: 1500, pose: 'wandUp', turn: 0.3, expr: 'stern', armB: { hand: 'hold', prop: wand(110) } })] },
  [say('McGonagall', 'Free Transfiguration can turn any subject into any target. It must be done wordlessly: by holding the subject, the target, and the transformation, within your own mind.', 510, 64, { anchor: 'tc', w: 380, fixed: true })], { mood: 'warm' });
ep.panel(820, { cam: mgCam(820, 400, 0.66, 20), bg: TR(NOTPERM), blur: 3, actors: [MG({ x: 1500, pose: 'stand', turn: -0.1, expr: { base: 'stern', mouth: { type: 'flat' } } })], over: (e) => rect(0, 0, e.w, e.h, { fill: '#1a1410', opacity: 0.18 }) },
...
ep.panel(1000, { cam: { on: ['harry'], fr: 'bust', dy: -0.2 }, bg: TR(NOTPERM), blur: 2, actors: CLASS({ h: { pose: 'raiseHand', expr: 'think' } }) },
...
ep.panel(700, { cam: mgCam(700, 540, 0.55, -140), bg: TR(NOTPERM), actors: [MG({ x: 1500, pose: 'present', expr: 'stern', turn: 0.3 })] },
...
const WANDUP = { pose: 'wandUp', expr: 'determined', armB: { hand: 'hold', prop: wand(100) } };
...
ep.multi(600, [
  { x: M, y: 96, w: 752, h: 236, mood: 'warm', art: { cam: { on: ['harry'], fr: 'eyes' }, bg: TR(NOTPERM), blur: 3, actors: WAR({ expr: { base: 'suspicious', eyes: { open: 0.55 } }, turn: 0.4 }) } },
  { x: M, y: 346, w: 752, h: 236, mood: 'warm', art: { cam: { on: ['hermione'], fr: 'eyes' }, bg: TR(NOTPERM), blur: 3, actors: WAR({}, { expr: { base: 'smug', eyes: { open: 0.55 } }, turn: -0.4 }) } },
], [cap('The two of them stared into each other\'s eyes, unblinking.', 44, 22, { w: 700, fixed: true })], { alt: 'Eye to eye.' });
ep.bleed(860, { cam: { x: 1040, y: 800, w: 460 }, bg: TR(NOTPERM), actors: WAR({ expr: 'menace', turn: 0.4 }, { expr: 'determined', turn: -0.4 }), over: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.45, { n: 30, op: 0.25 }) },
  [say('Harry', 'Of course you realise this means war.', 220, 60, { anchor: 'tc', w: 300, fixed: true }),
   say('Hermione', 'I didn\'t know we\'d been at peace.', 590, 820, { anchor: 'bc', w: 300, fixed: true })], { alt: 'Harry and Hermione, eye to eye across the desk.' });
...
ep.panel(740, { cam: mgCam(740, 560, 0.56, -150), bg: TR(NOTPERM), actors: [MG({ x: 1500, pose: 'present', expr: 'happy', turn: 0.3 })] },
...
ep.panel(480, { cam: mgCam(480, 360, 0.5, 95), bg: TR(NOTPERM), blur: 3, actors: [MG({ x: 1500, pose: 'stand', expr: 'smug', turn: -0.2 })] },
...
const HUP = (o = {}) => WAR({ pose: 'stand', seat: undefined, y: 1060, turn: 0.3, ...o }, { expr: 'shock' });
...
ep.panel(900, { cam: mgCam(900, 520, 0.72, 120), bg: TR(NOTPERM), actors: [MG({ x: 1500, pose: 'gesture', turn: -0.3, expr: 'smug' })] },
  [say('McGonagall', 'Those ten points are *unquestionably* deserved, Mr Potter. My, I wonder if any other student has ever earned *seventeen* House points on his first day of lessons? I\'ll have to look it up. Perhaps we should have an announcement at dinner?', 400, 64, { anchor: 'tc', w: 540, fixed: true })], { mood: 'warm' });
ep.panel(760, { cam: { x: 1010, y: 760, w: 480 }, bg: TR(NOTPERM), actors: HUP({ expr: 'yell', pose: 'fists' }), over: (e) => FX.burst(e.w, e.h, e.w * 0.33, e.h * 0.6, { n: 26, op: 0.2 }) },
  [shout('Harry', '*PROFESSOR!* This is *our* war! Stop *meddling!*', 440, 80, { anchor: 'tc', w: 400, fixed: true })], { mood: 'warm' });
ep.panel(760, { cam: mgCam(760, 520, 0.58, -140), bg: TR(NOTPERM), actors: [MG({ x: 1500, pose: 'chin', turn: 0.1, expr: 'smile' })] },
  [say('McGonagall', 'Now you have until Thursday of *next* week, Mr Potter. Unless, of course, you engage in some sort of mischief and *lose* House points before then. Addressing a professor disrespectfully, for example.', 505, 64, { anchor: 'tc', w: 400, fixed: true })], { mood: 'warm', alt: 'McGonagall, a finger on her cheek, looking reflective.' });
ep.panel(460, { cam: mgCam(460, 400, 0.5, 130), bg: TR(NOTPERM), blur: 3, actors: [MG({ x: 1500, pose: 'stand', expr: { base: 'smile', eyes: { open: 0.6 } }, turn: -0.2 })] },
  [say('McGonagall', 'I expect you\'ll hit negative numbers before the end of Friday.', 290, 230, { w: 380, fixed: true })], { mood: 'warm' });
...
ep.panel(760, { cam: mgCam(760, 560, 0.6, 150), bg: TR(NOTPERM), actors: [MG({ x: 1500, pose: 'present', turn: -0.4, expr: 'smile' })] },
  [say('McGonagall', '…and if anyone comes to you for help with their schoolwork, and is disappointed that you haven\'t even started reading your textbooks, you can always refer them to Miss Granger.', 290, 64, { anchor: 'tc', w: 400, fixed: true })], { mood: 'warm' });
ep.panel(560, { cam: { x: 1150, y: 800, w: 520 }, bg: TR(NOTPERM), actors: HUP({ expr: 'shock' }).map((a) => (a.id === 'hermione' ? { ...a, expr: 'yell', pose: 'fists' } : a)), over: (e) => FX.burst(e.w, e.h, e.w * 0.62, e.h * 0.5, { n: 24, op: 0.2 }) },
  [shout('Hermione', '*Professor!*', 290, 130, { w: 240, fixed: true })], { mood: 'warm' });
ep.panel(600, { cam: mgCam(600, 400, 0.52, -120), bg: TR(NOTPERM), blur: 3, actors: [MG({ x: 1500, pose: 'stand', turn: 0.2, expr: 'smile' })] },
  [say('McGonagall', 'My, I wonder how long it will take before Miss *Granger* does something deserving of a dinner-time announcement? I look forward to seeing it.', 530, 290, { w: 340, fixed: true })], { mood: 'warm' });
ep.bleed(640, { cam: { x: 930, y: 610, w: 900 }, bg: () => CS.corridor({ seed: 31, windows: [400, 1600], torches: [1000] }), actors: [{ def: harryRaven, id: 'harry', x: 1250, y: 900, s: 1.1, turn: 0.7, pose: 'walk', expr: 'angry' }, { def: hermioneRaven, id: 'hermione', x: 1050, y: 900, s: 1.1, turn: 0.7, pose: 'walk', expr: 'cross' }, ...[0, 1, 2, 3].map((i) => ({ def: [padma, anthony, terry, michael][i], id: 'r' + i, x: 800 - i * 170, y: 900, s: 1.05, turn: 0.6, pose: 'walk', expr: 'awe' }))] },
  [cap('Harry and Hermione, by unspoken mutual consent, turned and stormed out of the classroom. They were followed by a trail of hypnotised Ravenclaws.', 44, 40, { w: 620, fixed: true })], { alt: 'Harry and Hermione storm down the corridor side by side; a line of Ravenclaws trails behind, entranced.' });
...
// explicit camera on a walking kid (feet y 900, head centre ≈ 576)
const kidCam = (H, w, frac, x) => ({ x, y: 576 - (frac - 0.5) * w * (H - 36) / 752, w });
ep.panel(960, { cam: { x: 1100, y: 630, w: 640 }, bg: COR, actors: [HEW({ expr: 'smug' }), HW({ expr: 'embarrassed' })] },
  [say('Harry', 'Um. Are we still on for after dinner?', 540, 50, { anchor: 'tc', w: 300, fixed: true }),
   say('Hermione', 'Of course. I wouldn\'t want you to fall *further behind* on your studying.', 250, 150, { anchor: 'tc', w: 330, fixed: true }),
   say('Harry', 'Why, thank you. And as brilliant as you are already, I can\'t help but wonder what you\'ll be like once you have some elementary training in rationality.', 400, 910, { anchor: 'bc', w: 540, fixed: true })], { mood: 'warm' });
ep.panel(960, { cam: { x: 1100, y: 630, w: 640 }, bg: COR, actors: [HEW({ expr: 'smug', pose: 'stand' }), HW({ expr: 'deadpan' })] },
  [say('Hermione', 'Is it really that useful? It didn\'t seem to help you with Charms or Transfiguration.', 255, 50, { anchor: 'tc', w: 330, fixed: true }),
   cap('There was a slight pause.', 440, 290, { w: 300, fixed: true }),
...
ep.panel(1000, { cam: kidCam(1000, 420, 0.5, 960), bg: COR, actors: [HEW({ expr: 'smug', pose: 'gesture', turn: 0.2 })] },
...
ep.panel(900, { cam: { on: ['harry'], fr: 'close', dy: 0.1 }, bg: COR, blur: 3, actors: [HW({ expr: { base: 'scheme', eyes: { lookX: 0.3 } }, turn: -0.1 })], over: (e) => { const a = e.anchors.harry; return a ? K.glow(a.head[0] - a.hr * 0.2, a.head[1] + a.hr * 2.2, 60, '#ffe9a8', 0.9) + g({ transform: `translate(${a.head[0] - a.hr * 0.2},${a.head[1] + a.hr * 2.2}) scale(${a.hr / 200})` }, P2.timeTurner(1, { chain: false })) : ''; } },
