...
// the tiers, seen from the stage. who: overrides by id (false leaves that seat empty)
const SEATS = (o = {}) => {
  const who = o.who || {};
  const R = rng(3);
  const rows = [
  ...
  for (let k = rows.length - 1; k >= 0; k--) {
    for (const [def, id, x] of rows[k]) { const turn = R.range(-0.15, 0.15); if (who[id] !== false) out.push({ def, id, x, y: rowY(k), s: 1.05, turn, pose: 'sit', seat: 140, expr: 'neutral', ...who[id] }); }
    out.push(() => CS.tierFront(k, { screens: rows[k].map(([, , x]) => x + 95), lit: o.lit }));
  }
  return out;
};
const HB = (o = {}) => ({ def: harryRaven, id: 'harry', x: 1450, y: rowY(4), s: 1.1, turn: -0.1, pose: 'sit', seat: 140, ...o });

// Quirrell slumped at his desk; the drool is scaled to his head so it reads at any zoom
const DROOL = onActor('quirrell', (a) => { const k = a.hr / 40; return path(`M${a.mouth[0] + 8 * k},${a.mouth[1] + 7 * k} q${3 * k},${14 * k} ${1 * k},${26 * k}`, { fill: 'none', stroke: '#bfe0f0', 'stroke-width': Math.max(2, 2.6 * k), 'stroke-linecap': 'round' }) + circle(a.mouth[0] + 9 * k, a.mouth[1] + 34 * k, 3 * k, { fill: '#bfe0f0' }); });
...
ep.panel(760, { cam: { x: 1420, y: rowY(4) - 275, w: 640 }, bg: TI, actors: [HB({ x: 1420, turn: 0.15, headTilt: 8, expr: { base: 'focus', eyes: { lookY: 0.6 } } }), () => CS.tierFront(4, { screens: [1100, 1740] }), () => g({ transform: `translate(1420,${CS.FLOOR - 4 * CS.ROW - 90}) rotate(-4)` }, bookOpen({ w: 150, h: 90, col: '#3b3242' })), () => CS.tierFront(3, { screens: [1000, 1300, 1620, 1920] })] },
...
const MIN = (d = {}, c = {}, go = {}, h = {}) => [GO(go), CR(c), DR(d), HB({ turn: -0.4, ...h }), () => CS.tierFront(4, { x0: 1290, screens: [1660, 1980] })];
...
// the knuckle-crack (a fist pushed into a palm) isn't a stock hand shape, so it's drawn over the near hand
...
ep.multi(760, [
  ...
  { x: 408, y: 18, w: 368, h: 724, mood: 'candle', art: { cam: { on: ['crabbe'], fr: 'close', zoom: 1.2, dy: 0.05 }, bg: TI, blur: 2, actors: MIN({}, { pose: 'stand', lean: 6, turn: 0.25, poseMod: KNUCK }), over: onActor('crabbe', CRACK) } },
...
ep.panel(820, { cam: { x: 1070, y: -165, w: 480 }, bg: TI, blur: 2, actors: MIN({ expr: DYELL, turn: -0.4, pose: 'fists' }), over: onActor('draco', (a) => FX.emanata(a.head[0], a.head[1] - a.hr * 0.4, a.hr * 1.7, { n: 7 })) },
...
ep.panel(620, { cam: { on: ['harry'], fr: 'close' }, bg: TI, blur: 3, actors: [HB({ expr: 'deadpan' })] },
...
ep.multi(960, [
  ...
  { x: M, y: 560, w: 360, h: 382, mood: 'candle', shadow: true, art: { cam: { head: 'harry', hw: 0.36, hx: 0.42, hy: 0.46 }, bg: TI, actors: [HB({ x: 1400, turn: 0.35, expr: { base: 'teary', eyes: { lookY: -0.3, lookX: 0.6 } } }), () => CS.tierFront(4, { screens: [1580], lit: true })] } },
...
ep.panel(1150, { cam: { x: 1000, y: 350, w: 930 }, bg: ST(), actors: [Q({ pose: 'present', expr: 'coldSmile' }), QDESK, BACKS(930, [420, 760, 1100, 1440, 1780], 1.2)] },
...
ep.panel(720, { cam: { x: 820, y: 560, w: 700 }, bg: TI, actors: [...SEATS({ lit: true, who: { dean: DOWN('focus'), g1: DOWN('worried'), draco: DOWN('focus'), ernie: DOWN('worried'), neville: DOWN('worried'), h1: DOWN('focus'), padma: false, hermione: false, anthony: false, g2: false, g3: false } }), SCRIB([[400, 0], [650, 0], [950, 0], [300, 1], [560, 1], [820, 1]])] },
  [cap('There was a certain amount of frantic scrabbling for parchment.', 44, 30, { w: 560, fixed: true })], { mood: 'candle' });
ep.panel(900, { cam: { on: ['quirrell'], fr: 'waist', dy: -0.9 }, bg: ST(), blur: 2, actors: [QDESK, Q({ pose: 'raiseHand', expr: 'stern', turn: 0.15 })] },
...
ep.panel(1000, { cam: { x: 1330, y: 470, w: 780 }, bg: ST(), actors: [Q({ x: 1450, y: 920, pose: 'walk', expr: 'coldSmile', turn: -0.55 })] },
...
ep.panel(1000, { cam: { x: 1070, y: 360, w: 480 }, bg: ST(), actors: [QDESK, Q({ pose: 'wandUp', expr: 'coldSmile', turn: 0.2, armB: WAND() })], over: onActor('quirrell', (a) => K.glow(a.handB[0] + a.hr * 1.5, a.handB[1] - a.hr * 0.8, a.hr * 1.6, '#6ad07a', 0.6) + circle(a.handB[0] + a.hr * 1.5, a.handB[1] - a.hr * 0.8, a.hr * 0.18, { fill: '#c8ffb0' })) },
...
ep.panel(900, { cam: { head: 'quirrell', hw: 0.24, hx: 0.48, hy: 0.68 }, bg: ST(), blur: 2, actors: [QDESK, Q({ pose: 'holdUp', expr: { base: 'unimpressed', eyes: { lookY: -0.4, lookX: 0.4 } }, turn: 0.2, armB: { hand: 'hold', under: g({ transform: 'translate(0,84)' }, bookHeld('#6a5a3a', { rot: 180, w: 70, h: 94 })) } })] },
...
ep.panel(1050, { cam: { x: 1080, y: 515, w: 820 }, bg: ST(), actors: [QDESK, Q({ x: 1080, y: 960, pose: 'shrug', expr: 'smile', turn: -0.15 }), BACKS(1130, [700, 1480], 1.5)] },
...
ep.panel(1000, { cam: { x: 1010, y: 175, w: 330 }, bg: ST(), blur: 3, actors: [Q({ turn: 0.35, expr: { base: 'calm', eyes: { style: 'cold', open: 0.7 } } })], over: DOOMV(71, 0.8) },
...
ep.panel(574, { cam: { head: 'quirrell', hw: 0.8, hx: 0.5, hy: 0.4 }, bg: ST(), blur: 3, actors: [Q({ turn: 0.05, expr: { base: 'calm', eyes: { style: 'cold', open: 0.62 }, brows: { raise: -0.3, inner: -0.5 } } })], over: (e) => DOOMV(72, 1.8)(e) + LIDS(e) },
...
ep.bleed(1320, { cam: { x: 1060, y: 350, w: 600 }, bg: ST(), actors: [QDESK, Q({ pose: 'point', expr: { base: 'yell', eyes: { open: 0.85, style: 'normal', lidTilt: -0.35 }, brows: { raise: -0.2, inner: -1.3, outer: 0.4 }, mouth: { type: 'shout', open: 0.9 } }, turn: 0.1 })] },
...
ep.cutout(1150, { cam: { head: 'quirrell', hw: 0.175, hx: 0.5, hy: 0.15 }, actors: [Q({ pose: 'armsUp', expr: { base: 'yell', eyes: { open: 1, style: 'normal' }, brows: { raise: 0.7, inner: 0.2 }, mouth: { type: 'shout', open: 1.1 } }, turn: 0 })], behind: headBurst('quirrell', 34, 0.2, 4) },
...
// Harry claps: two palms pressed together, drawn over his hands
...
ep.panel(820, { cam: { x: 1330, y: rowY(4) - 230, w: 620 }, bg: TI, actors: [{ def: terry, id: 'terry', x: 1130, y: rowY(4), s: 1.05, turn: 0.35, pose: 'sit', seat: 140, expr: 'shock' }, HB({ x: 1450, pose: 'stand', expr: 'bigGrin', armF: { sh: 22, el: 100, hand: 'palm' }, armB: { sh: -22, el: -100, hand: 'palm', front: true }, turn: -0.15 }), () => CS.tierFront(4, { screens: [1250, 1600], lit: true })], over: onActor('harry', (a) => CLAP(a) + FX.emanata(a.handF[0], a.handF[1], a.hr * 0.6, { n: 5 })) },
...
ep.panel(900, { cam: { x: 1000, y: 355, w: 760 }, bg: ST(), actors: [QDESK, Q({ y: 800, pose: 'sit', seat: 250, expr: 'smile', turn: 0.15 })] },
...
ep.multi(720, [
  ...
  { x: 408, y: 18, w: 368, h: 684, mood: 'candle', art: { cam: { on: ['harry'], fr: 'close', zoom: 1.15, dy: 0.3 }, bg: TI, blur: 2, actors: SEATS({ lit: true, who: { harry: { expr: { base: 'delight', eyes: { sparkle: true } } } } }), over: onActor('harry', (a) => FX.sparkles([[a.head[0] - a.hr * 1.25, a.head[1] - a.hr * 0.2, a.hr * 0.22], [a.head[0] + a.hr * 1.3, a.head[1] - a.hr * 0.5, a.hr * 0.28], [a.head[0] + a.hr * 1.45, a.head[1] + a.hr * 0.3, a.hr * 0.16]], { stroke: C.ink })) } },
...
ep.panel(1100, { cam: { x: 1000, y: 270, w: 820 }, bg: ST(), actors: [Q({ pose: 'gesture', expr: 'scheme', turn: 0.15 }), QDESK, BACKS(800, [600, 930, 1260, 1590], 1.3)] },
...
ep.panel(1100, { cam: { x: 1020, y: 140, w: 480 }, bg: ST(), blur: 2, actors: [QDESK, Q({ pose: 'pointUp', expr: 'scheme', turn: 0.2 })] },
...
ep.panel(560, { cam: { on: ['harry'], fr: 'close' }, bg: TI, blur: 3, actors: [HB({ expr: 'scheme' })] },
  [inner('Harry', '*Hot* damn. *That would be Harry.*', 400, 64, { anchor: 'tc', w: 380, fixed: true })], { mood: 'candle' });
ep.panel(950, { cam: { x: 1150, y: 380, w: 1000 }, bg: ST(), actors: [QDESK, Q({ pose: 'present', expr: 'coldSmile', turn: 0.3 })] },
...
const LINE = (h) => [...[[padma, 'padma', 860], [dean, 'dean', 1320], [ernie, 'ernie', 1090]].map(([def, id, x]) => ({ def, id, x, y: 960, s: 1.05, turn: 0.3, pose: 'wandUp', expr: 'focus', armB: WAND() })), PR({ x: 1560, turn: 0.3, ...h })];
ep.bleed(1100, { cam: { x: 1270, y: 420, w: 1000 }, bg: ST({ spheres: 8 }), actors: LINE({ expr: { base: 'focus', eyes: { lookY: -0.5, lookX: 0.2 } } }), over: onActor('harry', (a, e) => { const t = TIP(a), sp = e.toPanel(SPH8[6]); return P2.bolt(t[0], t[1], sp[0], sp[1] + 20, { seed: 3 }); }) },
...
ep.panel(1100, { cam: { on: ['harry'], fr: 'waist', dy: -0.2, zoom: 0.85, roll: -5 }, bg: ST({ spheres: 6 }), actors: [PR({ expr: { base: 'bigGrin', eyes: { sparkle: true } }, pose: 'wandUp', turn: -0.1 })], behind: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.45, { n: 26, col: '#ff9a7a', op: 0.2 }), over: onActor('harry', (a, e) => { const t = TIP(a); return P2.bolt(t[0], t[1], e.w * 0.06, e.h * 0.22, { seed: 5 }) + P2.bolt(t[0], t[1], e.w * 0.3, e.h * 0.12, { seed: 9 }); }) },
...
// the class on the stage: Ravenclaws left, Harry, Slytherins right. who: overrides by id (false leaves them out)
const CROWD = (o = {}) => [
  ...[[padma, 'padma', 200], [anthony, 'anthony', 380], [terry, 'terry', 560], [michael, 'michael', 740]].map(([def, id, x]) => ({ def, id, x, y: 1040, s: 1.05, turn: 0.2, expr: 'worried' })),
  ...(o.harry !== false ? [{ def: harryRaven, id: 'harry', x: 1650, y: 1040, s: 1.1, turn: -0.3, expr: 'worried', ...o.h }] : []),
  ...[[dracoSly, 'draco', 1300], [crabbe, 'crabbe', 1150], [goyle, 'goyle', 1450], [zabini, 'zabini', 1850]].map(([def, id, x]) => ({ def, id, x, y: 1060, s: 1.08, turn: -0.2, expr: 'smug' })),
].filter((a) => o.who?.[a.id] !== false).map((a) => ({ ...a, ...o.who?.[a.id] }));
ep.panel(1000, { cam: { x: 1000, y: 520, w: 1500 }, bg: ST(), actors: [QDESK, Q({ x: 1200, pose: 'present', expr: 'calm' }), DA({ pose: 'walk' }), ...CROWD({ h: { expr: 'cross' } })] },
...
ep.panel(1100, { cam: { x: 1020, y: 390, w: 700 }, bg: ST(), actors: [QDESK, Q({ x: 1200, pose: 'gesture', expr: 'coldSmile', turn: -0.3 }), DA({ expr: 'shock' })] },
...
ep.panel(1000, { cam: { on: ['hermione'], fr: 'bust', dy: -0.1 }, bg: ST(), blur: 2, actors: [DA({ expr: { base: 'pained', eyes: { open: 1 } }, pose: 'wand', armB: WAND() })], over: onActor('hermione', (a) => FX.emanata(a.head[0], a.head[1], a.hr * 1.5, { n: 6 })) },
...
ep.panel(820, { cam: { x: 1650, y: 820, w: 420 }, bg: ST(), actors: [{ def: harryRaven, id: 'harry', x: 1650, y: 1040, s: 1.1, turn: -0.3, pose: 'stand', expr: 'determined', armF: { sh: 2, el: 132, hand: 'palm' } }], over: onActor('harry', TAP) },
...
ep.panel(560, { cam: { x: 1000, y: 520, w: 1500 }, bg: ST(), actors: [QDESK, Q({ x: 1200 }), DA({ expr: 'calm' }), ...CROWD({})] },
...
ep.panel(1060, { cam: { x: 1010, y: 440, w: 720 }, bg: ST(), actors: [QDESK, Q({ x: 1200, pose: 'stand', expr: 'calm', turn: -0.3 }), DA({ expr: { base: 'determined', eyes: { teary: true } } })] },
...
// Draco
ep.panel(1100, { cam: { x: 1100, y: 520, w: 1500 }, bg: ST(), actors: [QDESK, Q({ x: 1250, pose: 'point', expr: 'coldSmile' }), { def: dracoSly, id: 'draco', x: 950, y: 700, s: 1.1, turn: 0.2, pose: 'stand', expr: 'smug' }, ...CROWD({ who: { draco: false } })] },
...
ep.panel(1150, { cam: { x: 1100, y: 410, w: 640 }, bg: ST(), actors: [QDESK, Q({ x: 1250, pose: 'stand', expr: 'coldSmile', turn: -0.3 }), { def: dracoSly, id: 'draco', x: 950, y: 700, s: 1.1, turn: 0.3, pose: 'stand', expr: 'smug' }] },
...
// ---------------------------------------------------------------- Harry
ep.panel(1000, { cam: { on: ['quirrell'], fr: 'bust', dy: -0.45 }, bg: ST(), blur: 2, actors: [QDESK, Q({ x: 1000, pose: 'present', expr: { base: 'calm', eyes: { style: 'cold', open: 0.75 } } })], over: DOOMV(73, 0.8) },
...
ep.panel(820, { cam: { x: 1250, y: 560, w: 1100 }, bg: ST(), actors: [QDESK, Q({ x: 1000, pose: 'stand', expr: 'calm', turn: 0.3 }), HQ()] },
...
ep.panel(950, { cam: { x: 1030, y: 238, w: 520 }, bg: ST(), blur: 2, actors: [QDESK, Q({ x: 1000, pose: 'lecture', expr: 'coldSmile', turn: 0.3 })] },
...
ep.panel(900, { cam: { x: 1500, y: 651, w: 420 }, bg: ST(), blur: 2, actors: [HQ({ expr: { base: 'worried', eyes: { lookX: -0.5 } }, turn: -0.3 })], over: onActor('harry', (a) => FX.emanata(a.head[0], a.head[1], a.hr * 1.5, { n: 5, w: 3 })) },
...
ep.multi(800, [
  ...
  { x: 408, y: 18, w: 368, h: 764, mood: 'candle', art: { cam: { head: 'quirrell', hw: 0.3, hx: 0.45, hy: 0.72 }, bg: ST(), actors: [QDESK, Q({ x: 1000, pose: 'present', expr: 'smile', turn: 0.3 })] } },
...
ep.bleed(1000, { cam: { x: 1090, y: 200, w: 560 }, bg: ST(), actors: [QDESK, Q({ x: 1000, pose: 'point', expr: { base: 'coldSmile', eyes: { open: 0.9 } }, turn: 0.4 })], behind: headBurst('quirrell', 30, 0.14, 3.5) },
...
ep.panel(870, { cam: { x: 1400, y: 820, w: 430 }, bg: ST(), actors: UP({ base: 'bigGrin', eyes: { sparkle: true } }, 'armsUp'), behind: headBurst('harry', 30, 0.15, 3.2) },
...
ep.panel(640, { cam: { x: 1010, y: 228, w: 400 }, bg: ST(), blur: 2, actors: [QDESK, Q({ x: 1000, pose: 'stand', expr: 'smile', turn: 0.3 })] },
...
ep.panel(1000, { cam: { x: 1250, y: 420, w: 760 }, bg: ST(), actors: [QDESK, Q({ x: 1000, pose: 'raiseHand', expr: 'stern', turn: 0.3 }), HQ({ x: 1480, pose: 'crossArms', expr: 'smug' })] },
...
const SLAM = onActor('quirrell', (a, e) => { const [x, y] = [a.handF[0], e.toPanel([0, 492])[1]], k = a.hr / 58;
...
ep.panel(1100, { cam: { x: 1020, y: 380, w: 620 }, bg: ST({ desk: false }), actors: [Q({ x: 1000, y: 722, pose: 'stand', lean: 10, armF: { sh: 18, el: -4, hand: 'fist' }, armB: { sh: 4, el: 6 }, expr: { base: 'yell', eyes: { style: 'normal', open: 0.95, lidTilt: -0.45 }, brows: { raise: -0.4, inner: -1.5, outer: 0.3 }, mouth: { type: 'shout', open: 1 } }, turn: 0.15 }), () => CS.defenceDesk(1000, 650)], over: SLAM, behind: onActor('quirrell', (a, e) => FX.burst(e.w, e.h, a.handF[0], e.toPanel([0, 492])[1], { n: 40, op: 0.22, inner: a.hr * 1.2 })) },
...
ep.panel(1000, { cam: { x: 1380, y: 560, w: 600 }, bg: ST(), actors: [Q({ x: 1400, y: 920, pose: 'walk', expr: 'stern', turn: 0.55 })] },
...
ep.panel(1000, { cam: { x: 1470, y: 400, w: 440 }, bg: ST(), blur: 2, actors: [Q({ x: 1400, y: 920, pose: 'wand', expr: { base: 'unimpressed', eyes: { lookX: 0.5 } }, turn: 0.4, armB: WAND() })] },
...
ep.panel(1000, { cam: { x: 1030, y: 290, w: 420 }, bg: ST(), actors: [QDESK, Q({ x: 1000, pose: 'gesture', expr: { base: 'stern', eyes: { squint: 0.4 }, mouth: { type: 'smile', w: 0.6 } }, turn: 0.3 })], over: onActor('quirrell', (a) => { const x = a.head[0], y = a.head[1], r = a.hr; return path(`M${x - r * 0.75},${y - r * 0.02} l${-r * 0.22},${-r * 0.08} M${x - r * 0.75},${y + r * 0.08} l${-r * 0.24},${r * 0.04} M${x + r * 0.75},${y - r * 0.02} l${r * 0.22},${-r * 0.08} M${x + r * 0.75},${y + r * 0.08} l${r * 0.24},${r * 0.04}`, { stroke: C.ink, 'stroke-width': 2.2, 'stroke-linecap': 'round' }); }) },
...
ep.panel(1100, { cam: { x: 1000, y: 330, w: 780 }, bg: ST(), actors: [Q({ pose: 'present', expr: 'calm', turn: 0.15 }), QDESK, BACKS(760, [560, 900, 1240, 1580], 1.2)] },
...
ep.panel(900, { cam: { x: 1010, y: 245, w: 440 }, bg: ST(), blur: 2, actors: [QDESK, Q({ x: 1000, pose: 'stand', expr: { base: 'calm', eyes: { open: 0.7 } }, turn: 0.1 })] },
...
ep.panel(900, { cam: { x: 1010, y: 215, w: 370 }, bg: ST(), blur: 2, actors: [QDESK, Q({ x: 1000, pose: 'stand', expr: { base: 'calm', eyes: { style: 'cold', open: 0.7 } }, turn: 0.1 })], over: DOOMV(77, 0.9) },
...
ep.bleed(900, { cam: { x: 1270, y: 540, w: 900 }, bg: ST(), actors: [QDESK, Q({ x: 1000, pose: 'point', expr: { base: 'calm', eyes: { style: 'cold', open: 0.7 } }, turn: 0.4 }), HQ({ x: 1600, expr: 'shock' })], over: DOOMV(78, 1.1) },
...
ep.panel(1000, { cam: { x: 1270, y: 420, w: 860 }, bg: ST(), actors: [QDESK, Q({ x: 1000, pose: 'point', expr: 'calm', turn: 0.4 }), HQ({ x: 1600, expr: 'worried' })] },
...
ep.panel(900, { cam: { x: 1190, y: 640, w: 820 }, bg: ST(), actors: [...CROWD({ harry: false, who: { draco: { x: 1050, expr: { base: 'calm', eyes: { lookX: 0.5 } }, turn: 0.3 }, crabbe: { x: 860, expr: LOOM, turn: 0.3 }, goyle: { x: 1240, expr: LOOM, turn: 0.3 }, zabini: { x: 700, expr: 'smug', turn: 0.3 }, padma: false, anthony: false, terry: false, michael: false } }).map((a) => ({ ...a, y: 900 })), HQ({ x: 1480, y: 1120, s: 1.4, turn: -0.5, pose: 'wand', expr: { base: 'cold', eyes: { lookX: -0.5 } }, armB: WAND() })] },
...
ep.panel(900, { cam: { x: 1450, y: 680, w: 480 }, bg: ST(), actors: [HQ({ pose: 'stand', turn: 0.1, expr: { base: 'determined', eyes: { lookX: -0.7, lookY: 0.3 } }, armF: { sh: -80, el: -5, hand: 'open' }, armB: { sh: 0, el: -85 } })], over: onActor('harry', (a) => { const r = a.hr, k = r / 64, f = [a.handF[0] * 0.62 + a.neck[0] * 0.38, a.handF[1] * 0.62 + a.neck[1] * 0.38 + r * 0.2];
...
ep.panel(700, { cam: { x: 1500, y: 650, w: 480 }, bg: ST(), actors: [HQ({ pose: 'stand', turn: 0.2, expr: 'wince', armF: { sh: 40, el: 60, hand: 'open' } })], over: onActor('harry', (a) => FX.emanata(a.handF[0], a.handF[1], a.hr * 0.5, { n: 6 })) },
...
ep.panel(1000, { cam: { on: ['quirrell'], fr: 'bust', dy: -0.1 }, bg: ST(), blur: 2, actors: [QDESK, Q({ x: 1000, pose: 'stand', expr: 'exasperated', turn: 0.3 })] },
...
ep.cutout(1030, { cam: { head: 'harry', hw: 0.25, hx: 0.5, hy: 0.39 }, actors: [HQ({ pose: 'armsUp', expr: 'laugh', turn: 0 })], behind: headBurst('harry', 30, 0.18, 3) },
...
ep.panel(1060, { cam: { x: 1200, y: 580, w: 900 }, bg: ST(), actors: [QDESK, Q({ x: 920, pose: 'slump', expr: { base: 'asleep', mouth: { type: 'o' } }, turn: -0.7 }), HQ({ x: 1520, pose: 'walk', turn: -0.6, expr: 'determined' })] },
