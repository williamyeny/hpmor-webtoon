...
ep.panel(820, { cam: { x: 1070, y: -140, w: 480 }, bg: TI, blur: 2, actors: MIN({ expr: DYELL, turn: -0.4, pose: 'fists' }), over: (e) => { const a = e.anchors.draco; return a ? FX.emanata(a.head[0], a.head[1] - a.hr * 0.4, a.hr * 1.7, { n: 7 }) : ''; } },
  [shout('Draco', '*I told you not to do this in front of Harry Potter!*', 400, 64, { anchor: 'tc', w: 400, fixed: true })], { mood: 'candle', alt: 'Draco rounds on his two looming minions.' });
ep.multi(620, [
...
ep.panel(560, { cam: { x: 790, y: -200, w: 560 }, bg: TI, actors: MIN({ expr: 'exasperated', turn: 0.4 }, { expr: 'laugh', lean: 4, pose: 'stand' }, { expr: { base: 'laugh', eyes: { style: 'happy' }, mouth: { type: 'line' } }, lean: 4, pose: 'hold', armF: { sh: 24, el: 128, hand: 'palm' }, armB: { sh: 8, el: 6 } }), over: (e) => { const a = e.anchors.crabbe, b = e.anchors.goyle; if (!a || !b) return ''; const k = b.hr / 95;
    return FX.emanata(a.head[0], a.head[1], a.hr * 1.6, { n: 6 }) + g({ transform: `translate(${b.mouth[0]},${b.mouth[1]}) scale(${k})` }, ellipse(0, 4, 34, 25, { fill: '#e8c2a0', stroke: C.ink, 'stroke-width': 3 }), path('M-20,-12 q4,16 0,30 M-4,-18 q4,20 0,38 M12,-15 q4,18 0,33', { fill: 'none', stroke: '#c99a78', 'stroke-width': 2.5 })); } },
...
ep.panel(1180, { cam: { x: 1270, y: 115, w: 720 }, bg: TI, actors: MIN({ expr: 'calm', pose: 'stand' }, {}, {}, { expr: 'neutral' }) },
  [say('Draco', 'I do want to continue our last conversation. And I accept your conditions.', 300, 50, { anchor: 'tc', w: 360, fixed: true }),
   say('Harry', 'Would you mind terribly waiting until Saturday? I\'m in a bit of a contest. See if I can read all my textbooks as fast as Hermione Granger did.', 490, 480, { anchor: 'tc', w: 400, fixed: true }),
   say('Draco', 'Granger. The mudblood who thinks she\'s Merlin? If you\'re trying to show *her* up, then all Slytherin wishes you the *very* best of luck, Potter.', 330, 1116, { anchor: 'bc', w: 400, fixed: true })], { mood: 'candle' });
...
ep.panel(560, { cam: { x: 820, y: CS.FLOOR - 250, w: 620 }, bg: TI, actors: [...SEATS({ lit: true, who: { dean: DOWN('focus'), g1: DOWN('worried'), draco: DOWN('focus'), ernie: DOWN('worried'), neville: DOWN('worried'), h1: DOWN('focus') } }), SCRIB([[400, 0], [650, 0], [950, 0], [300, 1], [560, 1], [820, 1]])] },
...
ep.bleed(1150, BEAST('dragon', { y: 0.74 }),
  [shout('Quirrell', 'The Hungarian Horntail is taller than a dozen men! It breathes fire so quickly and so accurately that it can melt a Snitch in mid-flight!', 400, 84, { anchor: 'tc', w: 460, fixed: true, noTail: true }),
   shout('Quirrell', 'One Killing Curse will bring it down!', 400, 1100, { anchor: 'bc', w: 400, fixed: true, noTail: true })], { alt: 'A dragon\'s silhouette, wings raised, breathing a jet of fire.' });
...
ep.bleed(1000, BEAST('troll', { y: 1.05, s: 1.25, x: 0.62, stump: true, regrow: true, gy: 0.8 }),
  [shout('Quirrell', 'It is always Transfiguring itself into its own body. Rip off its arm, and it will grow another within seconds! Fire and acid will confuse its regeneration, for an hour or two!', 400, 84, { anchor: 'tc', w: 480, fixed: true, noTail: true })], { alt: 'The troll\'s torn-off arm already growing back, glowing.' });
ep.panel(1000, { cam: { x: 1070, y: 360, w: 480 }, bg: ST({}), actors: [QDESK, QS({ pose: 'wandUp', expr: 'coldSmile', turn: 0.2, armB: WAND() })], over: (e) => { const a = e.anchors.quirrell; return a ? K.glow(a.handB[0] + a.hr * 1.5, a.handB[1] - a.hr * 0.35, a.hr * 1.6, '#6ad07a', 0.6) + circle(a.handB[0] + a.hr * 1.5, a.handB[1] - a.hr * 0.35, a.hr * 0.18, { fill: '#c8ffb0' }) : ''; } },
...
ep.panel(900, { cam: { on: ['quirrell'], fr: 'waist', dy: -0.7 }, bg: ST({}), blur: 2, actors: [QDESK, QS({ pose: 'holdUp', expr: { base: 'unimpressed', eyes: { lookY: -0.4, lookX: 0.4 } }, turn: 0.2, armB: { hand: 'hold', prop: bookHeld('#6a5a3a', { rot: 180, w: 70, h: 94 }) } })] },
...
ep.panel(560, { cam: { on: ['quirrell'], fr: 'eyes', dy: 0.1 }, bg: ST({}), blur: 3, actors: [QS({ turn: 0.05, expr: { base: 'calm', eyes: { style: 'cold', open: 0.62 }, brows: { raise: -0.3, inner: -0.5 } } })], over: DOOMV(72, 1.3) },
  [say('Quirrell', '*The Dark Wizard.*', 400, 520, { anchor: 'bc', w: 300, fixed: true, noTail: true })], { mood: 'candle', alt: 'His eyes, very close. Something about them is wrong.' });
ep.bleed(1250, { cam: { x: 1060, y: 350, w: 600 }, bg: ST({}), actors: [QDESK, QS({ pose: 'point', expr: { base: 'yell', eyes: { open: 0.85, style: 'normal', lidTilt: -0.35 }, brows: { raise: -0.2, inner: -1.3, outer: 0.4 }, mouth: { type: 'shout', open: 0.9 } }, turn: 0.1 })] },
  [shout('Quirrell', 'There is no defence without offence! There is no defence without fighting!', 400, 64, { anchor: 'tc', w: 420, fixed: true }),
   shout('Quirrell', 'This reality is deemed too harsh for eleven-year-olds by the fat, overpaid, Auror-guarded politicians who mandated your curriculum.', 400, 1180, { anchor: 'bc', w: 440, fixed: true })], { alt: 'Quirrell jabs a finger at the class.' });
ep.bleed(1100, { cam: { x: 1000, y: 330, w: 520 }, bg: ST({}), actors: [QDESK, QS({ pose: 'armsUp', expr: { base: 'yell', eyes: { open: 1, style: 'normal' }, brows: { raise: 0.7, inner: 0.2 }, mouth: { type: 'shout', open: 1.1 } }, turn: 0 })], over: (e) => { const a = e.anchors.quirrell; return a ? FX.burst(e.w, e.h, a.head[0], a.head[1], { n: 34, op: 0.2, inner: a.hr * 4 }) : ''; } },
  [shout('Quirrell', 'To the abyss with those fools! Welcome to your first year of *Battle Magic!*', 400, 1030, { anchor: 'bc', w: 460, fixed: true })], { alt: 'Quirrell, arms flung wide, triumphant.' });
...
ep.panel(820, { cam: { x: 1330, y: rowY(4) - 230, w: 620 }, bg: TI, actors: [{ def: terry, id: 'terry', x: 1130, y: rowY(4), s: 1.05, turn: 0.35, pose: 'sit', seat: 140, expr: 'shock' }, HB({ x: 1450, pose: 'stand', seat: undefined, expr: 'bigGrin', armF: { sh: 28, el: 108, hand: 'palm' }, armB: { sh: 34, el: 96, hand: 'palm' }, turn: -0.15 }), () => CS.tierFront(4, { screens: [1250, 1600], lit: true })], over: (e) => { const a = e.anchors.harry; return a ? CLAP(a) + FX.emanata(a.handF[0], a.handF[1], a.hr * 0.6, { n: 5 }) : ''; } },
...
ep.panel(820, { cam: { x: 820, y: CS.FLOOR - 320, w: 700 }, bg: TI, blur: 2, actors: SEATS({ lit: true, who: { hermione: { expr: 'gasp' }, padma: { expr: 'horror' }, dean: { expr: 'horror' }, g1: { expr: 'gasp' }, draco: { expr: 'shock' }, ernie: { expr: 'horror' }, neville: { expr: 'gasp' }, h1: { expr: 'shock' } } }) },
  [cap('*Gasps of horror.*', 44, 30, { w: 260, fixed: true }),
   say('Quirrell', '…an increased load which I will make up to you by not assigning any homework.', 400, 772, { anchor: 'bc', w: 460, fixed: true, noTail: true })], { mood: 'candle' });
ep.multi(720, [
...
ep.bleed(1100, { cam: { on: ['harry'], fr: 'waist', dy: -0.2, zoom: 0.85 }, bg: ST({ spheres: 6 }), actors: [PR({ expr: { base: 'bigGrin', eyes: { sparkle: true } }, pose: 'wandUp', turn: -0.1 })], over: (e) => { const a = e.anchors.harry; if (!a) return ''; const t = TIP(a); return FX.burst(e.w, e.h, e.w / 2, e.h * 0.45, { n: 26, col: '#ff9a7a', op: 0.2 }) + P2.bolt(t[0], t[1], e.w * 0.06, e.h * 0.22, { seed: 5 }) + P2.bolt(t[0], t[1], e.w * 0.3, e.h * 0.12, { seed: 9 }); } },
...
// Harry taps his own chest (a palm drawn over the near hand, which otherwise hides in the sleeve)
const TAP = (a) => { const k = a.hr / 95, [x, y] = a.handF; return g({ transform: `translate(${x},${y}) scale(${k}) rotate(-70)` }, ellipse(0, 0, 20, 30, { fill: '#f1d2b4', stroke: C.ink, 'stroke-width': 3 * k }), path('M-8,-24 q0,-14 6,-16 M4,-26 q0,-14 6,-14', { fill: 'none', stroke: C.ink, 'stroke-width': 2.4 })) + FX.emanata(x, y, a.hr * 0.5, { n: 3, a0: -160, a1: -110, w: 3 }); };
ep.panel(820, { cam: { x: 1650, y: 820, w: 420 }, bg: ST({}), actors: [{ def: harryRaven, id: 'harry', x: 1650, y: 1040, s: 1.1, turn: -0.3, pose: 'stand', expr: 'determined', armF: { sh: 22, el: 128, hand: 'palm' } }], over: (e) => { const a = e.anchors.harry; return a ? TAP(a) : ''; } },
...
ep.bleed(900, { cam: { x: 880, y: 720, w: 880 }, bg: ST({}), actors: [{ def: hermioneRaven, id: 'hermione', x: 600, y: 860, s: 1.0, turn: 0.3, pose: 'fallBack', expr: 'wince' }, { def: dracoSly, id: 'draco', x: 1180, y: 1110, s: 1.35, turn: -0.7, pose: 'wand', lean: -4, expr: { base: 'cold', eyes: { lookX: -0.6 } }, armB: WAND() }], over: (e) => { const d = e.anchors.draco, h = e.anchors.hermione; if (!d || !h) return ''; const t = TIP(d, 95); return FX.speedLines(e.w, e.h, { n: 26, col: '#6a6470' }) + P2.bolt(t[0], t[1], h.neck[0] + h.hr * 0.3, h.neck[1] + h.hr * 0.4, { seed: 11 }) + FX.emanata(h.neck[0], h.neck[1], h.hr * 1.4, { n: 7, a0: -200, a1: 20, col: '#ff5a3a' }); } },
...
ep.panel(1100, { cam: { x: 1240, y: 296, w: 900 }, bg: ST({}), actors: [QDESK, QS({ x: 1000, pose: 'present', expr: 'smile', turn: 0.3 }), HQ({ x: 1480, pose: 'raiseHand', expr: 'worried' })] },
  [say('Harry', 'Ah, Professor Quirrell…', 560, 40, { anchor: 'tc', w: 260, fixed: true }),
   say('Quirrell', 'You\'re thinking I\'ve come up with a wrong answer, aren\'t you, Mr Potter? You will learn to expect better of *me.*', 300, 170, { anchor: 'tc', w: 380, fixed: true })], { mood: 'candle', alt: 'Harry, well back from the dais, raises a hand. Quirrell smiles.' });
ep.bleed(1000, { cam: { x: 1090, y: 224, w: 560 }, bg: ST({}), actors: [QDESK, QS({ x: 1000, pose: 'point', expr: { base: 'coldSmile', eyes: { open: 0.9 } }, turn: 0.4 })], over: (e) => { const a = e.anchors.quirrell; return a ? FX.burst(e.w, e.h, a.head[0], a.head[1], { n: 30, op: 0.14, inner: a.hr * 3.5 }) : ''; } },
  [shout('Quirrell', 'Mr Potter, all things have their accustomed uses. Give me ten *unaccustomed* uses of objects in this room for combat!', 400, 90, { anchor: 'tc', w: 460, fixed: true })], { alt: 'Quirrell points at Harry: a challenge.' });
...
ep.bleed(1150, { cam: { x: 1400, y: 602, w: 460 }, bg: ST({}), actors: UP('grin', 'point', { turn: -0.2 }) },
  [shout('Harry', '*Ha!* The floor can be removed to make a spike pit! The ceiling can be collapsed on someone! The walls can be raw material for Transfiguration into any number of deadly things. Knives, say.', 400, 100, { anchor: 'tc', w: 470, fixed: true })], { alt: 'Harry, grinning, pointing at the floor, the ceiling, the walls.' });
...
ep.bleed(900, { cam: { x: 1400, y: 691, w: 520 }, bg: ST({}), actors: UP({ base: 'bigGrin', eyes: { sparkle: true } }, 'armsUp'), over: (e) => { const a = e.anchors.harry; return a ? FX.burst(e.w, e.h, a.head[0], a.head[1], { n: 30, op: 0.15, inner: a.hr * 3.2 }) : ''; } },
...
ep.panel(1000, { cam: { x: 1250, y: 420, w: 760 }, bg: ST({}), actors: [QDESK, QS({ x: 1000, pose: 'raiseHand', expr: 'stern', turn: 0.3 }), HQ({ x: 1480, pose: 'crossArms', expr: 'smug' })] },
  [say('Quirrell', 'Five points, and *enough.*', 250, 40, { anchor: 'tc', w: 280, fixed: true }),
   say('Harry', 'Hmph. You should have let me keep going until I\'d won the House Cup. I haven\'t even started on what\'s in my pockets.', 525, 330, { anchor: 'tc', w: 380, fixed: true })], { mood: 'candle' });
...
// WRONG!: Quirrell's fist comes down on the desk. The fist is drawn on the desk top, over the desk front.
const SLAM = (e) => { const a = e.anchors.quirrell; if (!a) return ''; const [x, y] = [a.handF[0], e.toPanel([0, 492])[1]], k = a.hr / 58;
  return FX.burst(e.w, e.h, x, y, { n: 40, op: 0.22, inner: a.hr * 1.2 }) + FX.emanata(x, y - 10 * k, a.hr * 1.3, { n: 7, w: 5 }) +
...
ep.bleed(1100, { cam: { x: 1020, y: 380, w: 620 }, bg: ST({ desk: false }), actors: [QS({ x: 1000, y: 690, pose: 'stand', lean: 10, armF: { sh: 22, el: 12, hand: 'fist' }, expr: { base: 'yell', eyes: { style: 'normal', open: 0.95, lidTilt: -0.45 }, brows: { raise: -0.4, inner: -1.5, outer: 0.3 }, mouth: { type: 'shout', open: 1 } }, turn: 0.15 }), () => CS.defenceDesk(1000, 650)], over: SLAM },
  [shout('Quirrell', '*WRONG!*', 400, 60, { anchor: 'tc', w: 280, size: 64, fixed: true }),
   sfx('BANG', 620, 600, { size: 76, rot: 8 }),
   shout('Quirrell', 'All of Mr Potter\'s ideas were worse than useless!', 360, 1070, { anchor: 'bc', w: 420, fixed: true })], { alt: 'Quirrell\'s fist comes down on the desk with an amplified bang that makes everyone jump.' });
...
ep.panel(820, { cam: { x: 1500, y: 610, w: 330 }, bg: ST({}), blur: 3, actors: [HQ({ expr: 'horror' })], over: (e) => FX.speedLines(e.w, e.h, { n: 30, radial: true }) },
...
ep.bleed(900, { cam: { on: ['harry'], fr: 'waist', dy: -0.3, zoom: 0.9 }, bg: ST({}), actors: [HQ({ pose: 'armsUp', expr: 'laugh', turn: 0 })], over: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.5, { n: 30, op: 0.18 }) },
  [shout('Harry', '*Just kidding! RAVENCLAW!*', 400, 64, { anchor: 'tc', w: 460, size: 48, fixed: true }),
   cap('There was silence for a brief moment. A sound of people thinking. Then the murmurs started, and rapidly rose to a roar.', 44, 760, { w: 620, fixed: true })], { alt: 'Harry, arms up, singing it out.' });
