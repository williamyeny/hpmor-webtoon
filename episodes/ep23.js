...
const HL = (o = {}) => HR({ x: 1160, y: 735, turn: 0.7, pose: 'stand', rot: -90, ...o });
...
ep.panel(1000, { cam: { head: 'harry', w: 480, hx: 340 / 752, hy: 500 / 964 }, bg: RR, blur: 2, actors: [HL({ expr: { base: 'smile', eyes: { soft: true, lookX: 0.6 } } })] },
...
ep.panel(860, { cam: { head: 'harry', w: 470, hx: 400 / 752, hy: 520 / 824 }, bg: RR, blur: 3, actors: [HR({ x: 1000, expr: 'awe' })] },
...
const HD = (o = {}) => HR({ x: 780, y: 900, turn: -0.5, pose: 'stand', expr: 'worried', ...o });
...
ep.panel(640, { cam: { head: 'harry', w: 430, hx: 400 / 752, hy: 400 / 604 }, bg: RR, blur: 3, actors: [HD({ x: 560, expr: 'suspicious' })] },
...
// Close-ups, staged in panel pixels: close(id, actors, bg, fgActor)(spec, w, px, py, ph, o) is a shot of world width w with
// the head of `id` at point (px, py) of a pw × ph panel (o.pw, default 752). o.fg = { px, py, ...spec } adds fgActor(spec)
// in the foreground with its head at that panel point; o.blur, o.over and o.behind pass through.
const head = (a) => place(a.def, a).anchors.head;
const close = (id, actors, bg, fgActor) => (spec, w, px, py, ph, o = {}) => {
  const list = actors(spec), pw = o.pw ?? 752, k = w / pw;
  const fg = [];
  if (o.fg) { // the foreground figure's feet go wherever puts its head on the chosen point
    const [mx, my] = head(list[0]), f = fgActor(o.fg), [fx, fy] = head({ ...f, x: 0, y: 0 });
    fg.push({ ...f, x: mx + (o.fg.px - px) * k - fx, y: my + (o.fg.py - py) * k - fy });
  }
  return { cam: { head: id, w, hx: px / pw, hy: py / ph }, bg, blur: o.blur ?? 2, actors: [...list, ...fg], over: o.over, behind: o.behind };
};
// Quirrell close (from Harry's side), Harry's head in the foreground
const QC = close('quirrell', (q) => [Q(q), QD], ST, (f) => H({ turn: -0.7, s: 1.4, ...f }));
// Harry close, standing on the stage (from Quirrell's side: the tiers behind him), Quirrell's shoulder in the foreground
const HC = close('harry', (h) => [HR2(h)], REV, (f) => Q({ turn: 0.7, s: 1.5, ...f }));
...
const SITG = { pose: 'sitFloor', armB: { sh: 55, el: -70, hand: 'palm' } };
...
ep.cutout(900, { cam: { head: 'harry', w: 590, hx: 400 / 752, hy: 440 / 864 }, ground: false, mid: LIESHADOW, actors: [HLie()] },
...
ep.multi(1170, [
  ...
  P(436, 716, { cam: { head: 'harry', w: 560, hx: 520 / 752, hy: 470 / 716 }, bg: FLOORV, actors: [HLie({ expr: { base: 'think', eyes: { lookX: -0.3 } } })] }),
...
ep.panel(980, { cam: { head: 'harry', w: 440, hx: 400 / 752, hy: 780 / 944 }, bg: ST, blur: 2, actors: [HSit({ ...SITG, expr: 'neutral' })], over: (e) => FX.frost(e.w, e.h, 0.2, 173) },
...
ep.panel(960, HC({ ...SITG, y: 1460, expr: 'think' }, 560, 470, 760, 924, { fg: { px: 30, py: 640, s: 2.3, expr: { base: 'focus', eyes: { lookY: 0.4 } } } }),
...
ep.multi(1300, [
  ...
  P(856, 426, { cam: { head: 'harry', w: 480, hx: 520 / 752, hy: 250 / 426 }, bg: ST, blur: 2, actors: [HSit({ expr: 'determined' })] }),
...
ep.cutout(1300, { cam: { head: 'harry', w: 460, hx: 400 / 752, hy: 405 / 1264 }, actors: [H({ pose: 'armsUp', expr: { base: 'bigGrin', eyes: { sparkle: true } } })], behind: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.42, { n: 26, col: '#e8b850', op: 0.3, inner: 200 }) },
...
ep.bleed(1000, { cam: { head: 'harry', w: 480, hx: 0.5, hy: 0.47 }, bg: SPACE({ k: 0.8 }), actors: [HS({ expr: { base: 'teary', eyes: { sparkle: true, lookY: -0.4 } } }), QS()] },
...
ep.bleed(1200, { cam: { head: 'harry', w: 470, hx: 0.5, hy: 560 / 1200 }, bg: SPACE({ k: 0.6 }), actors: [HS({ expr: 'cry', pose: 'facepalm' })] },
...
ep.bleed(1160, { cam: { head: 'quirrell', w: 400, hx: 0.5, hy: 560 / 1160 }, bg: SPACE({ k: 0.9, disc: false }), actors: [QS({ turn: 0.1, expr: { base: 'sad', eyes: { lookY: -0.5, soft: true } } })] },
...
ep.bleed(860, { cam: { head: 'harry', w: 440, hx: 0.5, hy: 520 / 860 }, bg: SPACE({ k: 0.8 }), actors: [HS({ expr: { base: 'teary', eyes: { soft: true, lookY: -0.3 } } })] },
...
const DC = close('dumbledore', (d) => [DB(d)], ST2);
const HDC = close('harry', (h) => [Hd(h)], ST2);
...
ep.panel(1200, { cam: { x: 1960, y: 400, w: 910 }, bg: () => ST2() + smoke(), actors: [DB({ x: DX, y: 640, turn: -0.2, pose: 'point', expr: 'angry' }), () => leaf(1530, 800, -38, 1.2) + leaf(2420, 700, 52, 1.1) + splinters()], behind: (e) => FX.burst(e.w, e.h, e.w * 0.55, e.h * 0.4, { n: 34, op: 0.22 }) },
...
ep.bleed(1120, { cam: { head: 'quirrell', w: 560, hx: 0.5, hy: 0.5 }, bg: () => CS.defenceStage({ desk: false }), actors: [Q({ pose: SLAM, expr: 'stern', turn: 0.3 }), () => CS.defenceDesk(1000, 660)], behind: (e) => FX.burst(e.w, e.h, 519, 880, { n: 26, op: 0.2 }) },
...
ep.panel(900, { cam: { head: 'harry', w: 400, hx: 0.5, hy: 462 / 864 }, bg: ST2, blur: 2, actors: [Hd({ expr: 'yell' })], over: (e) => FX.frost(e.w, e.h, 0.4, 185), behind: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.53, { n: 20, op: 0.2 }) },
...
ep.multi(1500, [
  ...
  P(596, 420, HDC({ expr: 'hopeful', pose: 'raiseHand', armB: { sh: 140, el: 20, hand: 'palm' } }, 600, 590, 290, 420)),
...
ep.bleed(1200, { cam: { head: 'dumbledore', w: 440, hx: 0.5, hy: 520 / 1200 }, bg: ST2, blur: 2, actors: [DB({ turn: -0.3, expr: 'sad' })] },
...
ep.multi(700, [0, 1, 2].map((k) => ({ x: M + k * 254, y: 18, w: 244, h: 664, mood: 'candle', art: { cam: { head: 'harry', w: [200, 140, 90][k], hx: 0.5, hy: [300, 330, 360][k] / 664 }, bg: REV, blur: 3, actors: [HR2({ expr: { base: 'awe', eyes: { open: 1 + k * 0.12, sparkle: true } } })] } })),
