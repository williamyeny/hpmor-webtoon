...
// head position of an actor spec (world units), honouring a rotation about the feet
const HEAD = (a) => { const [x, y] = place(a.def, a).anchors.head; if (!a.rot) return [x, y]; const t = a.rot * Math.PI / 180, dx = x - a.x, dy = y - a.y; return [a.x + dx * Math.cos(t) - dy * Math.sin(t), a.y + dx * Math.sin(t) + dy * Math.cos(t)]; };
...
const QC = (q, w, px, py, ph, o = {}) => { const cam = AT(Q(q), w, px, py, ph, o.pw); const fg = o.fg ? [PUT(H({ turn: -0.7, s: 1.4, ...o.fg }), WPT(cam, o.fg.px, o.fg.py, ph, o.pw))] : []; return { cam, bg: ST, blur: o.blur ?? 2, actors: [Q(q), QD, ...fg], ...(o.over ? { over: o.over } : {}) }; };
...
const HC = (h, w, px, py, ph, o = {}) => { const cam = AT(HR2(h), w, px, py, ph, o.pw); const fg = o.fg ? [PUT(Q({ turn: 0.7, s: 1.5, ...o.fg }), WPT(cam, o.fg.px, o.fg.py, ph, o.pw))] : []; return { cam, bg: REV, blur: o.blur ?? 2, actors: [HR2(h), ...fg], ...(o.over ? { over: o.over } : {}) }; };
...
ep.bleed(900, { cam: { x: 1005, y: 360, w: 1000 }, bg: ST, actors: TWO({ pose: 'relaxed' }, { pose: 'walk' }) },
  [cap('They walked across the now-deserted stage to Professor Quirrell\'s desk. Professor Quirrell leaned on it. And Harry, as before, stopped short of the dais.', 48, 30, { w: 600, fixed: true }),
   say('Quirrell', 'So. What was it you wanted to talk to me about, Mr Potter?', 608, 290, { w: 260, fixed: true })], { alt: 'The empty white-marble stage. A gap between them.' });
...
ep.panel(880, HC({ expr: 'rant', pose: 'armsUp' }, 560, 400, 560, 844, { over: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.64, { n: 30, op: 0.14 }) }),
...
ep.panel(1060, { cam: { x: 1190, y: 330, w: 860 }, bg: ST, actors: TWO({ expr: 'smile', pose: 'relaxed' }, { x: 1360, expr: 'yell', pose: 'fists' }) },
  [say('Quirrell', 'And that was a *trifle* too much indignation.', 200, 80, { anchor: 'tc', w: 240, fixed: true }),
   shout('Harry', '*What on Earth do I have to do to convince you?*', 530, 350, { anchor: 'tc', w: 280, fixed: true })], { mood: 'candle' });
ep.multi(1300, [
...
ep.panel(860, { cam: { x: 1380, y: 700, w: 720 }, bg: FLOORV, actors: [HLie()] },
  [cap('Harry sat down on the hard marble floor, and then lay back on it, staring at the distant arches of the ceiling. It was as close as he could come to collapsing in despair without hurting himself.', 48, 650, { w: 600, fixed: true })], { alt: 'Seen from above: Harry, flat on his back on the stage, arms flung out.' });
ep.multi(1170, [
...
ep.multi(1110, [
  ...
  P(696, 396, HC({ expr: 'yell', pose: 'fists' }, 460, 560, 250, 396, { over: (e) => FX.burst(e.w, e.h, e.w * 0.75, e.h * 0.6, { n: 24, op: 0.14 }) })),
...
ep.panel(1000, QC({ expr: 'calm', turn: 0.3 }, 520, 280, 760, 964, { fg: { px: 660, py: 900, expr: 'exasperated' } }),
  [say('Quirrell', 'Please do not mistake me for one of the fools around you. The Hat playing its first prank in eight hundred years, while it happened to be on *your* head? By far the most probable explanation is that Dumbledore was not happy with the Hat\'s choice for the Boy-Who-Lived.', 400, 90, { anchor: 'tc', w: 500, fixed: true })], { mood: 'candle' });
...
ep.multi(1500, [
  ...
  P(1094, 388, HC({ expr: 'embarrassed' }, 460, 540, 230, 388)),
   ...
   say('Quirrell', 'Mr Potter… I promise you will not be graded on the answer. I only want your honest reply. *Why not?*', 280, 600, { anchor: 'tc', w: 360, fixed: true }),
...
ep.multi(1620, [
  ...
  P(936, 666, HC({ expr: 'unimpressed' }, 480, 400, 520, 666)),
   ...
   say('Harry', 'Oh, now *that* was around as subtle as *Atlas Shrugged.* Blah blah blah, appeal to my sense of superiority, other people are trying to keep me down, blah blah blah.', 400, 990, { anchor: 'tc', w: 480, fixed: true })]);
...
ep.multi(1440, [
  ...
  P(456, 966, HC({ expr: 'determined', pose: 'gesture' }, 480, 400, 810, 966)),
   ...
   shout('Harry', 'Other people have done *huge* amounts for me! My parents took me in when my parents died, because they were *good people!* And to become a Dark Lord is to betray that!', 400, 590, { anchor: 'tc', w: 430, fixed: true })]);
ep.panel(880, QC({ expr: { base: 'sad', eyes: { lookX: -0.4 } } }, 380, 420, 470, 844, { blur: 3, over: (e) => FX.doom(e.w, e.h, 175) }),
  [cap('Professor Quirrell was silent for a time.', 48, 34, { w: 560, fixed: true }),
   whisper('Quirrell', 'I confess, when I was your age, that thought could not ever have come to me.', 400, 840, { anchor: 'bc', w: 460, fixed: true })], { mood: 'candle' });
ep.multi(1560, [
  P(18, 330, HC({ expr: 'sad' }, 440, 560, 200, 330)),
  P(366, 700, QC({ expr: 'calm', turn: 0.3 }, 440, 580, 520, 700)),
  P(1084, 458, HC({ expr: 'determined', pose: 'fists' }, 440, 620, 300, 458)),
   ...
   say('Harry', 'No. Just no. It is their *impulse to kindness* that sheltered me. That impulse is not only in my parents. And that impulse is what would be betrayed.', 290, 1125, { anchor: 'tc', w: 400, fixed: true })]);
// the ambition
ep.multi(1340, [
  P(18, 400, QC({ expr: 'calm' }, 420, 590, 230, 400)),
  P(436, 886, HC({ expr: 'smile', pose: 'lecture' }, 480, 400, 720, 886)),
], [say('Quirrell', 'In any case, Mr Potter, you have not answered my original question. What *is* your ambition?', 280, 60, { anchor: 'tc', w: 380, fixed: true }),
...
ep.bleed(1000, { cam: BAT(H({ pose: 'armsUp' }), 470, 400, 500, 1000), bg: ST, blur: 2, actors: [H({ pose: 'armsUp', expr: { base: 'bigGrin', eyes: { sparkle: true } } })], over: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.56, { n: 26, op: 0.18 }) },
  [say('Harry', 'Step one is to become a scientist.', 400, 70, { anchor: 'tc', w: 360, fixed: true }),
   shout('Harry', 'I shall achieve my objectives through the power… of *Science!*', 400, 920, { anchor: 'bc', w: 400, fixed: true })], { alt: 'Harry, arms flung up, triumphant.' });
...
ep.panel(1250, QC({ expr: 'angry', pose: 'fists' }, 460, 400, 1062, 1214, { over: (e) => FX.doom(e.w, e.h, 177) }),
  [shout('Quirrell', 'You could be the best of all my students! The greatest fighting wizard to come out of Hogwarts in five decades!', 360, 110, { anchor: 'tc', w: 340, fixed: true }),
   shout('Quirrell', 'I cannot picture you wasting your days in a white lab coat, doing pointless things to *rats!*', 450, 600, { anchor: 'tc', w: 320, fixed: true })], { mood: 'candle' });
...
ep.multi(1780, [
  ...
  P(366, 934, QC({ expr: 'rant', pose: 'point' }, 460, 424, 770, 934, { pw: 800, over: (e) => FX.doom(e.w, e.h, 179) + FX.burst(e.w, e.h, e.w / 2, e.h * 0.82, { n: 20, op: 0.12 }) }), { x: 0, w: 800, border: 'bleed', fadeTop: false, fadeBottom: false }),
  P(1318, 444, QC({ expr: 'angry', pose: 'fists', turn: 0.4 }, 620, 560, 280, 444, { over: (e) => FX.doom(e.w, e.h, 176) })),
   ...
   shout('Quirrell', 'Those fool Muggles will kill us all some day! They will end it! End *all* of it!', 330, 470, { anchor: 'tc', w: 320, fixed: true }),
   shout('Quirrell', 'Nuclear weapons! Even He-Who-Must-Not-Be-Named never used those!', 389, 860, { anchor: 'tc', w: 440, size: 30, fixed: true }),
...
ep.multi(1880, [
  P(18, 1240, QC({ expr: 'rant', pose: 'fists', turn: 0.1 }, 480, 400, 1100, 1240, { over: (e) => FX.doom(e.w, e.h, 180) + FX.burst(e.w, e.h, e.w / 2, e.h * 0.85, { n: 20, op: 0.12 }) })),
  P(1276, 586, QC({ expr: 'yell', pose: 'fists', turn: 0.2 }, 640, 300, 450, 586, { fg: { px: 660, py: 560, expr: 'shock' }, over: (e) => FX.doom(e.w, e.h, 181) })),
], [shout('Quirrell', 'There are gates you do not open! There are seals you do not breach!', 340, 100, { anchor: 'tc', w: 320, fixed: true }),
   ...
   shout('Quirrell', 'And those idiot Muggles couldn\'t figure it out!', 400, 1376, { anchor: 'tc', w: 300, fixed: true })], { alt: 'Quirrell, almost shouting, standing straight instead of leaning.' });
ep.multi(1830, [
  P(18, 820, HC({ expr: 'think', pose: 'chin' }, 480, 400, 640, 820)),
  P(856, 600, QC({ expr: 'calm', pose: 'relaxed', turn: 0.3 }, 480, 580, 400, 600)),
  P(1474, 338, HC({ expr: { base: 'smile', eyes: { soft: true } } }, 460, 590, 200, 338)),
   ...
   say('Harry', 'I\'m a big fan of the space programme too. At least we have that much in common.', 290, 1520, { anchor: 'tc', w: 400, fixed: true })]);
ep.panel(1000, QC({ expr: { base: 'calm', eyes: { soft: true } }, pose: 'wand' }, 440, 400, 370, 964),
  [cap('Something flickered in the professor\'s eyes.', 48, 34, { w: 560, fixed: true }),
   say('Quirrell', 'I will have your word, your promise, and your oath never to speak of what follows. I will now cast a rare and powerful spell. Not on you: on the classroom around us. Stand still. Look only. And try not to fall over.', 400, 940, { anchor: 'bc', w: 480, fixed: true })], { mood: 'candle' });
...
ep.bleed(1200, { cam: { x: 1960, y: 400, w: 1100 }, bg: () => CS.defenceStage({}) + doorway() + smoke(), actors: [DB({ x: DX, y: 640, turn: -0.2, pose: 'point', expr: 'angry' }), () => leaf(1560, 820, -38, 1.25) + leaf(2440, 760, 52, 1.15) + splinters()], over: (e) => FX.burst(e.w, e.h, e.w * 0.55, e.h * 0.4, { n: 34, op: 0.22 }) },
...
ep.bleed(1120, { cam: AT(Q({ pose: SLAM }), 560, 400, 560, 1120, 800), bg: () => CS.defenceStage({ desk: false }), actors: [Q({ pose: SLAM, expr: 'stern', turn: 0.3 }), () => CS.defenceDesk(1000, 660)], over: (e) => FX.burst(e.w, e.h, 519, 880, { n: 26, op: 0.2 }) },
...
ep.multi(1840, [
  ...
  P(1414, 408, HDC({ expr: 'calm' }, 460, 580, 230, 408)),
   ...
   shout('Dumbledore', 'I heard students saying that this man had you abused by older Slytherins! That he forbade you to defend yourself!', 400, 720, { anchor: 'tc', w: 380, fixed: true }),
   say('Harry', 'He knew exactly what was wrong with me, and he showed me how to fix it.', 290, 1460, { anchor: 'tc', w: 360, fixed: true })], { alt: 'Dumbledore, towering; Harry between the two professors.' });
ep.multi(1450, [
  ...
  P(994, 438, HDC({ expr: 'deadpan' }, 460, 580, 250, 438)),
   ...
   say('Harry', 'Headmaster, what\'s wrong with me isn\'t that I keep quiet about abusive professors.', 290, 1040, { anchor: 'tc', w: 380, fixed: true })]);
ep.multi(1560, [
...
ep.bleed(900, { cam: BAT(Hd(), 420, 400, 480, 900), bg: ST2, blur: 2, actors: [Hd({ expr: 'yell' })], over: (e) => FX.frost(e.w, e.h, 0.4, 185) + FX.burst(e.w, e.h, e.w / 2, e.h * 0.53, { n: 20, op: 0.2 }) },
...
ep.multi(1900, [
  ...
  P(1174, 708, QC({ expr: 'cold', pose: 'relaxed', turn: 0.4 }, 480, 620, 600, 708)),
   ...
   say('Quirrell', 'I did not think so. Then *you* will pay for a licensed Occlumency instructor. One recommended by his account manager at Gringotts. Neutral. Bound by an Unbreakable Vow to reveal nothing, and Obliviated after each session.', 300, 1220, { anchor: 'tc', w: 400, fixed: true })]);
ep.multi(1500, [
  P(18, 560, DC({ expr: 'suspicious', pose: 'crossArms' }, 460, 590, 360, 560)),
  P(596, 420, HDC({ expr: 'hopeful', pose: 'raiseHand' }, 520, 580, 250, 420)),
...
ep.bleed(820, { cam: { x: 1560, y: 420, w: 1400 }, bg: ST2, actors: [Q({ expr: 'calm' }), QD, H({ expr: 'think', turn: 0.4 }), DB({ x: 1950, y: 620, s: 0.95, turn: 0.6, pose: 'walk', expr: 'sad' })] },
...
ep.multi(1500, [
  P(18, 420, HC({ expr: { base: 'smile', eyes: { soft: true } } }, 460, 580, 250, 420)),
...
ep.multi(1800, [
  ...
  P(1134, 648, QC({ expr: { base: 'bigGrin', eyes: { sparkle: true } }, pose: 'gesture' }, 400, 400, 490, 648)),
   ...
   say('Quirrell', 'I subscribe to a Muggle bulletin on space travel. When I discovered that Pioneer 11 would be leaving the Solar System forever…', 530, 660, { anchor: 'tc', w: 360, fixed: true }),
   say('Quirrell', 'I sneaked into NASA, I did. And I cast a lovely little spell on that lovely golden plaque, which will make it last a lot longer than it otherwise would.', 400, 1170, { anchor: 'tc', w: 480, fixed: true })], { alt: 'Quirrell, grinning the widest grin Harry has yet seen from him.' });
...
ep.multi(1500, [
  ...
  P(936, 546, QC({ expr: 'smug', pose: 'relaxed', turn: 0.4 }, 620, 280, 330, 546, { fg: { px: 660, py: 500, expr: { base: 'awe', mouth: { type: 'o' } } } })),
   ...
   say('Quirrell', '"You win" seems appropriate.', 440, 970, { anchor: 'tc', w: 300, fixed: true }),
   say('Harry', 'You win.', 520, 1200, { w: 180, fixed: true })], { alt: 'Professor Quirrell, who now seems to be standing about fifty feet taller.' });
...
ep.multi(1560, [
  ...
  P(936, 606, QC({ expr: { base: 'calm', eyes: { style: 'cold', open: 0.8 } } }, 440, 600, 460, 606)),
], [say('Harry', 'You didn\'t add any extra information to the plaque, did you? A holographic message? Or… a portrait seems to store a whole human mind… or you found a volunteer who was dying, and made sure their *ghost* ended up in the plaque…', 400, 90, { anchor: 'tc', w: 480, fixed: true }),
   say('Quirrell', 'Mr Potter. A spell requiring a human death would certainly be classified by the Ministry as Dark Arts, regardless of circumstances. Students should not be heard talking about such things.', 310, 985, { anchor: 'tc', w: 430, fixed: true })]);
ep.multi(1260, [
  P(18, 860, QC({ expr: { base: 'calm', eyes: { style: 'cold', open: 0.8 } } }, 380, 400, 560, 860, { blur: 3, over: (e) => FX.doom(e.w, e.h, 189) })),
...
], [say('Quirrell', 'Please be discreet about the whole matter. I prefer to go through my life without attracting public notice.', 230, 60, { anchor: 'tc', w: 320, fixed: true }),
