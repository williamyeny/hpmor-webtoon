...
import { atAnchor } from '../engine/core/scene.js';
...
const OF = () => CS.dumbledoreOffice({ bird: 'phoenix', rack: true });
...
const ROOM = (o = {}) => [THRONE, DB(o.d), DESK, CHAIR(520), MG(o.m), CHAIR(1520), SN(o.s)];

ep.bleed(1000, { cam: { x: 1010, y: 655, w: 1240 }, bg: OF, actors: [...ROOM(), () => g({ transform: 'translate(1010,1340) scale(1.3)' }, CS.stoolFront(0, 0))] },
...
ep.multi(1180, [
  { x: M, y: 18, w: 752, h: 470, mood: 'candle', art: { cam: { head: 'mcgonagall', w: 660, hy: 0.5, dx: 80, dy: 40 }, bg: OF, blur: 2, actors: [...ROOM({ m: { expr: 'pained' } }), STOOL] } },
  // the memory in her mind: no hard edges, it fades into the page
  { x: M, y: 506, w: 752, h: 656, mood: 'sepia', frame: 'dissolve', feather: 60, art: { cam: { head: 'harry', w: 640, hy: 0.5, dx: -1, dy: 100 }, bg: OF, blur: 4, actors: [{ def: harryRaven, id: 'harry', x: 1000, y: 1160, s: 1.1, turn: 0.15, pose: 'fists', expr: 'rant' }] } },
...
ep.cutout(1000, { cam: { x: 700, y: 400, w: 800 }, actors: [() => K.glow(700, 330, 560, '#ffb24a', 0.55) + FX.burst(900, 900, 700, 330, { n: 30, inner: 230, col: '#ffcf75', op: 0.2 }), () => g({ transform: 'translate(700,372) scale(1.3)' }, P2.phoenix(1, { fly: true }))], over: (e) => EMBERS(e, 22) },
...
ep.multi(1000, [
  { x: M, y: 18, w: 368, h: 440, mood: 'candle', art: { cam: { head: 'harry', w: 330, hy: 0.5, dy: -30 }, bg: OF, blur: 3, actors: [HX()] } },
  { x: 408, y: 18, w: 368, h: 440, mood: 'candle', art: { cam: { head: 'dumbledore', w: 230, hy: 0.5, dy: -15 }, bg: OF, blur: 3, actors: [THRONE, DB({ expr: { base: 'smile', eyeR: { style: 'happy' }, brows: { raise: 0.6 } } }), DESK] } },
  { x: M, y: 476, w: 240, h: 506, mood: 'candle', art: { cam: { head: 'harry', w: 210, hy: 0.5, dy: -20 }, bg: OF, blur: 3, actors: [HX({ expr: 'horror' })] } },
  { x: 280, y: 476, w: 240, h: 506, mood: 'candle', art: { cam: { head: 'harry', w: 210, hy: 0.5, dy: -20 }, bg: OF, blur: 3, actors: [HX({ expr: 'angry' })] } },
  { x: 536, y: 476, w: 240, h: 506, mood: 'cold', art: { cam: { head: 'harry', w: 210, hy: 0.5, dy: -20 }, bg: OF, blur: 3, actors: [HX({ expr: 'cold' })], over: (e) => FX.frost(e.w, e.h, 0.5, 123) } },
...
ep.panel(620, { cam: { head: 'mcgonagall', w: 520, hy: 0.5, dy: -40 }, bg: OF, blur: 3, actors: [...ROOM({ m: { expr: 'worried' } }), STOOL] },
...
const SOLO = (o = {}) => [() => CS.stoolFront(1900, 1120), { def: harryRaven, id: 'harry', x: 1900, y: 1100, s: 1.1, turn: -0.25, pose: 'sit', seat: 150, expr: 'cold', ...o }];
...
ep.panel(1100, { cam: { head: 'harry', w: 460, hy: 0.5, dy: 50 }, bg: OF, blur: 2, actors: SOLO({ expr: 'coldSmile' }), over: (e) => FX.frost(e.w, e.h, 0.35, 125) },
...
ep.multi(620, [
  { x: M, y: 150, w: 368, h: 452, mood: 'candle', art: { cam: { head: 'mcgonagall', w: 330, hy: 0.5, dy: 30 }, bg: OF, blur: 3, actors: [CHAIR(520), MG({ expr: 'gasp' })] } },
  { x: 408, y: 150, w: 368, h: 452, mood: 'candle', art: { cam: { head: 'snape', w: 330, hy: 0.5, dy: 30 }, bg: OF, blur: 3, actors: [CHAIR(1520), SN({ expr: 'laugh' })] } },
...
ep.panel(1260, { cam: { head: 'dumbledore', w: 520, hy: 0.5, dy: -20 }, bg: OF, blur: 2, actors: D1({ expr: 'cold' }) },
...
ep.panel(900, { cam: { head: 'harry', w: 480, hy: 0.5, dy: 10 }, bg: OF, blur: 2, actors: SOLO({ pose: 'stand', expr: 'cold' }), over: (e) => FX.frost(e.w, e.h, 0.3, 127) },
...
ep.panel(1100, { cam: { head: 'harry', w: 420, hy: 0.5, dy: -100 }, bg: OF, blur: 3, actors: SOLO({ pose: 'fists', expr: { base: 'angry', mouth: { type: 'scream', open: 0.6 } } }), over: (e) => FX.frost(e.w, e.h, 0.45, 128) },
...
ep.panel(620, { cam: { head: 'mcgonagall', w: 440, hy: 0.5, dy: -45 }, bg: OF, blur: 3, actors: [CHAIR(520), MG({ expr: 'teary' })] },
...
ep.multi(1240, [
  { x: M, y: 18, w: 752, h: 542, mood: 'candle', art: { cam: { head: 'dumbledore', w: 400, hy: 0.5, dy: -30 }, bg: OF, blur: 3, actors: D1({ expr: 'angry' }), over: (e) => K.glow(e.w / 2, e.h * 0.6, 420, '#9bc4e8', 0.22) } },
  { x: M, y: 578, w: 752, h: 644, mood: 'cold', art: { cam: { head: 'harry', w: 520, hy: 0.5, dy: -80 }, bg: OF, blur: 3, actors: SOLO({ expr: 'cold' }), over: (e) => FX.frost(e.w, e.h, 0.55, 130) } },
...
ep.panel(800, { cam: { x: 990, y: 590, w: 1300, roll: -5 }, bg: OF, actors: ROOM({ m: { expr: 'horror', lean: -9 }, s: { expr: 'shock' } }) },
...
ep.multi(1320, [
  { x: M, y: 18, w: 752, h: 740, mood: 'candle', art: { cam: { head: 'dumbledore', w: 440, hy: 0.5, dy: -150 }, bg: OF, blur: 2, actors: D1({ expr: 'cold' }) } },
...
ep.panel(1200, { cam: { head: 'harry', w: 440, hy: 0.5, dy: 125 }, bg: OF, blur: 2, actors: SOLO({ pose: 'fists', expr: { base: 'angry', mouth: { type: 'scream', open: 0.7 } } }) },
  [say('Dumbledore', 'Harry, did I not tell you that Severus Snape has my fullest confidence?', 400, 40, { anchor: 'tc', w: 460, fixed: true, noTail: true }),
   shout('Harry', 'This man\'s bullying makes you *vulnerable!* I am not the only one who could start a newspaper campaign against you! This is insane! Why are you doing this?', 400, 1070, { anchor: 'bc', w: 420, size: 34, pad: 56, fixed: true })], { mood: 'candle' });
ep.multi(1180, [
  { x: M, y: 18, w: 752, h: 460, mood: 'candle', art: { cam: { head: 'dumbledore', w: 400, hy: 0.5, dy: -10 }, bg: OF, blur: 2, actors: D1({ expr: 'sad' }) } },
  { x: M, y: 496, w: 752, h: 666, mood: 'cold', art: { cam: { head: 'harry', w: 500, hy: 0.5, dy: -120 }, bg: OF, blur: 3, actors: SOLO({ expr: 'cold' }), over: (e) => FX.frost(e.w, e.h, 0.4, 132) } },
...
ep.panel(700, { cam: { head: 'dumbledore', w: 380, hy: 0.5, dy: -55 }, bg: OF, blur: 3, actors: D1({ expr: { base: 'warm', brows: { raise: 0.7 } } }) },
...
ep.panel(760, { cam: { head: 'harry', w: 500, hy: 0.5, dy: -20 }, bg: OF, blur: 2, actors: SOLO({ pose: 'stand', expr: 'cold' }), over: (e) => FX.frost(e.w, e.h, 0.4, 133) },
...
ep.bleed(900, { cam: { head: 'harry', w: 250, hy: 0.5, dy: 40 }, bg: OF, blur: 4, actors: SOLO({ pose: 'stand', expr: { base: 'cold', eyes: { open: 0.5 } } }), over: (e) => FX.frost(e.w, e.h, 0.8, 134) },
...
ep.panel(1100, { cam: { head: 'dumbledore', w: 460, hy: 0.5, dy: -120 }, bg: OF, blur: 2, actors: D1({ expr: 'cold' }) },
...
ep.panel(1000, { cam: { head: 'harry', w: 460, hy: 0.5, dy: 30 }, bg: OF, blur: 2, actors: SOLO({ expr: 'coldSmile' }), over: (e) => FX.frost(e.w, e.h, 0.4, 133) },
...
ep.panel(560, { cam: { head: 'harry', w: 250, hy: 0.5, dy: 20 }, bg: OF, blur: 3, actors: SOLO({ expr: { base: 'shock', mouth: { type: 'line', curve: 0, w: 0.6 } } }) },
...
ep.multi(1200, [
  { x: M, y: 18, w: 752, h: 440, mood: 'candle', art: { cam: { head: 'dumbledore', w: 380, hy: 0.5, dy: -20 }, bg: OF, blur: 2, actors: D1({ expr: 'stern' }) } },
  { x: M, y: 476, w: 752, h: 706, mood: 'cold', art: { cam: { head: 'harry', w: 280, hy: 0.5, dy: -40 }, bg: OF, blur: 4, actors: SOLO({ expr: { base: 'cold', eyes: { lookY: 0.2 } } }), over: (e) => FX.frost(e.w, e.h, 0.5, 135) } },
...
const SNU = (o = {}) => SN({ pose: 'stand', ...o });
// Snape explodes: the panel bursts with him; Harry's cold reply sits outside it, on the page
ep.panel(1000, { cam: { head: 'snape', w: 440, hy: 0.5, dy: 20 }, bg: OF, blur: 2, actors: [CHAIR(1520), SNU({ pose: 'fists', expr: { base: 'angry', mouth: { type: 'scream', open: 0.7 }, eyes: { open: 1.1 } } })] },
...
ep.panel(640, { cam: { x: 1000, y: 700, w: 1500 }, bg: OF, actors: [...ROOM({ d: { expr: 'stern' }, m: { expr: 'sad' }, s: { expr: 'cold' } }), HO({ expr: 'cold', x: 1200, y: 1700 })] },
  [cap('And then there was silence.', 44, 30, { w: 360, fixed: true })], { mood: 'candle', alt: 'The four of them, very still.' });
ep.panel(660, { cam: { head: 'mcgonagall', w: 380, hy: 0.5, dx: 11, dy: -11 }, bg: OF, blur: 3, actors: [CHAIR(520), MG({ expr: { base: 'sad', eyes: { lookY: 0.7, open: 0.5 } }, headTilt: 10 })] },
  [whisper('McGonagall', 'I\'m sorry, Albus.', 560, 90, { anchor: 'tc', w: 260, size: 30, fixed: true })], { mood: 'candle' });
ep.panel(940, { cam: { head: 'harry', w: 460, hy: 0.5, dx: 1, dy: 12 }, bg: OF, blur: 2, actors: SOLO({ expr: 'neutral', pose: 'gesture', turn: -0.4 }) },
...
ep.multi(620, [
  { x: M, y: 18, w: 368, h: 584, mood: 'candle', art: { cam: { head: 'mcgonagall', w: 270, hy: 0.5, dx: 5, dy: -60 }, bg: OF, blur: 3, actors: [CHAIR(520), MG({ expr: { base: 'stern', eyes: { lookX: 0.6 } }, headTilt: -6 })] } },
  { x: 408, y: 18, w: 368, h: 584, mood: 'candle', art: { cam: { head: 'snape', w: 270, hy: 0.5, dx: 1, dy: -60 }, bg: OF, blur: 3, actors: [CHAIR(1520), SN({ expr: { base: 'cross', eyes: { lookX: -0.8, open: 0.55 } }, turn: -0.6 })] } },
...
ep.panel(1160, { cam: { x: 930, y: 534, w: 1000 }, bg: OF, actors: OTS({ d: { expr: 'calm' }, m: { expr: 'sad' }, h: { expr: 'calm' } }) },
  [say('Harry', 'Which she did to stop me going around asking questions. Threat\'s still on the table, and I do expect to be briefed *fully* at some point.', 528, 40, { anchor: 'tc', w: 370, shape: 'box', fixed: true, tail: [740, 900] })], { mood: 'candle' });
ep.multi(1340, [
  { x: M, y: 18, w: 752, h: 500, mood: 'candle', art: { cam: { head: 'dumbledore', w: 400, hy: 0.5, dy: -45 }, bg: OF, blur: 2, actors: D1({ expr: { base: 'cold', cold: false } }), over: (e) => K.glow(e.w / 2, e.h * 0.55, 380, '#9bc4e8', 0.25) } },
  { x: M, y: 536, w: 752, h: 786, mood: 'cold', art: { cam: { head: 'harry', w: 420, hy: 0.5, dy: -130 }, bg: OF, blur: 2, actors: SOLO({ pose: 'stand', expr: 'cold' }), over: (e) => FX.frost(e.w, e.h, 0.4, 137) } },
...
ep.panel(560, { cam: { head: 'dumbledore', w: 360, hy: 0.5, dy: -25 }, bg: OF, blur: 2, actors: D1({ expr: 'cold' }) },
...
ep.panel(1200, { cam: { head: 'harry', w: 380, hy: 0.5, dy: 30 }, bg: OF, blur: 3, actors: SOLO({ pose: 'stand', expr: 'coldSmile' }), over: (e) => FX.frost(e.w, e.h, 0.55, 138) },
...
ep.panel(780, { cam: { head: 'snape', w: 300, hy: 0.5, dy: -35 }, bg: OF, blur: 3, actors: [CHAIR(1520), SN({ expr: { base: 'smile', eyes: { lookX: 0.6 } }, turn: 0.5 })] },
...
ep.panel(980, { cam: { head: 'dumbledore', w: 440, hy: 0.5, dy: -130 }, bg: OF, blur: 2, actors: D1({ expr: 'calm' }) },
...
ep.panel(760, { cam: { head: 'harry', w: 440, hy: 0.5, dy: -90 }, bg: OF, blur: 2, actors: SOLO({ pose: 'fists', expr: { base: 'angry', mouth: { type: 'scream', open: 0.6 } } }) },
  [shout('Harry', 'Leave *me* alone? I am not his only victim, and certainly not the most vulnerable!', 400, 104, { anchor: 'tc', w: 420, size: 34, pad: 50, fixed: true })], { mood: 'candle' });
ep.panel(1240, { cam: { head: 'harry', w: 400, hy: 0.5, dy: 20 }, bg: OF, blur: 3, actors: SOLO({ pose: 'fists', expr: HURT }) },
...
ep.multi(1090, [
  ...
  { x: M, y: 150, w: 752, h: 520, mood: 'candle', breakout: 'top', art: { cam: { head: 'dumbledore', w: 620, hy: 0.5, dx: -95, dy: 100 }, bg: () => OF() + THRONE(), actors: [DLAUGH(), DESK], over: (e) => { const [x, y] = e.anchors.dumbledore.head; return K.glow(x, y, 380, '#ffcf75', 0.3) + FX.emanata(x, y + 30, 150, { n: 10, a0: -175, a1: -5 }); } } },
  { x: M, y: 690, w: 752, h: 382, mood: 'cold', art: { cam: { head: 'harry', w: 440, hy: 0.5, dx: 146, dy: 20 }, bg: OF, blur: 3, actors: SOLO({ expr: 'cold' }), over: (e) => FX.frost(e.w, e.h, 0.4, 139) } },
...
ep.panel(1360, { cam: { head: 'dumbledore', w: 560, hy: 0.5, dx: 6, dy: -153 }, bg: OF, blur: 2, actors: D1({ expr: 'laugh', headTilt: -6, armF: { sh: 30, el: 55, hand: 'fist' } }), over: (e) => { const [x, y] = e.anchors.dumbledore.handF; return FX.emanata(x, y, 70, { n: 7 }); } },
  [shout('Dumbledore', 'Oh, indeed, in very deed, this is my punishment if ever there was one!', 330, 96, { anchor: 'tc', w: 330, size: 32, pad: 50, fixed: true, noTail: true }),
   ...
   cap('He pounded his fist on the desk three times.', 744, 1310, { anchor: 'br', w: 480, fixed: true })], { mood: 'candle' });
ep.multi(1000, [
  { x: M, y: 18, w: 752, h: 460, mood: 'candle', art: { cam: { head: 'harry', w: 400, hy: 0.5, dx: 90, dy: -30 }, bg: OF, blur: 3, actors: SOLO({ expr: 'confused', turn: -0.7 }) } },
  { x: M, y: 496, w: 752, h: 486, mood: 'candle', art: { cam: { head: 'dumbledore', w: 440, hy: 0.5, dx: -110, dy: -60 }, bg: OF, blur: 3, actors: D1({ expr: 'happy' }) } },
...
ep.multi(1020, [
  { x: M, y: 18, w: 752, h: 440, mood: 'candle', art: { cam: { head: 'snape', w: 480, hy: 0.5, dx: 72, dy: -40 }, bg: OF, blur: 2, actors: [CHAIR(1520), SNU({ pose: 'point', turn: -0.6, expr: 'angry' })] } },
  { x: M, y: 476, w: 752, h: 526, mood: 'candle', art: { cam: { head: 'harry', w: 420, hy: 0.5, dx: -60, dy: -50 }, bg: OF, blur: 2, actors: SOLO({ expr: 'neutral' }) } },
...
ep.multi(1480, [
  { x: M, y: 18, w: 752, h: 320, mood: 'candle', art: { cam: { head: 'mcgonagall', w: 330, hy: 0.5, dx: -90, dy: -10 }, bg: OF, blur: 3, actors: [CHAIR(520), MG({ expr: 'worried' })] } },
  { x: M, y: 356, w: 752, h: 620, mood: 'candle', art: { cam: { head: 'dumbledore', w: 380, hy: 0.5, dy: -70 }, bg: OF, blur: 3, actors: D1({ expr: { base: 'bigGrin' } }), over: (e) => K.glow(e.w / 2, e.h * 0.6, 360, '#ffcf75', 0.3) } },
  // the shock jolts the frame
  { x: M, y: 994, w: 752, h: 466, mood: 'candle', shape: 'jag', jag: 12, seed: 5, art: { cam: { x: 1000, y: 770, w: 1450 }, bg: OF, actors: [...ROOM({ d: { expr: 'bigGrin' }, m: { expr: 'shock' }, s: { expr: 'shock' } }), HO({ expr: 'shock', y: 1700 })] } },
...
ep.panel(1340, { cam: { head: 'dumbledore', w: 520, hy: 0.5, dy: 12 }, bg: OF, blur: 2, actors: D1({ expr: 'happy', pose: 'present' }) },
...
ep.multi(960, [
  { x: M, y: 18, w: 368, h: 420, mood: 'candle', art: { cam: { head: 'harry', w: 300, hy: 0.5, dy: -70 }, bg: OF, blur: 3, actors: SOLO({ expr: 'confused' }) } },
  { x: 408, y: 18, w: 368, h: 420, mood: 'candle', art: { cam: { head: 'snape', w: 300, hy: 0.5, dy: -70 }, bg: OF, blur: 3, actors: [CHAIR(1520), SN({ expr: 'blank' })] } },
  { x: M, y: 456, w: 368, h: 486, mood: 'candle', art: { cam: { head: 'snape', w: 340, hy: 0.5, dy: -45 }, bg: OF, blur: 3, actors: [CHAIR(1520), SNU({ pose: 'fists', expr: { base: 'angry', mouth: { type: 'scream', open: 0.8 } } })] } },
  { x: 408, y: 456, w: 368, h: 486, mood: 'candle', art: { cam: { head: 'dumbledore', w: 380, hy: 0.5, dx: 2, dy: -35 }, bg: OF, blur: 3, actors: [THRONE, DLAUGH({ headTilt: -10 }), DESK], over: (e) => FX.emanata(e.w / 2, e.h * 0.62, 150, { n: 8 }) } },
   ...
   shout('Snape', 'This is *insanity!*', 208, 510, { anchor: 'tc', w: 200, size: 34, fixed: true, tail: 'snape@2' }),
   shout('Dumbledore', 'Bwah ha ha!', 592, 510, { anchor: 'tc', w: 200, size: 34, fixed: true })]);
ep.multi(1000, [
  { x: M, y: 18, w: 752, h: 340, mood: 'candle', art: { cam: { head: 'harry', w: 380, hy: 0.5, dx: 110, dy: -10 }, bg: OF, blur: 3, actors: SOLO({ expr: 'neutral' }) } },
  { x: M, y: 376, w: 752, h: 606, mood: 'candle', art: { cam: { head: 'mcgonagall', w: 460, hy: 0.5, dx: 127, dy: 20 }, bg: OF, blur: 2, actors: [CHAIR(520), MG({ expr: 'stern', pose: 'lecture' })] } },
...
ep.panel(940, { cam: { head: 'harry', w: 460, hy: 0.5, dy: -120 }, bg: OF, blur: 2, actors: SOLO({ expr: 'neutral' }) },
...
ep.multi(1260, [
  { x: M, y: 18, w: 752, h: 460, mood: 'candle', art: { cam: { head: 'mcgonagall', w: 420, hy: 0.5, dx: 120, dy: -40 }, bg: OF, blur: 2, actors: [CHAIR(520), MG({ expr: 'stern' })] } },
  { x: M, y: 496, w: 752, h: 746, mood: 'candle', art: { cam: { head: 'harry', w: 440, hy: 0.5, dy: 100 }, bg: OF, blur: 2, actors: SOLO({ expr: { base: 'calm', eyes: { soft: true } } }), over: (e) => K.glow(e.w * 0.5, e.h * 0.4, 200, '#ffcf75', 0.18) } },
...
ep.multi(1300, [
  { x: M, y: 18, w: 752, h: 460, mood: 'candle', art: { cam: { head: 'mcgonagall', w: 420, hy: 0.5, dx: 120, dy: 20 }, bg: OF, blur: 2, actors: [CHAIR(520), MG({ expr: 'stern', pose: 'gesture' })] } },
  { x: M, y: 496, w: 752, h: 786, mood: 'candle', art: { cam: { head: 'harry', w: 440, hy: 0.5, dy: -120 }, bg: OF, blur: 2, actors: SOLO({ expr: 'suspicious' }) } },
...
ep.multi(1060, [
  { x: M, y: 18, w: 368, h: 500, mood: 'candle', art: { cam: { head: 'snape', w: 290, hy: 0.5, dy: -72 }, bg: OF, blur: 3, actors: [CHAIR(1520), SNU({ pose: 'fists', expr: { base: 'angry', mouth: { type: 'scream', open: 0.6 } } })] } },
  { x: 408, y: 18, w: 368, h: 500, mood: 'candle', art: { cam: { head: 'dumbledore', w: 340, hy: 0.5, dy: -110 }, bg: OF, blur: 3, actors: D1({ expr: 'smile' }) } },
  { x: M, y: 536, w: 368, h: 506, mood: 'candle', art: { cam: { head: 'harry', w: 270, hy: 0.5, dy: -92 }, bg: OF, blur: 3, actors: SOLO({ expr: 'shock' }) } },
  { x: 408, y: 536, w: 368, h: 506, mood: 'candle', art: { cam: { head: 'dumbledore', w: 340, hy: 0.5, dy: -110 }, bg: OF, blur: 3, actors: D1({ expr: { base: 'smile', eyes: { lookX: 0.4 } } }) } },
], [shout('Snape', 'In your *dreams*, Potter!', 208, 78, { anchor: 'tc', w: 200, size: 33, fixed: true }),
   say('Dumbledore', 'Minerva is thinking that you\'re righter than you have any right to be.', 592, 54, { anchor: 'tc', w: 230, fixed: true }),
   shout('Harry', 'Are *you* reading *her* mind?', 208, 596, { anchor: 'tc', w: 200, size: 33, fixed: true }),
   say('Dumbledore', 'Common sense is often mistaken for Legilimency.', 592, 566, { anchor: 'tc', w: 230, fixed: true })]);
ep.panel(1200, { cam: { x: 870, y: 340, w: 800 }, bg: () => CS.dumbledoreOffice({ bird: 'none', rack: true }), blur: 2, actors: [THRONE, DB({ expr: 'calm', pose: 'present' }), DESK, () => g({ transform: 'translate(700,340) scale(0.8)' }, P2.phoenix(1, { fly: true }))] },
...
// Fawkes riding on McGonagall's shoulder
const PERCHED = (k = 0.5) => atAnchor('mcgonagall', 'head', P2.phoenix(1, { glow: true }), { dx: mcgonagall.body.headRy * 1.25, dy: mcgonagall.body.headRy * 1.2, k });
...
ep.bleed(1160, { cam: { x: 1000, y: 610, w: 820 }, bg: STAIR, actors: [HSTEP(), HGS(), MGS({ expr: { base: 'calm', eyes: { soft: true } } }), PERCHED(0.6)], over: NOTES(10, 4) },
...
ep.multi(1360, [
  ...
  { x: M, y: 18, w: 752, h: 600, mood: 'candle', frame: 'glow', glow: '#ffc861', art: { cam: { head: 'mcgonagall', w: 460, hy: 0.5, dx: 120, dy: -10 }, bg: STAIR, blur: 2, actors: [MGS({ expr: { base: 'calm', eyes: { soft: true, lookX: 0.6 } } }), PERCHED(0.75)], over: (e) => K.glow(e.w * 0.62, e.h * 0.55, 330, '#ffb24a', 0.45) + NOTES(16, 5)(e) } },
  { x: M, y: 636, w: 752, h: 706, mood: 'candle', art: { cam: { head: 'harry', w: 380, hy: 0.5, dx: 1, dy: 50 }, bg: STAIR, blur: 3, actors: [HSTEP(), HGS({ expr: { base: 'teary', eyes: { lookX: -0.6, lookY: -0.3 } }, turn: -0.3 })], over: NOTES(10, 6) } },
...
ep.panel(940, { cam: { x: 990, y: 700, w: 540 }, bg: STAIR, actors: [HSTEP(), HGS({ expr: 'cry', turn: 0.8, armF: { sh: -70, el: 0, hand: 'open' } }), MGS({ expr: { base: 'sad', eyes: { lookX: 0.7 } }, armB: { sh: 10, el: 5, hand: 'hold' } }), PERCHED()], over: NOTES(8, 7) },
...
ep.bleed(1000, { cam: { x: 1190, y: 720, w: 620 }, bg: GAR, actors: [MGC({ expr: 'sad' }), HGC({ expr: { base: 'teary', eyes: { lookX: 0.6, lookY: -0.2 } } }), () => K.glow(1350, 580, 300, '#ffb24a', 0.5) + g({ transform: 'translate(1350,620) scale(0.9)' }, P2.phoenix(1, { fly: true }))], over: EMBERS },
...
ep.panel(760, { cam: { head: 'mcgonagall', w: 420, hy: 0.5, dy: -70 }, bg: MO, blur: 2, actors: MCS({ expr: 'stern' }) },
  [say('McGonagall', 'Mr Potter. I need to see your Time-Turner, please.', 400, 40, { anchor: 'tc', w: 440, fixed: true })], { mood: 'warm' });
ep.panel(1040, { cam: { head: 'harry', w: 440, hy: 0.5, dx: -7, dy: 41 }, bg: MO, blur: 2, actors: HMS({ expr: 'horror', pose: 'cower' }) },
...
ep.panel(1240, { cam: { head: 'mcgonagall', w: 500, hy: 0.5, dy: 30 }, bg: MO, blur: 2, actors: MCS({ expr: 'sad', pose: 'present' }) },
...
ep.panel(760, { cam: { head: 'harry', w: 420, hy: 0.5, dx: -52, dy: -31 }, bg: MO, blur: 2, actors: HMS({ expr: { base: 'think', eyes: { lookY: 0.7, lookX: 0.3 } }, armF: { sh: 20, el: 110, hand: 'fist' }, headTilt: 8 }) },
...
ep.panel(1400, { cam: { head: 'mcgonagall', w: 480, hy: 0.5 }, bg: MO, blur: 2, actors: MCS({ expr: 'sad' }) },
...
ep.panel(1200, { cam: { head: 'mcgonagall', w: 380, hy: 0.5, dy: 10 }, bg: MO, blur: 3, actors: MCS({ expr: 'stern' }) },
...
ep.multi(1160, [
  { x: M, y: 18, w: 752, h: 620, mood: 'warm', art: { cam: { head: 'harry', w: 400, hy: 0.5, dy: -68 }, bg: MO, blur: 2, actors: HMS({ expr: 'pleading', pose: 'gesture' }) } },
  { x: M, y: 656, w: 752, h: 486, mood: 'warm', art: { cam: { head: 'mcgonagall', w: 440, hy: 0.5, dx: -110, dy: -60 }, bg: MO, blur: 2, actors: MCS({ expr: 'stern' }) } },
...
const HMU = (o = {}) => HM({ pose: 'stand', y: 1060, ...o });
ep.panel(1060, { cam: { head: 'harry', w: 440, hy: 0.5, dy: -80 }, bg: MO, blur: 2, actors: [HCH, HMU({ pose: 'fists', expr: { base: 'angry', eyes: { teary: true }, mouth: { type: 'scream', open: 0.9 } } })] },
...
const MCU = (o = {}) => MC({ pose: 'stand', y: 960, ...o });
ep.multi(1760, [
  // her pointing arm crosses the frame, straight at Harry (and the reader)
  { x: M, y: 18, w: 640, h: 880, mood: 'warm', breakout: 'right', art: { cam: { head: 'mcgonagall', w: 420, hy: 0.5, dx: -71, dy: 70 }, bg: MO, blur: 2, actors: [MCU({ expr: 'angry', pose: 'point', turn: 0.4 })] } },
  { x: M, y: 916, w: 752, h: 826, mood: 'warm', art: { cam: { head: 'mcgonagall', w: 380, hy: 0.5, dx: -11, dy: -115 }, bg: MO, blur: 3, actors: [MCU({ expr: { base: 'angry', mouth: { type: 'shout', open: 0.7 } }, pose: 'point', turn: 0.4 })] } },
...
ep.panel(560, { cam: { head: 'harry', w: 220, hy: 0.5, dy: 10 }, bg: MO, blur: 3, actors: [HMU({ expr: 'shock' })] },
...
ep.multi(1060, [
  { x: M, y: 18, w: 752, h: 560, mood: 'warm', art: { cam: { head: 'mcgonagall', w: 420, hy: 0.5, dy: -60 }, bg: MO, blur: 2, actors: MCS({ expr: { base: 'stern', eyes: { teary: true } } }) } },
...
// overrides keyed by actor id; harry: false leaves him out
const RAVT = (o = {}) => [
  { def: padma, id: 'padma', x: 380, y: 1050, s: 1.05, turn: 0.4, expr: 'worried', ...o.padma },
  { def: hermioneRaven, id: 'hermione', x: 560, y: 1050, s: 1.1, turn: 0.3, expr: 'worried', ...o.hermione },
  o.harry !== false && { def: harryRaven, id: 'harry', x: 830, y: 1050, s: 1.1, turn: 0, expr: 'blank', ...o.harry },
  { def: terry, id: 'terry', x: 1100, y: 1050, s: 1.05, turn: -0.4, expr: 'cross', ...o.terry },
  { def: anthony, id: 'anthony', x: 1270, y: 1050, s: 1.05, turn: -0.4, expr: 'awe', ...o.anthony },
  () => HG.tableFront(),
];
ep.panel(900, { cam: { x: 820, y: 720, w: 1000 }, bg: LT, actors: RAVT() },
  [cap('No-one was speaking to Harry. Some of the Ravenclaws shot him angry looks. Others sympathetic ones. A few of the youngest, admiring. Even Hermione hadn\'t tried to come over.', 44, 30, { w: 620, fixed: true }),
   cap('It was probably the utterly expressionless look on his face.', 44, 860, { anchor: 'bl', w: 470, fixed: true })], { mood: 'day' });
ep.panel(1100, { cam: { on: ['harry'], fr: 'close', dy: 0.15 }, bg: LT, blur: 3, actors: RAVT({ harry: { expr: { base: 'blank', mouth: { type: 'wobble' } } } }) },
...
ep.panel(900, { cam: { head: 'dumbledore', w: 520, hy: 0.5, dy: 40 }, bg: DAIS, actors: [DUM({ pose: 'present', armB: { sh: 60, el: 60, hand: 'hold' } }), () => HG.goblet(1010, 690, 1)] },
...
const RAV4 = (ex) => RAVT({ harry: false, hermione: { x: 720, expr: ex.he }, padma: { x: 500, expr: ex.padma }, terry: { x: 940, expr: ex.terry }, anthony: { x: 1160, expr: ex.anthony } });
...
ep.cutout(820, { cam: { head: 'harry', hw: 0.26, hx: 0.44, hy: 0.42 }, actors: [HS({ expr: 'blank', armB: { sh: 25, el: 95, hand: 'fist' } })], over: (e) => { const [x, y] = e.anchors.harry.handB; return K.glow(x, y, 60, '#fff3c9', 0.5) + FX.emanata(x, y, 30, { n: 6, w: 4 }); } },
...
ep.multi(1020, [
  ...
  { x: M, y: 676, w: 752, h: 326, mood: 'day', art: { cam: { head: 'harry', w: 460, hy: 0.5, dx: 110, dy: 20 }, bg: DAIS, blur: 2, actors: [HS({ expr: 'blank' })] } },
...
ep.panel(720, { cam: { x: 830, y: 740, w: 1040 }, bg: LT, actors: RAVT({ hermione: { expr: 'gasp' }, padma: { expr: 'horror' }, terry: { expr: 'laugh' }, anthony: { expr: 'shock' } }) },
  [cap('There was the sound of many people making muffled chokes at the same time.', 44, 30, { w: 620, fixed: true })], { mood: 'day' });
ep.panel(1040, { cam: { head: 'snape', w: 540, hy: 0.5, dy: -60 }, bg: DAIS, blur: 2, actors: [SNS({ expr: 'menace' }), () => HG.staffTable()] },
...
ep.panel(820, { cam: { head: 'snape', w: 420, hy: 0.5, dx: 23, dy: 40 }, bg: DAIS, blur: 2, actors: [SNS({ expr: 'angry', pose: 'wand', turn: -0.3, armB: { hand: 'hold', prop: g({ transform: 'translate(0,20)' }, rect(-3, -50, 6, 90, { fill: '#8a5d38' })) } })], over: (e) => { const [x, y] = e.anchors.snape.handB; return K.glow(x - 40, y, 70, '#dfe9f0', 0.7) + FX.emanata(x - 40, y, 40, { n: 7 }); } },
...
ep.multi(1060, [
  { x: M, y: 18, w: 752, h: 700, mood: 'day', art: { cam: { head: 'snape', w: 330, hy: 0.5, dy: -20 }, bg: DAIS, blur: 3, actors: [SNS({ expr: 'cold' })] } },
  { x: M, y: 736, w: 752, h: 306, mood: 'day', art: { cam: { head: 'dumbledore', w: 440, hy: 0.5, dx: -120, dy: 20 }, bg: DAIS, blur: 2, actors: [DUM({ expr: 'happy' })] } },
...
ep.multi(1600, [
  ...
  { x: M, y: 1098, w: 368, h: 484, mood: 'day', art: { cam: { head: 's1', w: 300, hy: 0.5, dy: -60 }, bg: () => HG.hallTable('h', { day: true }), blur: 2, actors: [STU, () => HG.tableFront()] } },
  { x: 408, y: 1098, w: 368, h: 484, mood: 'day', art: { cam: { head: 'sprout', w: 300, hy: 0.5, dy: -60 }, bg: DAIS, blur: 2, actors: [SPR, () => HG.staffTable()] } },
...
ep.panel(760, { cam: { on: ['harry'], fr: 'close', dy: 0.1 }, bg: LT, blur: 3, actors: RAVT({ harry: { expr: { base: 'blank', eyes: { lookX: 0.3 } } } }) },
