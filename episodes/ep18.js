...
const SHAKE = (x, y, k = 1) => () => [-1, 1].map((d) => path(`M${x + d * 120 * k},${y - 40 * k} q${d * 14 * k},${34 * k} 0,${68 * k} M${x + d * 146 * k},${y - 24 * k} q${d * 10 * k},${22 * k} 0,${44 * k}`, { fill: 'none', stroke: '#f3e6c4', 'stroke-width': 4, 'stroke-linecap': 'round' })).join('');
ep.panel(760, { cam: { on: ['harry'], fr: 'bust', dy: -0.2 }, bg: () => CS.trunkCavern(), blur: 2, actors: [{ def: harryRaven, id: 'harry', x: 900, y: 900, s: 1.1, turn: 0.1, pose: 'hold', expr: 'hopeful', armF: { sh: 30, el: 90, hand: 'hold', prop: P2.slip(70, 50, {}) } }], over: SHAKE(420, 630) },
...
ep.panel(1150, { cam: { on: ['harry'], fr: 'bust', dy: -0.2 }, bg: () => CS.trunkCavern(), blur: 2, actors: [{ def: harryRaven, id: 'harry', x: 900, y: 900, s: 1.1, turn: 0.1, pose: 'hold', expr: 'horror', armF: { sh: 30, el: 80, hand: 'hold', prop: g({ transform: 'rotate(-6)' }, P2.slip(60, 40, {})) } }], over: SHAKE(430, 690) },
...
ep.bleed(1200, { cam: { x: 1100, y: -160, w: 900 }, bg: FIELD(), actors: [...GHOST({ def: nevilleHuff, x: 1100, y: -120, s: 1.05, turn: 0.2, pose: 'panic', expr: 'horror' }, [-30, 110]), { def: nevilleHuff, id: 'neville', x: 1100, y: -120, s: 1.05, turn: 0.2, pose: 'panic', expr: 'horror', rot: 40 }, CS.broomUnder('neville', 60, 60, 0.9)], over: (e) => FX.speedLines(e.w, e.h, { n: 34, angle: 90 }) },
...
ep.panel(1000, { cam: { on: ['harry'], fr: 'waist', dy: -0.2 }, bg: FIELD(), actors: [{ def: harryRaven, id: 'harry', x: 1500, y: 960, s: 1.1, turn: 0, pose: 'wandUp', expr: 'yell', armB: WAND_UP() }], over: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.3, { n: 20, op: 0.15 }) },
...
const NEV_DOWN = (o = {}) => ({ def: nevilleHuff, id: 'neville', x: 1100, y: 1010, s: 1.05, turn: 0.2, pose: 'fallBack', expr: 'hurt', rot: 80, ...o });
...
ep.panel(720, { cam: { x: 1170, y: 860, w: 760 }, bg: FIELD(), actors: [LOOSEBROOM(1250, 1110, -4), NEV_DOWN()] },
  [sfx('THUD', 250, 170, { size: 110, rot: -6 }), sfx('crack', 650, 540, { size: 50, rot: 8, font: "'Caveat', cursive" })], { mood: 'day', alt: 'The boy lies face down on the grass in a heap.' });
ep.panel(1090, { cam: { x: 1120, y: 500, w: 840 }, bg: FIELD(), actors: [NEV_DOWN({ x: 1060 }), () => g({ transform: 'translate(1250,1060) scale(0.6)' }, healerKit(1)), { def: harryRaven, id: 'harry', x: 1420, y: 1010, s: 1.1, turn: -0.5, pose: 'kneel', expr: 'yell', armB: { sh: 120, el: 40, hand: 'hold', prop: SYRINGE } }, HO({ x: 820, y: 960, pose: 'reach', expr: 'cross', turn: 0.6 })] },
...
ep.panel(1150, { cam: { x: 1370, y: 726, w: 900 }, bg: FIELD(), actors: GR({ d: { x: 1640, turn: -0.5, pose: 'crossArms', expr: 'smug' }, h: { x: 1380, turn: -0.3, expr: 'neutral' }, e: { x: 1110, pose: 'bow', expr: 'calm' } }) },
  [say('Ernie', 'Thank you, Harry Potter, on behalf of Hufflepuff. It was a good try, and a good thought.', 270, 56, { anchor: 'tc', w: 360, fixed: true }),
   say('Draco', 'A good thought *indeed.* Why didn\'t anyone in Hufflepuff have their wands out? I thought Hufflepuffs were supposed to stick together? I guess that\'s why it\'s better to have one Ravenclaw as a friend than *all* of Hufflepuff.', 440, 1096, { anchor: 'bc', w: 430, fixed: true })], { mood: 'day' });
ep.panel(1000, { cam: { on: ['ernie', 'goyle'], fr: 'waist', dy: -1.3 }, bg: FIELD(), actors: GR({ d: { x: 1640 }, cr: { x: 2420 }, e: { x: 1880, turn: 0.4, pose: 'reach', expr: 'worried' }, go: { x: 2170, pose: 'stand', turn: -0.4, expr: 'grin', armB: { sh: 100, el: 40, hand: 'hold', prop: P2.remembrall(1.5) } } }) },
  [say('Goyle', 'Hey, what\'s this?', 610, 50, { anchor: 'tc', w: 240, fixed: true }),
   say('Ernie', 'Neville\'s Remembrall! It turns red if you\'ve forgotten something. Give it here, please, and I\'ll hand it back to him later.', 250, 150, { anchor: 'tc', w: 380, fixed: true })], { mood: 'day', alt: 'Goyle picks up a glass marble full of swirling white mist.' });
ep.bleed(1200, { cam: { x: 2230, y: 470, w: 1020 }, bg: FIELD(), actors: [{ def: goyle, id: 'goyle', x: 2360, y: 250, s: 1.12, turn: -0.3, pose: 'sit', seat: 60, expr: 'laugh', armB: { sh: 120, el: 30, hand: 'hold', prop: P2.remembrall(1.5) } }, CS.broomUnder('goyle', 60, -8, 1.0), ...GR({ go: { x: -9999 }, who: { s1: { pose: 'armsUp' }, s2: { pose: 'armsUp' }, zabini: { pose: 'armsUp' } } })] },
...
ep.panel(1100, { cam: { x: 1550, y: 575, w: 520 }, bg: FIELD(), actors: GR({ d: { x: 1660, turn: -0.4, expr: 'pained' }, h: { x: 1440, turn: 0.4, expr: 'cross' } }) },
  [whisper('Harry', 'Draco, if you don\'t order that idiot back on the ground, the teacher\'s going to get back and…', 262, 50, { anchor: 'tc', w: 370, fixed: true }),
...
ep.multi(1520, [
  ...
  STRIP(766, 362, 1000, [WO('dean', dean, 0.3), { ...WO('gx', student(1805, 'g'), 0.3), x: 770 }], '#9a2a2a'),
  ...
  shout('Dean', '*Wands out, Gryffindor!*', 560, 947, { w: 230, fixed: true }),
...
ep.bleed(960, { cam: { on: ['harry'], fr: 'waist', dy: -0.5 }, bg: FIELD(), actors: GR({ h: { pose: 'point', expr: 'yell', turn: 0.2, armB: { sh: 145, el: 5, hand: 'point' } } }), over: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.4, { n: 26, op: 0.2 }) },
...
ep.panel(1000, { cam: { on: ['draco'], fr: 'bust', dy: -0.5 }, bg: FIELD(), blur: 2, actors: GR({ h: { x: 1200 }, d: { x: 1650, pose: 'point', expr: 'grin' } }) },
  [shout('Draco', '*Hah,* Potter! I\'d like to see you do *that!* Mr Goyle accepts!', 400, 64, { anchor: 'tc', w: 460, fixed: true }),
...
ep.panel(900, { cam: { x: 1835, y: 640, w: 720 }, bg: FIELD(), actors: DUEL({ pose: 'cower', expr: 'wince' }, { pose: 'stand', expr: 'calm', armB: { sh: 100, el: 70, hand: 'fist' } }), over: (e) => { const a = e.anchors.harry; return a ? FX.burst(e.w, e.h, a.handB[0], a.handB[1], { n: 16, inner: 60, op: 0.22 }) : ''; } },
...
ep.panel(860, { cam: { on: ['ernie'], fr: 'bust', dy: -0.35 }, bg: FIELD(), blur: 2, actors: [{ def: ernie, id: 'ernie', x: 1000, y: 980, s: 1.05, turn: 0.3, pose: 'stand', expr: { base: 'shock', eyes: { open: 1.2 } }, armB: { sh: 110, el: 50, hand: 'open' } }], over: ONHAND('ernie') },
  [shout('Ernie', '*Merlin!* It\'s *here!* Right where he fell!', 400, 60, { anchor: 'tc', w: 420, fixed: true })], { mood: 'day', alt: 'Ernie holds up a Remembrall he has just found on the grass.' });
...
ep.panel(1150, { cam: { x: 1930, y: 563, w: 800 }, bg: FIELD(), actors: GR({ who: { s1: { x: 2600 }, s2: { x: 2800 }, zabini: { x: 2170, pose: 'point', expr: 'cross', turn: -0.4 } }, go: { x: 2420 }, d: { x: 1660, pose: 'stand', expr: 'think', turn: 0.2 }, cr: { x: 1920, expr: 'menace', pose: 'fists', turn: 0.5 } }) },
  [shout('Zabini', 'Hold on! How do we know that\'s Neville\'s Remembrall? You could\'ve just dropped *another* one there!', 505, 60, { anchor: 'tc', w: 340, size: 30, fixed: true }),
...
ep.panel(1000, { cam: { x: 1540, y: 688, w: 480 }, bg: FIELD(), actors: GR({ d: { x: 1670, turn: -0.4, expr: 'suspicious' }, h: { x: 1410, turn: 0.4, expr: 'calm' } }) },
  [say('Draco', 'You promise that actually *is* Neville\'s Remembrall?', 540, 50, { anchor: 'tc', w: 340, fixed: true }),
   say('Harry', 'Yes. That\'s the one that\'ll go back to Neville, and it was his originally.', 290, 966, { anchor: 'bc', w: 360, fixed: true })], { mood: 'day' });
ep.panel(900, { cam: { on: ['draco'], fr: 'bust', dy: -0.5 }, bg: FIELD(), blur: 2, actors: GR({ h: { x: 1250 }, d: { x: 1650, pose: 'present', turn: 0, expr: 'determined' } }) },
  [say('Draco', 'I won\'t question the word of the Noble House of Potter, then, no matter how strange that all was. And the Noble and Most Ancient House of Malfoy keeps its word as well. Mr Goyle, give that to Mr Potter.', 400, 50, { anchor: 'tc', w: 540, fixed: true })], { mood: 'day' });
...
ep.panel(1150, { cam: { on: ['mcgonagall'], fr: 'waist', dy: -0.9 }, bg: MO, actors: [MCG({ expr: 'yell', pose: 'fists', seat: undefined, y: 960 }), DESK] },
  [shout('McGonagall', '*You are not to use the Time-Turner in that fashion, Mr Potter!* Is the concept of secrecy not something that you understand?', 400, 80, { anchor: 'tc', w: 500, size: 30, fixed: true })], { mood: 'warm' });
...
ep.panel(1250, { cam: { on: ['mcgonagall'], fr: 'waist', padX: 2.6, dy: -1.2 }, bg: MO, actors: [MCG({ expr: 'yell', pose: 'point', seat: undefined, y: 960, turn: 0.4 }), DESK] },
  [shout('McGonagall', 'You did *not* have to do it! All you needed was to get this *anonymous Slytherin* back on the ground! You could have challenged him to a game of Exploding Snap!', 400, 44, { anchor: 'tc', w: 600, fixed: true }),
   shout('McGonagall', 'But no, you had to use the Time-Turner in a flagrant and unnecessary manner!', 330, 1116, { anchor: 'bc', w: 420, fixed: true })], { mood: 'warm' });
...
ep.bleed(1000, { cam: { on: ['mcgonagall'], fr: 'close', dy: -0.3 }, bg: MO, blur: 3, actors: [MCG({ expr: 'yell', pose: 'fists', seat: undefined, y: 960, turn: 0.05 })], under: (e) => rect(0, 0, e.w, e.h, { fill: '#7b2433' }), over: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.66, { n: 40, op: 0.7, inner: e.w * 0.36, col: '#f6e2b8' }) },
  [shout('McGonagall', '*Then you should have picked WRESTLING!*', 400, 40, { anchor: 'tc', w: 600, size: 62, fixed: true })], { alt: 'McGonagall, at full volume.' });
...
ep.panel(620, { cam: { on: ['harry'], fr: 'eyes' }, bg: MO, blur: 3, actors: [HP({ expr: 'sad' })], over: (e) => FX.frost(e.w, e.h, 0.2, 83) },
  [capC('*Intent to win.* That was what had got him.', 400, 520, { w: 500, fixed: true })], { mood: 'warm' });
...
ep.panel(900, TWO({ expr: 'neutral', pose: 'sit' }, { expr: 'worried' }, { dy: -0.2 }),
  [say('Harry', 'More importantly, why did the Remembrall go off like that? Does it mean I\'ve been Obliviated?', 525, 50, { anchor: 'tc', w: 400, fixed: true })], { mood: 'warm' });
...
ep.panel(900, TWO({ expr: 'neutral', pose: 'sit' }, { expr: 'neutral', pose: 'raiseHand' }, { dy: -0.2 }),
  [say('Harry', 'Um, sorry, I did have something else. It\'s about Professor Quirrell…', 540, 50, { anchor: 'tc', w: 400, fixed: true })], { mood: 'warm' });
...
ep.bleed(1050, { cam: { on: ['mcgonagall'], fr: 'bust', dy: -0.55 }, bg: MO, blur: 2, actors: [MCG({ expr: 'yell', pose: 'raiseHand', seat: undefined, y: 960, turn: 0.1 })], over: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.66, { n: 30, op: 0.55, inner: e.w * 0.36, col: '#f6e2b8' }) },
  [shout('McGonagall', 'Mr Potter! I have a sense of doom as well! And my sense of doom is suggesting that *you must not finish that sentence!*', 400, 50, { anchor: 'tc', w: 560, pad: 64, fixed: true })], { alt: 'McGonagall cuts him off.' });
ep.panel(1150, { cam: { on: ['harry'], fr: 'bust', dy: -0.75 }, bg: MO, blur: 2, actors: [HP({ expr: 'rant', pose: 'gesture' })] },
  [shout('Harry', '*This isn\'t like you!* That seems *unbelievably* irresponsible! If there\'s some kind of jinx on the Defence position, if you already *know* something\'s going to go wrong, I\'d think you\'d all be on your toes…', 400, 64, { anchor: 'tc', w: 620, pad: 64, fixed: true })], { mood: 'warm' });
...
ep.bleed(1050, { cam: { on: ['mcgonagall'], fr: 'waist', padX: 2.3, dx: 0.9, dy: -0.8 }, bg: MO, actors: [MCG({ expr: 'yell', pose: 'point', seat: undefined, y: 960, turn: 0.3 }), DESK], under: (e) => rect(0, 0, e.w, e.h, { fill: '#2a1a10' }), over: (e) => FX.burst(e.w, e.h, e.w * 0.42, e.h * 0.6, { n: 44, op: 0.55, inner: e.w * 0.3, col: '#f6e2b8' }) },
  [shout('McGonagall', '*GET OUT OF MY OFFICE!*', 400, 40, { anchor: 'tc', w: 560, size: 70, fixed: true })], { alt: 'McGonagall points at the door.' });
ep.end();
export default ep;
