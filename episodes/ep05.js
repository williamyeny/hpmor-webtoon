...
ep.panel(900, { cam: { on: ['mcgonagall', 'harry'], fr: 'knees' }, bg: DA, actors: [{ def: mcgonagall, id: 'mcgonagall', x: MX - 160, y: 1080, turn: 0.4, pose: 'stand', expr: 'calm' }, { def: harry, id: 'harry', x: MX + 80, y: 1080, s: 1.1, turn: -0.4, pose: 'stand', expr: 'unimpressed' }] },
  [say('McGonagall', 'I\'m going to go off for a few minutes while you get fitted. Will you be all right with that, Mr Potter?', 540, 110, { w: 370 }),
   cap('Harry nodded. He hated clothes shopping with a fiery passion, and couldn\'t blame the older witch for feeling the same way.', 44, 740, { w: 420 })], { mood: 'day' });
ep.panel(900, { cam: { on: ['mcgonagall', 'harry'], fr: 'knees' }, bg: DA, actors: [{ def: mcgonagall, id: 'mcgonagall', x: MX - 160, y: 1080, turn: 0.4, pose: 'wand', expr: 'calm', armB: { sh: 120, el: 30, hand: 'hold', prop: WAND } }, { def: harry, id: 'harry', x: MX + 80, y: 1080, s: 1.1, turn: -0.4, pose: 'stand', expr: 'worried' }] },
  [say('McGonagall', 'You\'ll need to be clear to Madam Malkin\'s senses, so I am removing the disguise.', 540, 110, { w: 360 })], { mood: 'day' });
...
ep.panel(820, { cam: { on: ['harry'], fr: 'bust' }, bg: MI, blur: 2, actors: [HAR({ expr: { base: 'pleading', tearDrop: true }, pose: 'hold' })], under: (e) => FX.sparkles([[100, 200, 20], [700, 160, 16], [120, 600, 12], [680, 560, 18]], { col: '#fff3b0' }) },
...
ep.panel(780, { cam: { on: ['draco'], fr: 'bust' }, bg: MI, blur: 2, actors: [DRA({ expr: 'smug', pose: 'crossArms' })] },
  [say('Draco', 'Oh.', 230, 90, { w: 110 }),
   say('Draco', 'It\'s good to meet someone who knows his place.', 400, 680, { w: 460, fixed: true })], { mood: 'warm' });
...
ep.panel(860, { cam: { on: ['draco'], fr: 'waist' }, bg: MI, actors: [DRA({ expr: 'delight', pose: 'hold', armF: { sh: 22, el: 70, hand: 'open' }, armB: { sh: 30, el: 60, hand: 'open' } })], under: (e) => FX.sparkles([[100, 200, 20], [700, 160, 16], [120, 600, 12], [680, 560, 18], [400, 80, 14]], { col: '#fff3b0' }) },
...
ep.bleed(1250, { cam: { x: 1800, y: 642, w: 548 }, bg: MI, blur: 1, actors: [{ def: lucius, id: 'lucius', x: 1800, y: 1000, turn: -0.3, pose: 'stand', expr: 'menace', armB: { sh: 20, el: 25, hand: 'hold', prop: g({ transform: 'translate(0,6)' }, cane(330), circle(0, -4, 14, { fill: '#c9ced4', stroke: '#2a1b14', 'stroke-width': 3 })) } }],
...
ep.panel(560, { cam: { on: ['harry'], fr: 'eyes' }, bg: MI, blur: 3, actors: [HAR({ expr: { base: 'shock', eyes: { lookX: 1 } } })], over: (e) => FX.speedLines(e.w, e.h, { n: 20, col: '#fff', angle: 0 }) },
  [inner('Harry', 'In one split second of sympathetic panic, Harry formulated a rescue plan.', 400, 100, { w: 560 })], { mood: 'warm' });
...
ep.panel(460, { cam: { on: ['asst1'], fr: 'bust' }, bg: MI, blur: 3, actors: [AS1({ turn: 0.5, pose: 'facepalm', expr: 'wince', armB: undefined })] },
...
], [cap('Their heads swivelled to look at each other in duplicate motions—', 40, 34, { w: 290, fixed: true }), cap('—and then they recoiled from one another as though performing a synchronised dance.', 424, 34, { w: 290, fixed: true })]);
...
ep.bleed(900, { cam: { x: 820, y: 640, w: 1060 }, bg: MI,
  actors: [MAL({ x: 600, turn: 0.2, pose: 'slump', expr: 'laugh', lean: -20 }), AS2({ x: 380, pose: 'crouch', expr: 'laugh' }), AS1({ x: 1230, pose: 'kneel', expr: 'laugh', turn: -0.2 }), { def: harry, id: 'harry', x: 900, y: 925, s: 1.1, turn: 0.4, pose: 'stand', expr: 'confused' }],
...
ep.panel(900, { cam: { on: ['mcgonagall'], fr: 'bust' }, bg: MI, blur: 2, actors: [{ def: mcgonagall, id: 'mcgonagall', x: 1560, y: 1010, turn: -0.3, pose: 'handsHips', expr: 'stern' }], over: (e) => FX.frost(e.w, e.h, 0.3, 11) },
  [say('McGonagall', 'I leave you alone for six minutes.', 250, 100, { w: 320 }),
...
ep.panel(820, { cam: { x: 830, y: 680, w: 1080 }, bg: MI, actors: [MAL({ x: 560, turn: 0.2, pose: 'slump', expr: 'laugh', lean: -24 }), AS2({ x: 380, pose: 'sitFloor', expr: 'laugh' }), AS1({ x: 1250, pose: 'kneel', expr: 'laugh', turn: -0.2 }), { def: harry, id: 'harry', ...H0, s: 1.1, turn: 0.5, pose: 'stand', expr: { base: 'calm', eyes: { lookX: -0.8 } } }], fg: (e) => L.tapeMeasures(780, 790, 0.55, 9) },
