...
// =============================================================== COLD OPEN
ep.setBg(NIGHT);
ep.bleed(1000, { cam: { x: 800, y: 700, w: 900 }, bg: () => O.moonNight({ moonX: 1030, moonY: 420 }),
  over: (e) => FX.silverThread(-20, e.h * 0.86, e.w + 20, e.h * 0.34, { gx: e.w * 0.62, gy: e.h * 0.55 }) },
  [dark('Beneath the moonlight glints a tiny fragment of silver, a fraction of a line…', 400, 120, { w: 560 })],
...
ep.bleed(900, { cam: { x: 800, y: 800, w: 1000 }, bg: () => O.moonNight({ moonX: 1300, moonY: 300, seed: 5 }),
  ...
  [dark('(black robes, falling)', 400, 450, { w: 420 })], { alt: 'Black robes tumble through the moonlit air like crows.' });
...
// =============================================================== OXFORD
ep.setBg(C.paper);
ep.panel(760, { cam: { x: 800, y: 620, w: 1500 }, bg: () => O.houseExterior() },
  [cap('Oxford, England. The wettest July anyone could remember.', 44, 40, { w: 470 })], { mood: 'rainy', alt: 'A row of Victorian brick houses in the rain. One window glows warm.' });

ep.panel(1000, { cam: { x: 1000, y: 560, w: 2000 }, bg: LR(),
  actors: [DAD({ pose: 'gesture', expr: 'unimpressed' }), MUM({ pose: 'hold', expr: 'worried' }), chairBack, HARRY_CHAIR(), chairFront] },
  [cap('Every inch of wall space is covered by a bookcase.', 44, 36, { w: 460 }),
   cap('And it still isn\'t enough.', 44, 118, { w: 300 }),
   cap('This is the home of Professor Michael Verres-Evans, his wife Petunia Evans-Verres, and their adopted son, Harry James Potter-Evans-Verres.', 330, 790, { w: 420 })],
  { mood: 'warm', alt: 'A cosy living room drowning in books: shelves to the ceiling, heaps under the window. A man and a woman stand arguing by the fire. A small boy with messy black hair and round glasses reads in an armchair.' });

ep.panel(560, { cam: { x: 1000, y: 880, w: 330 }, bg: LR(), blur: 0 },
...
ep.panel(700, { cam: { on: ['dad'], fr: 'bust' }, bg: LR(), actors: [DAD({ expr: 'yell', pose: 'shrug', turn: 0.5 })] },
  [shout('Dad', 'This is absurd! They were at our *wedding!* They came for *Christmas!*', 480, 120, { w: 400, size: 30 })], { mood: 'warm' });
...
ep.panel(760, { cam: { on: ['lily', 'pet'], fr: 'knees' }, bg: gardenMem,
...
// ---- back to the present
ep.panel(700, { cam: { on: ['mum'], fr: 'close' }, bg: LR(), actors: [MUM({ expr: 'teary', turn: -0.3 })] },
...
ep.panel(760, { cam: { on: ['dad', 'mum'], fr: 'waist' }, bg: LR(), actors: [DAD({ expr: 'warm', pose: 'gesture' }), MUM({ expr: 'teary', pose: 'hold' })] },
  [say('Dad', 'Darling. You got sick, you rested in bed, and your skin cleared up on its own.', 230, 90, { w: 340 }),
   say('Mum', 'She was a witch. I *saw* it.', 600, 520, { w: 240 })], { mood: 'warm' });
...
ep.panel(900, { cam: { on: ['harry'], fr: 'full', dy: -0.2 }, bg: LR(), blur: 1,
  ...
  under: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.35, { bg: '#f3e2b8', col: '#d9a55a', op: 0.9 }) },
  [shout('Harry', 'DAD! MUM!', 400, 110, { w: 400, size: 56 })], { mood: 'warm', alt: 'Harry leaps up on the armchair, fists clenched, book flying.' });

ep.panel(620, { cam: { on: ['dad', 'mum'], fr: 'bust' }, bg: LR(), actors: [DAD({ expr: 'gasp', turn: 0.8 }), MUM({ expr: 'gasp', turn: 0.6 })] },
  [cap('They looked at him as though they\'d forgotten there was a third person in the room.', 44, 30, { w: 470 })], { mood: 'warm' });
...
ep.panel(620, { cam: { on: ['mum'], fr: 'bust' }, bg: LR(), actors: [MUM({ expr: 'confused', turn: 0.4 })] },
  [say('Mum', 'Ah… They didn\'t just send a letter. They sent a professor from Hogwarts.', 500, 90, { w: 360 }),
   say('Mum', 'She showed us some magic.', 540, 480, { w: 250 })], { mood: 'warm' });

ep.panel(820, { cam: { on: ['harry'], fr: 'waist' }, bg: LR(), actors: [{ def: harry, id: 'harry', x: 1400, y: 1010, s: 1.12, turn: -0.3, pose: 'present', expr: 'delight' }],
  under: (e) => rect(0, 0, e.w, e.h, { fill: '#f6e3b0' }) },
  [say('Harry', 'Then you don\'t have to fight about this!', 250, 70, { w: 320 }),
   say('Harry', 'We get a Hogwarts professor here, and we *look.* If it\'s real, Dad admits it. If it isn\'t, Mum admits it.', 240, 250, { w: 360 }),
   say('Harry', 'That\'s what experiments are *for!* So we don\'t have to settle things by arguing!', 560, 640, { w: 360 })], { mood: 'warm', alt: 'Harry, lit up with the idea, spreads his hands.' });

ep.panel(760, { cam: { on: ['dad', 'harry'], fr: 'waist' }, bg: LR(),
  ...
  [say('Dad', 'Oh, come now, Harry. Really—*magic?*', 230, 80, { w: 300 }),
   say('Dad', 'I thought *you\'d* know better than to take this seriously, son. Even if you\'re only ten.', 250, 570, { w: 360 })], { mood: 'warm' });
...
ep.panel(760, { cam: { on: ['harry'], fr: 'close' }, bg: LR(), blur: 3, actors: [{ def: harry, id: 'harry', x: 1400, y: 1010, s: 1.12, turn: -0.15, expr: 'hurt' }] },
  [cap('Harry was given anything reasonable he wanted. Books. Tutors. Every maths competition he cared to enter.', 44, 30, { w: 420 }),
   cap('Anything, except the slightest shred of respect.', 300, 640, { w: 440 })], { mood: 'warm' });
...
ep.panel(520, { cam: { on: ['harry'], fr: 'eyes' }, bg: LR(), blur: 3, actors: [{ def: harry, id: 'harry', x: 1400, y: 1010, s: 1.12, turn: -0.1, expr: 'cold' }],
  ...
  [inner('Harry', '*Sometimes Harry wanted to scream at his father.*', 400, 420, { w: 520 })], { mood: 'cold', alt: 'Extreme close-up on Harry\'s eyes, gone flat and cold. Frost creeps at the edges of the panel.' });

ep.panel(700, { cam: { on: ['dad', 'mum', 'harry'], fr: 'waist' }, bg: LR(),
  ...
  [say('Mum', '—just *once*, trust your wife who loves you—', 180, 70, { w: 280, size: 26 }),
...
ep.panel(760, { cam: { on: ['harry'], fr: 'full', dx: 0.5 }, bg: HB(), actors: [{ def: harry, id: 'harry', x: 330, y: 960, s: 1.1, turn: 0.35, pose: 'slump', expr: 'sad' }] },
...
ep.panel(760, { cam: { on: ['harry'], fr: 'waist' }, bg: HB(), actors: [chair(430, 960), { def: harry, id: 'harry', x: 470, y: 965, s: 1.1, turn: 0.5, pose: 'chin', expr: 'think' }] },
  [inner('Harry', 'No-one has ever seen real evidence of magic. It should be a clean case for Mum joking, lying, or being insane…', 250, 110, { w: 400 }),
   inner('Harry', '…in ascending order of awfulness.', 540, 560, { w: 330 })], { mood: 'candle' });

ep.panel(560, { cam: { x: 560, y: 632, w: 360 }, bg: HB(), actors: [g({ transform: 'translate(560,640) scale(0.9) rotate(-4)' }, envelope({}))] },
  [inner('Harry', 'Except some part of me is *utterly certain* that magic is real.', 400, 90, { w: 520 }),
   inner('Harry', '*Where do you come from, strange little prediction?*', 400, 460, { w: 520 })], { mood: 'candle', alt: 'The Hogwarts envelope in the candlelight.' });

ep.panel(680, { cam: { on: ['harry'], fr: 'close' }, bg: HB(), blur: 2, actors: [{ def: harry, id: 'harry', x: 470, y: 965, s: 1.1, turn: 0.2, expr: 'grin' }] },
  [say('Harry', 'Well. The thing to do with a testable hypothesis—', 230, 90, { w: 300 }),
   say('Harry', '—is to go and *test* it.', 560, 560, { w: 260 })], { mood: 'candle' });
...
ep.panel(700, (ctx) => rect(0, 0, ctx.w, ctx.h, { fill: '#5e3e25' }) + K.glow(ctx.w * 0.7, ctx.h * 0.3, 500, C.candle, 0.5) +
  ...
  [cap('If he was going to descend into this madness, he was going to do it with *style.*', 44, 34, { w: 440 })], { mood: 'candle', alt: 'Red wax drips onto the envelope flap; Harry has pressed his initials into it with a penknife: H.J.P.E.V.' });
...
], [cap('His father was reading a book of higher maths, to show how smart he was.', 40, 30, { w: 320 }),
    cap('His mother was cooking his father\'s favourite dinner, to show how loving she was.', 424, 470, { w: 330 })]);
ep.beat(360, [capC('As scary as arguments could be, *not* arguing was somehow much worse.', 400, 180, { w: 560 })]);

ep.panel(760, { cam: { on: ['mum', 'harry'], fr: 'waist' }, bg: () => O.kitchen(),
...
ep.panel(760, { cam: { on: ['harry', 'dad'], fr: 'waist' }, bg: LR(),
  ...
  [say('Harry', 'Well, the letter got here *somehow.* I\'ll wave it around outside and ask for an owl.', 540, 80, { w: 360 }),
...
ep.panel(820, { cam: { x: 900, y: 520, w: 1800 }, bg: GD(), actors: [{ def: harry, id: 'harry', x: 780, y: 1030, s: 1.1, turn: 0.3, pose: 'holdOne', expr: 'worried', armF: { prop: g({ transform: 'translate(0,30) scale(0.35) rotate(90)' }, envelope({ back: true })) } }] },
...
ep.panel(700, { cam: { on: ['harry'], fr: 'waist' }, bg: GD(), blur: 2, actors: [{ def: harry, id: 'harry', x: 780, y: 1030, s: 1.1, turn: -0.5, pose: 'cower', expr: 'flustered' }],
  ...
  [cap('Harry whipped his arm down and hid the envelope behind his back like it was drug money.', 44, 34, { w: 440 })], { mood: 'dusk' });

// Mrs Figg over the fence
const fence = (env) => O.garden();
ep.panel(820, { cam: { on: ['figg', 'harry'], fr: 'waist' }, bg: GD(),
  actors: [{ def: figg, id: 'figg', x: 1500, y: 1180, turn: -0.5, expr: 'suspicious', pose: 'stand' },
    (env) => rect(1100, 640, 1200, 420, { fill: 'none' }) + g({}, ...Array.from({ length: 22 }, (_, i) => path(`M${1100 + i * 44},900 L${1100 + i * 44},720 L${1120 + i * 44},702 L${1140 + i * 44},720 L${1140 + i * 44},900Z`, { fill: '#8a6e50', stroke: '#3e2a1f', 'stroke-width': 2 })), rect(1100, 760, 1100, 18, { fill: '#6e5438', stroke: '#3e2a1f', 'stroke-width': 1.6 })),
    g({ transform: 'translate(1330,712) scale(0.9)' }, cat({ col: '#8f8f8f' })),
    { def: harry, id: 'harry', x: 900, y: 1040, s: 1.1, turn: 0.5, pose: 'cower', expr: 'embarrassed' }] },
  [say('Mrs Figg', 'What are you doing, Harry?', 560, 90, { w: 280 }),
   say('Harry', 'Nothing! Just—testing a really silly theory—', 230, 560, { w: 300 })], { mood: 'dusk', alt: 'An old woman in a hairnet peers over the fence, a grey cat perched beside her.' });

ep.panel(700, { cam: { on: ['figg'], fr: 'close' }, bg: GD(), blur: 2, actors: [{ def: figg, id: 'figg', x: 1500, y: 1180, turn: -0.4, expr: 'smile' }] },
  [say('Mrs Figg', 'Did you get your acceptance letter from Hogwarts?', 260, 90, { w: 320 })], { mood: 'dusk' });
...
ep.panel(760, { cam: { on: ['figg', 'harry'], fr: 'bust' }, bg: GD(),
  actors: [{ def: figg, id: 'figg', x: 1500, y: 1180, turn: -0.5, expr: 'worried', pose: 'stand' }, { def: harry, id: 'harry', x: 1000, y: 1040, s: 1.1, turn: 0.5, pose: 'stand', expr: 'blank' }] },
...
ep.bleed(1200, { cam: { x: 900, y: 380, w: 1500 }, bg: GD({ star: true }), actors: [{ def: harry, id: 'harry', x: 780, y: 1030, s: 1.1, turn: 0.1, pose: 'stand', expr: 'blank' }] },
