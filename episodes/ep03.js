...
const calendar = (ctx) => {
  let out = rect(0, 0, ctx.w, ctx.h, { fill: '#3e4d66' }) + rect(ctx.w * 0.18, 30, ctx.w * 0.64, ctx.h - 60, { fill: '#f4ecd8', stroke: C.ink, 'stroke-width': 3 });
  out += text(ctx.w / 2, 100, 'AUGUST 1991', { 'font-family': 'IM Fell English SC', 'font-size': 40, 'text-anchor': 'middle', fill: C.ink });
  const x0 = ctx.w * 0.2, cw = ctx.w * 0.6 / 7, y0 = 130, ch = (ctx.h - 190) / 5;
  for (let d = 0; d < 31; d++) {
...
ep.panel(760, calendar, [cap('Thirty days is a very long time to wait, when you\'ve just found out that magic is real.', 440, 560, { w: 320 })], { alt: 'A wall calendar for August 1991, every day crossed off in red up to the 29th, which is circled: "D.A.!"' });
ep.multi(560, [
  { x: M, y: 18, w: 368, h: 524, mood: 'candle', art: { cam: { on: ['harry'], fr: 'waist' }, bg: () => O.bedroom(), actors: [{ def: harry, id: 'harry', x: 560, y: 960, s: 1.1, turn: 0.3, pose: 'sitRead', expr: 'focus', armF: { prop: bookHeld('#2f5a40', { rot: 180 }) } }] } },
  { x: 408, y: 18, w: 368, h: 524, mood: 'warm', art: { cam: { on: ['harry'], fr: 'waist' }, bg: () => O.bedroom({ night: false }), actors: [{ def: harry, id: 'harry', x: 560, y: 960, s: 1.1, turn: 0.2, pose: 'wand', expr: 'determined', armB: { sh: 92, el: -8, hand: 'hold', prop: g({ transform: 'translate(0,20) rotate(180)' }, pencil(90)) } }] } },
], [note('3 a.m.', 110, 80, { size: 34 }), note('practising', 520, 80, { size: 34 }), say('Harry', 'Wingardium… Leviosa.', 600, 450, { w: 220, size: 25 })]);

// =============================================================== Charing Cross Road
const CX = () => L.charingCross();
const passers = [makeExtra(31, { muggle: true }), makeExtra(32, { muggle: true }), makeExtra(33, { muggle: true })];
ep.panel(820, { cam: { x: 1100, y: 620, w: 1500 }, bg: CX,
  ...
  [cap('London. The twenty-ninth of August.', 44, 34, { w: 400 }),
   say('Harry', 'Nobody\'s even *looking* at it.', 560, 540, { w: 260 }),
   say('McGonagall', 'They can\'t see it.', 230, 540, { w: 220 })], { mood: 'day', alt: 'A busy London street of bookshops. Between them, a grubby little pub called the Leaky Cauldron that no passer-by so much as glances at.' });
...
const crowd = (o = {}) => [
  { def: patrons[0], id: 'p0', x: 640, y: 1010, turn: 0.4, pose: o.stand ? 'reach' : 'holdOne', armF: o.stand ? undefined : MUG, expr: o.e || 'neutral' },
  { def: patrons[1], id: 'p1', x: 880, y: 1000, turn: -0.4, pose: 'holdOne', armF: MUG, expr: o.e || 'smile' },
  { def: patrons[2], id: 'p2', x: 990, y: 1020, turn: 0.5, pose: o.stand ? 'reach' : 'crossArms', expr: o.e || 'neutral' },
  { def: patrons[3], id: 'p3', x: 460, y: 1030, turn: 0.6, pose: o.stand ? 'reach' : 'holdOne', armF: o.stand ? undefined : MUG, expr: o.e || 'neutral' },
];
ep.panel(760, { cam: { x: 1050, y: 700, w: 1500 }, bg: LI,
  actors: [QUI({ expr: 'neutral', pose: 'stand', s: 0.95 }), ...crowd(), { def: doris, id: 'doris', x: 1150, y: 1000, turn: 0.4, pose: 'hold', expr: 'neutral' }, TOM({ expr: 'shock' }), COUNTER(), MCG({ x: 1180, y: 1100, turn: 0.5 }), HAR({ x: 1350, y: 1110, expr: 'awe' })] },
...
ep.panel(760, { cam: { on: ['tom'], fr: 'bust', dx: -0.3 }, bg: LI, fg: COUNTER, actors: [TOM({ expr: 'shock', pose: 'hold' })] },
  [say('Tom', 'Good Lord. Is this—can this be—?', 260, 110, { w: 300 })], { mood: 'candle' });
ep.panel(700, { cam: { on: ['harry'], fr: 'bust' }, bg: LI, blur: 2, mid: COUNTER, actors: [HAR({ x: 1420, y: 1080, expr: { base: 'smug', eyes: { open: 0.8 } }, pose: 'present' })] },
  [cap('A question like *that* deserved his very best.', 44, 30, { w: 380 }),
   say('Harry', 'Am I—could I be—maybe—you never know—if I\'m *not*—but then the question is—', 280, 470, { w: 380 }),
   say('Harry', '—*who?*', 620, 620, { w: 140 })], { mood: 'candle' });
...
ep.panel(660, { cam: { on: ['harry', 'mcgonagall'], fr: 'bust' }, bg: LI, blur: 1, actors: [MCG({ expr: 'stern', pose: 'handsHips' }), HAR({ expr: 'smug', pose: 'gesture' })] },
  [say('Harry', 'Well, yes, you\'re quite perceptive, most people don\'t realise it so quickly—', 540, 90, { w: 330 }),
   say('McGonagall', 'That\'s enough. Don\'t pester the boy, Tom. He\'s new to all this.', 250, 560, { w: 340 })], { mood: 'candle' });
...
ep.panel(760, { cam: { on: ['doris', 'harry'], fr: 'waist' }, bg: LI, actors: [{ def: doris, id: 'doris', x: 1220, y: 1060, turn: 0.4, pose: 'reach', expr: 'teary' }, HAR({ x: 1440, turn: -0.4, expr: 'worried', pose: 'stand' })] },
  [say('Doris', 'But it *is* him? It\'s Harry Potter?', 230, 90, { w: 300 }),
   say('Doris', 'I only want to shake his hand.', 240, 250, { w: 260 })], { mood: 'candle' });
ep.panel(720, (ctx) => {
  ...
  return out;
    ...
    say('Doris', 'Thank you, Harry Potter. Thank heavens for you.', 330, 610, { w: 380, tail: [40, 470] })], { mood: 'candle', alt: 'Close on two hands: an old woman\'s, wrinkled, clasping Harry\'s small one. A tear falls onto them.' });
ep.panel(600, { cam: { on: ['harry'], fr: 'close' }, bg: LI, blur: 2, actors: [HAR({ x: 1440, turn: -0.2, expr: 'pleading' })] },
  [say('Harry', 'You\'re… welcome?', 560, 100, { w: 220 }),
   cap('He turned his head and shot Professor McGonagall a frightened, pleading look.', 44, 450, { w: 380 })], { mood: 'candle' });
ep.panel(760, { cam: { x: 900, y: 700, w: 1500 }, bg: LI, mid: COUNTER,
  ...
  [cap('Chairs scraped. The whole room was rising.', 44, 34, { w: 360 })], { mood: 'candle' });
ep.bleed(900, { cam: { on: ['mcgonagall'], fr: 'full' }, bg: LI, blur: 2, actors: [MCG({ x: 1560, turn: -0.2, expr: 'menace', pose: 'fists', legF: { hip: -10, knee: 30 } })],
  under: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.95, { bg: '#2a1a10', col: '#f3c66f', op: 0.5, n: 70 }),
  over: (e) => FX.sfxText(e.w / 2, e.h * 0.9, 'CRACK', { size: 120, fill: '#f6e3b0' }) },
  [cap('Professor McGonagall slammed her foot down. It gave Harry a new referent for the phrase "Crack of Doom".', 44, 34, { w: 420 })], { mood: 'candle', alt: 'McGonagall stamps her foot; the sound is enormous; everyone freezes.' });
ep.panel(620, { cam: { on: ['mcgonagall'], fr: 'close' }, bg: LI, blur: 2, actors: [MCG({ x: 1560, turn: -0.3, expr: 'calm' })] },
  [say('McGonagall', 'We\'re in a hurry.', 260, 110, { w: 220 }), cap('…in a voice that sounded perfectly, utterly normal.', 380, 480, { w: 340 })], { mood: 'candle' });
...
ep.panel(460, { cam: { on: ['harry'], fr: 'eyes' }, bg: LI, blur: 3, actors: [HAR({ x: 1440, turn: -0.5, expr: { base: 'worried', eyes: { lookX: -1 } } })], over: (e) => FX.doom(e.w, e.h, 5) },
  [note('mmmmmmmmmm', 400, 360, { size: 34, color: '#7a3a4a', w: 500 })], { mood: 'candle', alt: 'Harry\'s eyes meet the stranger\'s. A low, wrong hum.' });
...
ep.panel(760, { cam: { on: ['harry', 'mcgonagall'], fr: 'waist' }, bg: CY, actors: [HC({ expr: 'think', pose: 'chin' }), MC({ expr: 'calm' })] },
  [say('Harry', 'Professor? Who was that pale man, by the corner? The one with the twitching eye?', 230, 90, { w: 330 }),
   say('McGonagall', 'Hm? That was Professor Quirinus Quirrell. He\'ll be teaching Defence Against the Dark Arts at Hogwarts this year.', 570, 560, { w: 360 })], { mood: 'day' });
ep.panel(680, { cam: { on: ['harry'], fr: 'bust' }, bg: CY, blur: 2, actors: [HC({ expr: 'pained', pose: 'facepalm', turn: 0.3 })] },
  [say('Harry', 'I had the strangest feeling that I knew him.', 540, 90, { w: 300 }),
   say('Harry', 'And that I shouldn\'t shake his hand.', 560, 560, { w: 280 })], { mood: 'day' });
...
ep.panel(760, { cam: { on: ['harry'], fr: 'bust' }, bg: CY, blur: 2, actors: [HC({ expr: 'determined', pose: 'crossArms' })] },
  [say('Harry', 'My parents are alive and well. They always refused to talk about how my *genetic* parents died.', 560, 100, { w: 340 }),
   say('Harry', 'From which I infer that it wasn\'t good.', 560, 590, { w: 300 })], { mood: 'day' });
ep.panel(700, { cam: { on: ['mcgonagall'], fr: 'close' }, bg: CY, blur: 2, actors: [MC({ expr: 'pained' })] },
  [say('McGonagall', 'An admirable loyalty. Though it hurts a little, to hear you say it like that.', 260, 100, { w: 340 }),
   say('McGonagall', 'Lily and James were friends of mine.', 280, 560, { w: 280 })], { mood: 'day' });
ep.panel(700, { cam: { on: ['harry'], fr: 'bust' }, bg: CY, blur: 2, actors: [HC({ expr: 'sad', pose: 'slump', turn: 0.2 })] },
  [whisper('Harry', 'I\'m sorry. But I *have* a Mum and Dad.', 560, 90, { w: 300, size: 27 }),
   whisper('Harry', 'And I\'d just make myself unhappy, comparing that reality to… something perfect I built up in my imagination.', 520, 560, { w: 380, size: 27 })], { mood: 'day' });
ep.panel(720, { cam: { on: ['harry', 'mcgonagall'], fr: 'bust' }, bg: CY, actors: [HC({ expr: 'sad', pose: 'slump' }), MC({ expr: 'warm' })] },
  [say('McGonagall', 'That is amazingly wise of you.', 560, 90, { w: 260 }),
   say('McGonagall', 'But your parents died very well indeed, Mr Potter. Protecting you.', 560, 250, { w: 320 })], { mood: 'day' });
ep.panel(520, { cam: { on: ['harry'], fr: 'eyes' }, bg: CY, blur: 3, actors: [HC({ expr: 'hurt', turn: 0.1 })] },
  [inner('Harry', '*Protecting me?*', 400, 380, { w: 260 })], { mood: 'day' });
ep.panel(760, { cam: { on: ['mcgonagall', 'harry'], fr: 'waist' }, bg: CY,
  actors: [HC({ expr: { base: 'shock', eyes: { lookY: -0.8 } }, pose: 'stand' }), MC({ expr: 'calm', pose: 'wand', armB: { sh: 115, el: 20, hand: 'hold', prop: WAND } })],
...
ep.panel(700, { cam: { x: 1000, y: 560, w: 900 }, bg: CY, actors: [MC({ x: 800, turn: 0.6, expr: 'calm', pose: 'wand', armB: { sh: 100, el: 10, hand: 'hold', prop: WAND } })] },
  [note('tap', 820, 320, { size: 40, rot: -10 }), note('tap', 870, 370, { size: 40, rot: 6 }), note('tap', 920, 330, { size: 40, rot: -4 })], { mood: 'day', alt: 'McGonagall taps a brick three times with her wand.' });
ep.multi(620, [
...
], [cap('The brick hollowed—', 44, 34, { w: 200 }), cap('—dilated, and shivered—', 420, 520, { w: 260 })]);
...
ep.panel(760, { cam: { x: 900, y: 700, w: 1100 }, bg: DA(), mid: () => L.stall(700, 'BOUNCE BOOTS!', C.burgundy, 'boots') + L.stall(1140, 'Knives +3!  Forks +2!', C.forest, 'cutlery'),
...
ep.panel(700, (ctx) => {
  const s = shot({ cam: { on: ['harry'], fr: 'bust' }, bg: DA(), blur: 3, actors: [{ def: harry, id: 'harry', x: 1300, y: 1150, s: 1.2, turn: 0, expr: 'delight' }] })(ctx);
  let arcs = ''; for (let i = 0; i < 4; i++) arcs += path(`M${ctx.w / 2 - 170 - i * 20},${ctx.h * 0.45} A${190 + i * 20},${140 + i * 16} 0 0 1 ${ctx.w / 2 + 170 + i * 20},${ctx.h * 0.45}`, { fill: 'none', stroke: C.ink, 'stroke-width': 4, opacity: 0.6 - i * 0.12, 'stroke-linecap': 'round' });
  return s + arcs;
}, [cap('Harry\'s head kept rotating, like it was trying to wind itself off his neck.', 44, 30, { w: 380 }),
    inner('Harry', 'It\'s like the magic items section of a *Dungeons & Dragons* rulebook! What if one of these is one of the three you need to complete the cycle of infinite *wish* spells?!', 400, 590, { w: 560 })], { mood: 'day' });
ep.panel(760, { cam: { x: 3000 * 0 + 520, y: 620, w: 700 }, bg: DA({ start: 0 }), actors: [{ def: harry, id: 'harry', x: 560, y: 1060, s: 1.1, turn: 0.4, pose: 'walk', expr: 'awe' }, { def: mcgonagall, id: 'mcgonagall', x: 820, y: 1080, turn: -0.3, pose: 'handsHips', expr: 'stern' }] },
  [say('McGonagall', 'Mr Potter?', 600, 110, { w: 180 }),
   cap('Harry had veered off, entirely without thinking, toward a shop with fiery letters in the window.', 44, 34, { w: 330 })], { mood: 'day' });
ep.panel(720, { cam: { on: ['harry'], fr: 'bust' }, bg: DA({ start: 0 }), blur: 2, actors: [{ def: harry, id: 'harry', x: 560, y: 1060, s: 1.1, turn: 0.5, pose: 'gesture', expr: 'embarrassed' }] },
  [say('Harry', 'I\'m sorry! When you walk past a bookshop you haven\'t visited before, you *have* to go in and look around.', 540, 100, { w: 360 }),
   say('Harry', 'It\'s the family rule.', 560, 580, { w: 220 })], { mood: 'day' });
...
], [say('Harry', 'What?', 120, 70, { w: 120 }), say('McGonagall', 'Nothing.', 700, 70, { w: 150 })]);

ep.panel(760, { cam: { on: ['harry', 'mcgonagall'], fr: 'waist' }, bg: DA({ start: 3 }), actors: [{ def: harry, id: 'harry', x: 700, y: 1060, s: 1.1, turn: 0.4, pose: 'walk', expr: 'determined' }, { def: mcgonagall, id: 'mcgonagall', x: 940, y: 1080, turn: 0.4, pose: 'walk2', expr: 'calm' }] },
  [say('Harry', 'Don\'t get me wrong, this is a *great* distraction. Probably the best distraction anyone has ever tried on me.', 230, 90, { w: 360 }),
   say('Harry', 'But don\'t think I\'ve forgotten about our pending discussion.', 240, 580, { w: 340 })], { mood: 'day' });
ep.panel(700, { cam: { on: ['mcgonagall'], fr: 'bust' }, bg: DA({ start: 3 }), blur: 2, actors: [{ def: mcgonagall, id: 'mcgonagall', x: 940, y: 1080, turn: -0.2, pose: 'stand', expr: 'sad' }] },
  [say('McGonagall', 'I suppose it would be rather pointless to hide it, when anyone on the street could tell you.', 260, 110, { w: 360 }),
   say('McGonagall', 'Very well.', 270, 560, { w: 180 })], { mood: 'day' });
...
ep.panel(1000, { cam: { x: 800, y: 620, w: 760 }, bg: stage('#1d3a29', '#6a8f5a'),
...
ep.panel(600, { cam: { on: ['harry'], fr: 'close' }, bg: DA({ start: 3 }), blur: 3, actors: [{ def: harry, id: 'harry', x: 700, y: 1060, s: 1.1, turn: 0.1, expr: 'cold' }], over: (e) => FX.frost(e.w, e.h, 0.7, 8) },
  [whisper('Harry', 'Voldemort?', 560, 110, { w: 180 }),
   cold('Harry', 'The name burned with a cold feeling. Ruthlessness. Diamond clarity. A hammer of titanium descending on an anvil of flesh.', 400, 480, { w: 560 })],
  { mood: 'cold', alt: 'Harry whispers the name, and the panel freezes over.' });
ep.panel(460, (ctx) => rect(0, 0, ctx.w, ctx.h, { fill: '#e9f1f6' }), [inner('Harry', 'He resolved, then and there, to use safer terms. Like *You-Know-Who.*', 400, 220, { w: 520 })], { border: 'none' });
// the war
...
ep.panel(760, { cam: { x: 800, y: 640, w: 1400 }, bg: stage('#2a1a2a', '#8a5a6a'), actors: [masked(300), masked(520, 1.1), masked(760, 1.2), masked(1000, 1.1), masked(1240)],
...
ep.panel(700, (ctx) => {
  ...
  out += g({ transform: `translate(${w * 0.5},${h * 0.4}) rotate(-4)` }, rect(-140, -120, 280, 220, { fill: '#f4ecd8', stroke: C.ink, 'stroke-width': 3 }), text(0, -70, 'THE DAILY PROPHET', { 'font-family': 'UnifrakturMaguntia', 'font-size': 28, 'text-anchor': 'middle', fill: C.ink }), text(0, -20, 'ENOUGH!', { 'font-family': 'IM Fell English SC', 'font-size': 44, 'text-anchor': 'middle', fill: C.ink }), text(0, 20, 'by Yermy Wibble', { 'font-family': 'IM Fell English', 'font-size': 20, 'text-anchor': 'middle', fill: C.ink }), path('M-120,50 L120,50 M-120,66 L100,66 M-120,82 L110,82', { stroke: '#8a7d68', 'stroke-width': 3 }));
  ...
  return out;
    ...
    dark('The next morning, they found what was left of him—and of his wife, and his two daughters—nailed to the newsroom wall.', 400, 620, { w: 600 })],
...
ep.panel(900, { cam: { x: 800, y: 660, w: 1000 }, bg: stage('#3a2a14', '#e9b86a'),
  ...
  [dark('They might have died with their wands in their hands and not regretted it, for they *were* heroes.', 400, 90, { w: 600 }),
   dark('But they had an infant son.', 400, 780, { w: 400 })], { border: 'none', alt: 'Silhouettes: a young man with untidy hair and glasses, and a young woman holding a baby.' });
...
ep.panel(900, { cam: { on: ['mcgonagall', 'harry'], fr: 'waist', padX: 1.05 }, bg: DA({ start: 3 }), blur: 2,
  actors: [{ def: mcgonagall, id: 'mcgonagall', x: 900, y: 1080, turn: -0.3, pose: 'hug', expr: 'teary' }, { def: harry, id: 'harry', x: 820, y: 1060, s: 1.1, turn: 0.7, pose: 'hug', expr: 'sob' }] },
...
ep.panel(700, { cam: { on: ['harry'], fr: 'bust' }, bg: DA({ start: 3 }), blur: 2, actors: [{ def: harry, id: 'harry', x: 820, y: 1060, s: 1.1, turn: 0.4, expr: 'hurt' }] },
  [whisper('Harry', 'So what happened?', 560, 110, { w: 220 })], { mood: 'day' });
// Godric's Hollow
ep.setBg('#0c0f1a');
...
ep.beat(380, [plain('?', 670, 190, { font: "'IM Fell English', serif", size: 34, color: '#b9ad92' }),
...
ep.panel(700, { cam: { on: ['harry', 'mcgonagall'], fr: 'bust' }, bg: DA({ start: 3 }), blur: 2, actors: [{ def: harry, id: 'harry', x: 820, y: 1060, s: 1.1, turn: 0.4, pose: 'stand', expr: 'warm' }, { def: mcgonagall, id: 'mcgonagall', x: 1060, y: 1080, turn: -0.3, pose: 'stand', expr: 'sad' }] },
  [say('Harry', 'You can call them my parents, if you want. You don\'t have to say "genetic parents".', 250, 90, { w: 340 }),
   say('Harry', 'I guess there\'s no reason I can\'t have two mothers and two fathers.', 250, 590, { w: 340 })], { mood: 'day' });
