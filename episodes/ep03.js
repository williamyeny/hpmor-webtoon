...
ep.setBg(C.paper);
// a short strip that fades the page from one colour to the next
const shiftBg = (from, to) => { ep.setBg(to); ep.tile({ h: 140, panels: [], bubbles: [], bg: { top: from, bottom: to } }); };
...
ep.panel(780, { cam: { x: 1100, y: 640, w: 1180 }, bg: CX,
  actors: [{ def: passers[0], x: 480, y: 1060, turn: 0.7, pose: 'walk' }, { def: passers[1], x: 1600, y: 1080, turn: -0.7, pose: 'walk2' },
    MCG({ x: 1000, y: 1060, turn: 0.3, pose: 'stand', expr: 'calm' }), HAR({ x: 1180, y: 1070, turn: -0.2, pose: 'stand', expr: 'suspicious' }),
...
const MUG = { sh: 30, el: 75, hand: 'hold', under: g({ transform: 'translate(0,12) rotate(105)' }, tankard(1.1)) };
ep.panel(720, { cam: { x: 1070, y: 600, w: 1360 }, bg: LI,
  actors: [QUI({ x: 440, y: 960, expr: 'neutral', pose: 'stand', s: 0.9 }),
    { def: patrons[1], id: 'p1', x: 745, y: 935, s: 0.88, turn: 0.5, pose: 'holdOne', armF: MUG, expr: 'smile' },
    { def: patrons[2], id: 'p2', x: 1030, y: 940, s: 0.88, turn: -0.5, pose: 'crossArms', expr: 'neutral' },
    { def: patrons[3], id: 'p3', x: 600, y: 1030, turn: 0.6, pose: 'holdOne', armF: MUG, expr: 'neutral' },
    { def: patrons[0], id: 'p0', x: 870, y: 1040, turn: -0.4, pose: 'holdOne', armF: MUG, expr: 'neutral' },
    { def: doris, id: 'doris', x: 1110, y: 1030, turn: 0.4, pose: 'hold', expr: 'neutral' }, TOM({ expr: 'shock' }), COUNTER(), MCG({ x: 1270, y: 1100, turn: 0.5 }), HAR({ x: 1430, y: 1110, expr: 'awe' })] },
...
// Doris
ep.panel(760, { cam: { on: ['doris', 'harry'], fr: 'waist' }, bg: LI, actors: [{ def: doris, id: 'doris', x: 1220, y: 1060, turn: 0.4, pose: 'reach', armF: { sh: 48, el: 12, hand: 'open' }, armB: { sh: 40, el: 18, hand: 'open' }, expr: 'teary' }, HAR({ x: 1440, turn: -0.4, expr: 'worried', pose: 'stand' })] },
...
ep.panel(700, { cam: { x: 1010, y: 610, w: 1300 }, bg: LI, mid: COUNTER,
  actors: [{ def: patrons[0], id: 'p0', x: 600, y: 1010, turn: 0.4, pose: 'reach', expr: 'hopeful' }, { def: patrons[1], id: 'p1', x: 900, y: 1000, turn: -0.4, pose: 'holdOne', armF: MUG, expr: 'hopeful' },
    { def: patrons[2], id: 'p2', x: 1060, y: 1020, turn: 0.5, pose: 'reach', expr: 'hopeful' }, { def: patrons[3], id: 'p3', x: 430, y: 1030, turn: 0.6, pose: 'reach', expr: 'hopeful' },
    { def: doris, id: 'doris', x: 1230, y: 1060, turn: 0.4, pose: 'stand', expr: 'teary' }, { def: patrons[4], id: 'p4', x: 760, y: 1050, turn: 0.5, pose: 'reach', expr: 'delight' }, HAR({ x: 1440, turn: -0.3, expr: 'shock', pose: 'cower' }), MCG({ x: 1560, turn: -0.3, expr: 'stern' })] },
...
ep.panel(760, { cam: { on: ['quirrell'], fr: 'bust' }, bg: LI, blur: 1, actors: [QUI({ expr: { base: 'twitch', eyes: { lookX: 0.8 } }, pose: 'stand', turn: 0.3 })],
  ...
  [cap('In the corner, a pale young man with a twitching eye was watching him.', 44, 34, { w: 380 })], { mood: 'candle', alt: 'In the corner by the fire, a pale, thin, balding young man with a twitching eye watches Harry. The panel seems subtly wrong, its colours split.' });
...
const HC = (o = {}) => (HAR({ x: 700, y: 1040, turn: 0.4, ...o }));
const MC = (o = {}) => (MCG({ x: 920, y: 1060, turn: -0.4, ...o }));
...
ep.panel(680, { cam: { on: ['harry'], fr: 'bust', dx: 0.7 }, bg: CY, blur: 2, actors: [HC({ expr: 'pained', pose: 'stand', turn: 0.3 })] },
...
ep.panel(760, { cam: { on: ['mcgonagall', 'harry'], fr: 'waist' }, bg: CY,
  actors: [HC({ expr: { base: 'shock', eyes: { lookY: -0.8 } }, pose: 'stand' }), MC({ expr: 'calm', pose: 'wand', armB: { sh: 105, el: -35, hand: 'hold', under: WAND_OUT } })],
  over: (e) => { const [x, y] = e.anchors.harry.head; return FX.sparkles([[x, y - 50, 18], [x + 30, y - 70, 10], [x - 34, y - 64, 8]], { col: '#fff3b0' }); } },
...
ep.panel(640, { cam: { x: 930, y: 560, w: 700 }, bg: CY, actors: [MC({ x: 880, y: 1020, turn: 0.6, expr: 'calm', pose: 'wand', armB: { sh: 150, el: 2, hand: 'hold', under: g({ transform: 'translate(0,-34)' }, wand(150, '#4a2e1b')) } })] },
...
ep.panel(720, { cam: { x: 890, y: 660, w: 900 }, bg: DA(), mid: () => L.stall(700, 'BOUNCE BOOTS!', C.burgundy, 'boots') + L.stall(1140, 'Knives +3!  Forks +2!', C.forest, 'cutlery'),
  ...
  [shout('merch', '"Made with real Flubber!"', 250, 120, { w: 280, size: 28 }),
   say('?', 'Spoons with a +4 bonus!', 600, 170, { w: 240, tail: [700, 400] })], { mood: 'day' });
...
ep.panel(920, (ctx) => {
  ...
  const a = ctx.anchors.harry, hx = a.head[0], hy = a.head[1] - a.hr * 0.2, R = a.hr * 1.18;
...
ep.panel(760, { cam: { x: 520, y: 620, w: 700 }, bg: DA({ start: 0 }), actors: [HAR({ x: 560, y: 1060, turn: 0.4, pose: 'walk', expr: 'awe' }), MCG({ x: 820, y: 1080, turn: -0.3, pose: 'handsHips', expr: 'stern' })] },
...
ep.panel(760, { cam: { on: ['harry'], fr: 'bust', dx: 0.6, dy: -0.3 }, bg: DA({ start: 0 }), blur: 2, actors: [HAR({ x: 560, y: 1060, turn: 0.5, pose: 'gesture', expr: 'embarrassed' })] },
...
ep.panel(560, { cam: { on: ['mcgonagall'], fr: 'close' }, bg: DA({ start: 0 }), blur: 2, actors: [MCG({ x: 820, y: 1080, turn: -0.2, expr: 'unimpressed' })] },
  [say('McGonagall', 'That is the most Ravenclaw thing I have ever heard.', 260, 110, { w: 320 })], { mood: 'day' });
ep.multi(360, [
  { x: M, y: 18, w: 368, h: 324, mood: 'day', art: { cam: { on: ['harry'], fr: 'close' }, bg: DA(), blur: 3, actors: [HAR({ x: 560, y: 1060, turn: 0.4, expr: 'confused' })] } },
  { x: 408, y: 18, w: 368, h: 324, mood: 'day', art: { cam: { on: ['mcgonagall'], fr: 'close' }, bg: DA(), blur: 3, actors: [MCG({ x: 820, y: 1080, turn: -0.4, expr: 'calm' })] } },
], [say('Harry', 'What?', 120, 70, { w: 120 }), say('McGonagall', 'Nothing.', 675, 72, { w: 150 })]);

ep.panel(900, { cam: { on: ['harry', 'mcgonagall'], fr: 'waist', dy: -0.75 }, bg: DA({ start: 3 }), actors: [HAR({ x: 700, y: 1060, turn: 0.4, pose: 'walk', expr: 'determined' }), MCG({ x: 940, y: 1080, turn: 0.4, pose: 'walk2', expr: 'calm' })] },
...
ep.panel(700, { cam: { on: ['mcgonagall'], fr: 'bust', dx: -0.6 }, bg: DA({ start: 3 }), blur: 2, actors: [MCG({ x: 940, y: 1080, turn: -0.2, pose: 'stand', expr: 'sad' })] },
...
shiftBg(C.paper, '#1b1210');
ep.panel(900, { cam: { x: 800, y: 700, w: 620 }, bg: stage('#1d3a29', '#6a8f5a'),
  actors: [{ def: darkLord, id: 'dl', x: 800, y: 1080, turn: 0.1, pose: 'stand', ...SIL }],
  over: (e) => { const [x, y] = e.anchors.dl.head; return ellipse(x - 14, y + 4, 9, 4, { fill: '#ff2a2a' }) + ellipse(x + 16, y + 4, 9, 4, { fill: '#ff2a2a' }) + K.glow(x, y + 4, 60, '#ff2a2a', 0.5); } },
...
ep.panel(820, { cam: { on: ['harry'], fr: 'close', dy: 0.45 }, bg: DA({ start: 3 }), blur: 3, actors: [HAR({ x: 700, y: 1060, turn: 0.1, expr: 'cold' })], over: (e) => FX.frost(e.w, e.h, 0.7, 8) },
  [whisper('Harry', 'Voldemort?', 600, 92, { w: 200, fixed: true, tail: 'harry' }),
...
ep.panel(1010, { cam: { x: 795, y: 800, w: 640 }, bg: stage('#3a2a14', '#e9b86a'),
  ...
  over: (e) => { const a = e.anchors.lily, [x, y] = a.handF, k = a.hr / 40; return ember(e) + g({ transform: `translate(${x - 10 * k},${y + 4 * k}) scale(${k})` }, ellipse(0, 0, 34, 24, { fill: '#f1e6cc', stroke: '#000', 'stroke-width': 2 }), circle(16, -10, 12, { fill: '#f3d2b5', stroke: '#000', 'stroke-width': 2 }), path('M8,-18 q6,-8 14,-2', { stroke: '#000', 'stroke-width': 2, fill: 'none' })) + K.glow(x, y, 120 * k, '#fff3c9', 0.5); } },
...
// Harry breaks
shiftBg('#1b1210', C.paper);
ep.panel(760, { cam: { on: ['harry'], fr: 'close' }, bg: DA({ start: 3 }), blur: 3, actors: [HAR({ x: 700, y: 1060, turn: 0.1, expr: 'teary' })] },
...
ep.panel(900, { cam: { on: ['mcgonagall', 'harry'], fr: 'waist', padX: 1.05 }, bg: DA({ start: 3 }), blur: 2,
  actors: [{ def: mcgonagall, id: 'mcgonagall', x: 900, y: 1080, turn: -0.3, pose: 'stand', armF: { sh: 38, el: 62, hand: 'open' }, armB: { sh: 24, el: 50, hand: 'open' }, expr: 'teary' }, { def: harry, id: 'harry', x: 820, y: 1060, s: 1.1, turn: 0.6, pose: 'stand', armF: { sh: 40, el: 30, hand: 'open' }, armB: { sh: 35, el: 30, hand: 'open' }, expr: 'sob' }] },
...
ep.panel(700, { cam: { on: ['harry'], fr: 'bust' }, bg: DA({ start: 3 }), blur: 2, actors: [HAR({ x: 820, y: 1060, turn: 0.4, expr: 'hurt' })] },
  [whisper('Harry', 'So what happened?', 555, 100, { w: 280, fixed: true })], { mood: 'day' });
// Godric's Hollow
shiftBg(C.paper, '#0c0f1a');
...
ep.panel(620, { cam: { on: ['harry'], fr: 'bust' }, bg: DA({ start: 3 }), blur: 2, actors: [HAR({ x: 820, y: 1060, turn: 0.5, pose: 'slump', expr: 'sad' })] },
  [say('Harry', 'I\'ll—have to think about this.', 560, 90, { w: 280 })], { mood: 'day' });
ep.panel(920, { cam: { on: ['harry', 'mcgonagall'], fr: 'waist', dy: -0.45 }, bg: DA({ start: 3 }), blur: 2, actors: [HAR({ x: 820, y: 1060, turn: 0.4, pose: 'stand', expr: 'warm' }), MCG({ x: 1060, y: 1080, turn: -0.3, pose: 'stand', expr: 'sad' })] },
...
ep.panel(620, { cam: { on: ['mcgonagall'], fr: 'close' }, bg: DA({ start: 3 }), blur: 3, actors: [MCG({ x: 900, y: 1080, turn: -0.2, expr: { base: 'teary', mouth: { type: 'line', curve: 0.3 } } })] },
...
ep.bleed(1300, { cam: { x: 1000, y: 230, w: 1500 }, bg: () => L.gringotts(),
  actors: [{ def: mcgonagall, id: 'mcgonagall', x: 880, y: 1090, s: 1, turn: 0.1, pose: 'stand', expr: 'calm' }, HAR({ x: 1060, y: 1100, turn: -0.1, pose: 'stand', expr: 'awe' })] },
