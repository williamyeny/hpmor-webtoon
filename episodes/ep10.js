...
ep.panel(1000, { cam: { on: ['hermione'], fr: 'waist', zoom: 0.75, dy: -0.3 }, bg: CP(), actors: [HER({ pose: 'sitRead', expr: 'smile', armF: BOOK })] },
  [cap('But let it be quite clear that Hermione Granger—sitting alone, on the first day of school, in the last carriage of the train, with the door left open just in case anyone wanted to talk to her—', 40, 30, { w: 640, size: 26, fixed: true }),
   cap('—was *not* sad, lonely, gloomy, depressed, or despairing. She was rereading *Hogwarts: A History* for the third time, and quite enjoying it.', 40, 820, { w: 640, size: 26, fixed: true })], { mood: 'day' });
...
ep.panel(760, { cam: { x: 930, y: 730, w: 660 }, bg: CORR, actors: [{ def: harryRobes, id: 'harry', x: 1000, y: 1000, s: 1.1, turn: 0.6, pose: 'gesture', expr: 'neutral', mask: 'scarf' }, (e) => L.trunk(820, 1010, 0.32, '#7a4e2e', true)] },
  [say('Harry', 'Excuse me—does anyone here know the six quarks, or where I can find a first-year girl named Hermione Granger?', 420, 120, { w: 460, fixed: true })], { mood: 'day', alt: 'In the corridor, a boy in robes with a scarf wrapped round his face knocks on a compartment door, his trunk scuttling behind him.' });
...
ep.panel(820, { cam: { x: 875, y: 690, w: 760 }, bg: CORR, actors: [{ def: hermione, id: 'hermione', x: 700, y: 1000, s: 1.1, turn: 0.6, pose: 'stand', expr: 'neutral', lean: 12 }, { def: harryRobes, id: 'harry', x: 1050, y: 1000, s: 1.1, turn: -0.5, pose: 'stand', expr: 'neutral', mask: 'scarf' }] },
  [say('Hermione', 'Can I help you with something?', 185, 110, { w: 240, fixed: true }),
   say('Harry', 'Not unless you can name the six quarks, or tell me where to find Hermione Granger.', 590, 180, { w: 270, fixed: true })], { mood: 'day' });
...
ep.panel(900, { cam: { on: ['harry'], fr: 'bust', zoom: 0.85, dy: -0.1 }, bg: CORR, blur: 2, actors: [{ def: harryRobes, id: 'harry', x: 1050, y: 1000, s: 1.1, turn: -0.3, pose: 'present', expr: 'bigGrin', mask: 'scarf' }] },
  [say('Harry', 'Ah, so *you\'re* a first-year girl named Hermione Granger. On the train to Hogwarts, no less.', 400, 110, { w: 480, fixed: true }),
   say('Harry', 'Presumably I\'m meant to invite you to join my party, or get a key magical item from you. PC or NPC, that is the question?', 400, 770, { w: 540, fixed: true })], { mood: 'day' });
...
ep.panel(860, { cam: { x: 900, y: 700, w: 1060 }, bg: CP(), mid: TRUNKS, actors: [HER({ pose: 'stand', y: 900, seat: undefined, x: 560, expr: 'confused', turn: 0.5 }), HAR({ expr: 'grin', mask: 'scarf' })] },
  [cap('He sat down across from her. His trunk scurried in after him, grew to three times its size, and snuggled up next to hers in an oddly disturbing fashion.', 40, 30, { w: 660, size: 26, fixed: true }),
   say('Harry', 'Please, have a seat. And close the door, if you would. Don\'t worry—I don\'t bite anyone who doesn\'t bite me first.', 480, 715, { w: 440, size: 27, fixed: true, tail: [640, 470] })], { mood: 'day' });
...
ep.panel(960, { cam: { on: ['harry'], fr: 'bust', zoom: 0.85, dy: -0.05 }, bg: CP(), blur: 2, actors: [HAR({ expr: 'grin', mask: 'scarfDown' })] },
  [say('Harry', '*I* didn\'t say you *said* you were Hermione Granger. I just said you *were.* If you\'re asking how I know, it\'s because I know everything.', 400, 115, { w: 540, size: 28, fixed: true }),
   say('Harry', 'Good evening, ladies and gentlemen. My name is Harry James Potter-Evans-Verres, or Harry Potter for short. I know that probably doesn\'t mean anything to *you*, for a change—', 400, 820, { w: 580, size: 27, fixed: true })], { mood: 'day' });
...
ep.panel(980, { cam: { on: ['hermione'], fr: 'bust', zoom: 0.9, dy: -0.4 }, bg: CP(), blur: 2, actors: [HER({ pose: 'present', expr: 'shock' })] },
  [shout('Hermione', '*Harry Potter!* You\'re in *Modern Magical History* and *The Rise and Fall of the Dark Arts* and *Great Wizarding Events of the Twentieth Century!*', 400, 190, { w: 420, size: 27, fixed: true }),
...
ep.panel(620, { cam: { on: ['harry'], fr: 'close', zoom: 0.85, dy: 0.15 }, bg: CP(), blur: 3, actors: [HAR({ expr: 'blank' })] }, [say('Harry', 'I\'m in *books?* Wait—of course I\'m in books… What a strange thought.', 400, 110, { w: 520, fixed: true })], { mood: 'day' });
...
ep.panel(860, { cam: { on: ['harry'], fr: 'bust', zoom: 0.85, dy: -0.1 }, bg: CP(), blur: 2, actors: [HAR({ expr: 'deadpan' })] },
  [say('Harry', 'Miss Granger, it has been less than seventy-two hours since I discovered my claim to fame. *Believe me*, I intend to find out everything I can.', 400, 125, { w: 540, fixed: true }),
   say('Harry', 'What *do* the books say about me?', 470, 760, { w: 460, fixed: true })], { mood: 'day' });
ep.panel(820, { cam: { on: ['hermione'], fr: 'waist', zoom: 0.9, dy: -0.75 }, bg: CP(), actors: [HER({ expr: { base: 'focus', eyes: { lookY: -0.8, lookX: 0.4 } }, pose: 'lecture' })], under: (e) => rect(0, 0, e.w, e.h, { fill: '#f6e3b0', opacity: 0.2 }) },
  [say('Hermione', 'You\'re the only one who\'s ever survived the Killing Curse, so you\'re called the Boy-Who-Lived. Born the thirty-first of July, 1980. On the thirty-first of October 1981, He-Who-Must-Not-Be-Named—though I don\'t know why not—attacked your home.', 400, 175, { w: 560, size: 26, fixed: true })], { mood: 'day', alt: 'Hermione recites, rapid-fire, eyes rolled up as if reading from memory.' });
ep.panel(820, { cam: { on: ['hermione'], fr: 'close', zoom: 0.8, dy: 0.4 }, bg: CP(), blur: 2, actors: [HER({ expr: { base: 'focus', eyes: { lookY: -0.8, lookX: -0.4 } }, pose: 'lecture' })], under: (e) => rect(0, 0, e.w, e.h, { fill: '#f6e3b0', opacity: 0.2 }) },
  [say('Hermione', '*The Rise and Fall of the Dark Arts* claims you survived because of your mother\'s love, that your scar contains all of the Dark Lord\'s magical power, and that the centaurs fear you. But *Modern Magical History* warns that there are lots of crackpot theories about you.', 400, 630, { w: 620, size: 26, fixed: true })], { mood: 'day' });
...
ep.panel(880, { cam: { on: ['hermione'], fr: 'bust', zoom: 0.85, dy: -0.55 }, bg: CP(), blur: 2, actors: [HER({ expr: 'smug' })] },
  [say('Hermione', 'Wait until the potion turns exactly the shade of the cloudless dawn sky, eight degrees from the horizon, eight minutes before sunrise. Stir eight times widdershins and once deasil, and then add eight drams of unicorn bogies.', 400, 175, { w: 530, size: 26, fixed: true })], { mood: 'day' });
...
ep.panel(820, { cam: { on: ['hermione'], fr: 'bust', zoom: 0.85, dy: -0.1 }, bg: CP(), blur: 2, actors: [HER({ expr: 'smile', pose: 'holdOne', armF: { sh: 60, el: 95, hand: 'hold', prop: g({ transform: 'translate(0,8)' }, comedCan(1)) } })] },
  [cap('Hermione politely accepted the fizzy drink. As she started to drink, the boy said:', 40, 28, { w: 640, size: 26, fixed: true }),
   say('Harry', 'I\'d like you to help me take over the universe.', 540, 715, { w: 380, tail: [790, 745], fixed: true })], { mood: 'day' });
...
ep.panel(860, { cam: { on: ['harry'], fr: 'waist', zoom: 0.8, dy: -0.9 }, bg: CP(), actors: [HAR({ expr: 'delight', pose: 'armsUp' })], under: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.4, { col: '#f0c878', op: 0.4 }) },
  [say('Harry', 'I meant it in the sense of the Baconian project—"the effecting of all things possible." Experimental studies of spells. The underlying laws. Bring magic into science, merge the wizarding and Muggle worlds, raise the whole planet\'s standard of living—', 400, 175, { w: 560, size: 26, fixed: true })], { mood: 'day' });
ep.panel(800, { cam: { on: ['harry'], fr: 'close', zoom: 0.8, dy: 0.6 }, bg: CP(), blur: 3, actors: [HAR({ expr: { base: 'delight', glint: true }, pose: 'armsUp' })] },
  [say('Harry', '—discover the secret of immortality, colonise the Solar System, explore the galaxy, and most importantly, *figure out what the heck is really going on here*, because all of this is blatantly impossible.', 400, 605, { w: 560, size: 26, fixed: true })], { mood: 'day' });
...
ep.panel(720, { cam: { on: ['harry'], fr: 'bust', zoom: 0.85, dy: -0.25 }, bg: CP(), blur: 2, actors: [HAR({ expr: 'warm', pose: 'gesture' })] },
  [say('Harry', 'With your encyclopaedic memory added to my intelligence and rationality, we\'ll have it finished in no time. Where by "no time" I mean at least thirty-five years.', 400, 135, { w: 540, size: 27, fixed: true })], { mood: 'day' });
...
ep.panel(820, { cam: { on: ['harry'], fr: 'close', zoom: 0.82, dy: 0.05 }, bg: CP(), blur: 3, actors: [HAR({ expr: 'scheme' })] },
  [say('Harry', 'Well, maybe *this* will impress you. I\'ve found out I don\'t need the wand. I can make anything I want happen, just by snapping my fingers.', 400, 128, { w: 540, fixed: true })], { mood: 'day' });
...
ep.panel(860, { cam: { on: ['hermione'], fr: 'bust', zoom: 0.85, dy: -0.1 }, bg: CP(), blur: 2, actors: [HER({ expr: 'horror', pose: 'cower', seat: undefined, y: 900, x: 560 })], over: (e) => FX.frost(e.w, e.h, 0.25, 30) },
  [inner('Hermione', '*Wordless, wandless magic! At HIS age? When he only got his schoolbooks three days ago?*', 400, 85, { w: 560, fixed: true }),
   inner('Hermione', '*ALL THE DARK LORD\'S MAGICAL POWER! IN HIS SCAR!*', 400, 790, { w: 500, fixed: true })], { mood: 'day' });
...
ep.panel(920, { cam: { on: ['harry'], fr: 'bust', zoom: 0.85, dy: -0.08 }, bg: CP(), blur: 2, actors: [HAR({ expr: 'focus', pose: 'gesture' })] },
  [say('Harry', 'I did *warn* you that challenging my ingenuity makes your life surreal. Remember that, the next time I warn you about something.', 400, 120, { w: 540, fixed: true }),
...
ep.panel(960, { cam: { on: ['hermione'], fr: 'bust', zoom: 0.85, dy: -0.05 }, bg: CP(), blur: 2, actors: [HER({ expr: 'think', pose: 'think' })] },
  [say('Hermione', 'My hypothesis is that the robes come from the shop with a Charm on them to keep them clean. You found that out by spilling something on *yourself* earlier.', 400, 130, { w: 560, fixed: true }),
   say('Hermione', 'And now I do Step Two: an experiment. I\'ll pour some on *your* robes, and I predict the stain will disappear.', 400, 830, { w: 540, fixed: true })], { mood: 'day' });
...
ep.panel(840, { cam: { on: ['hermione'], fr: 'close', zoom: 0.8, dy: 0.05 }, bg: CP(), blur: 3, actors: [HER({ expr: 'hurt' })] },
  [cap('A stab of disappointment. He wasn\'t a teacher—but it was still a test, and getting a question wrong always felt like a little punch in the stomach.', 40, 28, { w: 640, size: 26, fixed: true }),
   cap('(It said almost everything you needed to know about Hermione Granger that she had never once let that stop her.)', 40, 690, { w: 570, size: 25, fixed: true })], { mood: 'day' });

// the 2-4-6 game
ep.panel(1000, { cam: { on: ['harry'], fr: 'bust', zoom: 0.8, dy: -0.15 }, bg: CP(), blur: 2, actors: [HAR({ expr: 'focus', pose: 'present', armB: { sh: 62, el: 30, hand: 'palm', prop: g({ transform: 'translate(0,-10)' }, foldedNote(1.2)) } })] },
  [say('Harry', 'This is a game based on a famous experiment. I have a *rule*—which I\'ve written down and folded up, so you know it\'s fixed—that fits some triplets of numbers, but not others.', 400, 130, { w: 560, size: 27, fixed: true }),
   say('Harry', '2-4-6 fits the rule. You give me triplets, and I\'ll say "Yes" or "No". *I am Nature, the rule is one of my laws, and you are investigating me.*', 400, 860, { w: 560, size: 27, fixed: true })], { mood: 'day' });
...
ep.panel(600, { cam: { on: ['hermione'], fr: 'close', zoom: 0.85, dy: 0.12 }, bg: CP(), blur: 3, actors: [HER({ expr: 'smug' })] }, [say('Hermione', 'The rule is that the numbers have to go up by two each time.', 400, 95, { w: 560, fixed: true })], { mood: 'day' });
ep.panel(660, { cam: { on: ['harry'], fr: 'close', zoom: 0.85, dy: 0.15 }, bg: CP(), blur: 3, actors: [HAR({ expr: 'scheme' })] }, [say('Harry', 'Now suppose I tell you this test is harder than it looks, and only twenty percent of grown-ups get it right.', 400, 110, { w: 540, fixed: true })], { mood: 'day' });
...
ep.panel(800, { cam: { on: ['harry'], fr: 'waist', zoom: 0.9, dy: -0.45 }, bg: CP(), actors: [HAR({ expr: 'warm', pose: 'lecture' })] },
  [say('Harry', 'What you\'ve just discovered is called *positive bias.* You had a rule in your mind, and you kept thinking of triplets that should make it say "Yes". You never tested any that should make it say "No".', 400, 150, { w: 620, size: 27, fixed: true })], { mood: 'day' });
...
ep.panel(820, { cam: { on: ['harry'], fr: 'waist', zoom: 0.72, dy: -0.3 }, bg: CP(), actors: [HAR({ expr: 'bigGrin', pose: 'bowGrand', seat: undefined, y: 900, x: 1150 })] },
  [cap('The boy stood up and bowed to her, solemnly. He was grinning widely now.', 40, 28, { w: 640, fixed: true }),
   say('Harry', 'Then… may I help you with your research, Hermione Granger?', 400, 715, { w: 560, fixed: true })], { mood: 'day' });
...
ep.panel(720, { cam: { on: ['hermione'], fr: 'bust', zoom: 0.85, dy: -0.25 }, bg: CP(), blur: 2, actors: [HER({ pose: 'handsHips', seat: undefined, y: 900, x: 540, turn: -0.4, expr: 'determined' })] },
  [say('Hermione', 'Then we\'ll just have to check all the other carriages! I\'ll help you. My name is Hermione Granger, by the way.', 400, 120, { w: 540, fixed: true })], { mood: 'day' });
...
ep.panel(800, { cam: { on: ['hermione'], fr: 'bust', zoom: 0.85, dy: -0.15 }, bg: CP(), blur: 2, actors: [HER({ expr: 'confused', pose: 'slump' })] },
  [say('Hermione', 'Can you *please* explain to me what\'s going on?', 400, 105, { w: 480, fixed: true }),
   cap('She wondered if hanging around Harry Potter meant always being this confused.', 40, 695, { w: 640, fixed: true })], { mood: 'day' });
ep.panel(880, { cam: { on: ['harry'], fr: 'bust', zoom: 0.85, dy: -0.45 }, bg: CP(), blur: 2, actors: [HAR({ expr: 'grin', pose: 'gesture' })] },
  [say('Harry', 'Oh—we saw him on the platform, looking terrified. The fear is often worse than the thing itself, so we let him see his worst nightmare come true—and it wasn\'t so bad. We gave him sweets. That\'s *desensitisation therapy.*', 400, 180, { w: 560, size: 27, fixed: true })], { mood: 'day' });
ep.panel(760, { cam: { on: ['hermione'], fr: 'bust' }, bg: CP(), blur: 2, actors: [HER({ expr: 'rant', pose: 'fists' })], under: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.45, { col: '#e8b4a0', op: 0.5 }) },
  [shout('Hermione', 'That\'s *awful!* *You\'re* awful! That poor boy! What you did was *mean!*', 400, 130, { w: 440, size: 32, fixed: true })], { mood: 'day' });
ep.panel(880, { cam: { on: ['harry'], fr: 'bust', zoom: 0.85, dy: -0.4 }, bg: CP(), blur: 2, actors: [HAR({ expr: 'unimpressed', pose: 'raiseHand' })] },
  [say('Harry', 'I think the word you\'re looking for is *enjoyable.* And you\'re asking the wrong question. The question is: did it do more good than harm? It\'s called *consequentialism.* Whether an act is right isn\'t about whether it *looks* bad.', 400, 170, { w: 560, size: 27, fixed: true })], { mood: 'day' });
ep.panel(760, { cam: { on: ['hermione'], fr: 'close', zoom: 0.8, dy: -0.05 }, bg: CP(), blur: 3, actors: [HER({ expr: 'cross' })] }, [cap('Hermione opened her mouth to say something utterly *searing*—but seemed to have neglected the part where she thought of it first.', 40, 28, { w: 640, size: 26, fixed: true }), say('Hermione', 'What if he has *nightmares?*', 400, 670, { w: 480, fixed: true })], { mood: 'day' });
ep.panel(680, { cam: { on: ['harry'], fr: 'close', zoom: 0.85, dy: 0.2 }, bg: CP(), blur: 3, actors: [HAR({ expr: 'smug' })] }, [say('Harry', 'Then they\'ll be nightmares about horrible monsters who give you chocolate. Which was sort of the whole *point.*', 400, 120, { w: 560, fixed: true })], { mood: 'day' });
ep.panel(520, { cam: { on: ['hermione'], fr: 'close', zoom: 0.9, dx: -0.5, dy: 0.1 }, bg: CP(), blur: 3, actors: [HER({ expr: 'exasperated' })] },
  [say('Hermione', 'Is your life always this peculiar?', 195, 110, { w: 300, fixed: true })], { mood: 'day' });
...
// quiz montage
ep.multi(920, [
  { x: M, y: 18, w: 368, h: 884, mood: 'day', art: { cam: { on: ['harry'], fr: 'close', zoom: 1.25, dy: -1.3 }, bg: CP(), blur: 3, actors: [HAR({ expr: 'focus' })] } },
  { x: 408, y: 18, w: 368, h: 884, mood: 'day', art: { cam: { on: ['hermione'], fr: 'close', zoom: 1.25, dy: 0.0 }, bg: CP(), blur: 3, actors: [HER({ expr: 'determined' })] } },
], [say('Harry', 'I can do calculus, and I know Bayesian probability theory, and decision theory, and I\'ve read *The Feynman Lectures*—', 208, 240, { w: 290, size: 24, fixed: true, tail: 'harry' }), say('Hermione', '—name the twelve uses of dragon\'s blood—', 592, 730, { w: 270, size: 24, fixed: true, tail: 'hermione' })],
...
ep.panel(840, { cam: { on: ['harry', 'neville'], fr: 'waist', dy: -0.8 }, bg: CP(), actors: [NEV({ x: 700, turn: 0.4, expr: 'shock', pose: 'stand' }), HAR({ expr: 'determined', seat: undefined, y: 900, x: 900, pose: 'holdOne', turn: -0.4 })] },
  [say('Harry', 'Come on, Neville. *I\'ll* come with you this time. We\'ll see if the Boy-Who-Lived gets more attention.', 400, 125, { w: 560, fixed: true, tail: 'harry' }),
