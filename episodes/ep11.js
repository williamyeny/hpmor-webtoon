...
ep.panel(820, { cam: { x: 720, y: 780, w: 1000 }, bg: LAKE({ boats: [] }), actors: [...BOATHAR], fg: boatFg },
  [cap('Neville had his toad back. (A Ravenclaw prefect had jabbed her wand at him without even looking up from her book, and he\'d wandered off in a daze, straight to it.)', 44, 34, { w: 480, size: 25 }),
   say('Hermione', 'It\'s bewitched so it can\'t be reached except by the lake! I read it in *Hogwarts: A History!*', 560, 620, { w: 360, size: 27 })], { mood: 'night', alt: 'In a small boat: Neville, dazed, clutching his toad; Harry, awestruck; Hermione, lecturing happily.' });
...
ep.panel(760, { cam: { x: 850, y: 700, w: 1300 }, bg: () => HG.hallTable('g'), actors: FY({}) },
  [say('Hermione', 'It\'s bewitched to look like the sky outside!', 520, 100, { w: 320 }), inner('Harry', '*Of course it is.*', 200, 620, { w: 280 })], { mood: 'candle' });
// the Hat's song
const DAIS = () => HG.dais();
const HAT = (mood = 'talk', x = 1000) => (e) => HG.stool(x, 900) + g({ transform: `translate(${x},${770})` }, HG.sortingHat(0.9, { mood }));
ep.panel(820, { cam: { x: 1000, y: 620, w: 900 }, bg: DAIS, actors: [HAT('amused')], over: (e) => FX.sparkles([[e.w * 0.2, e.h * 0.2, 14], [e.w * 0.8, e.h * 0.25, 12]], { col: '#fff3b0' }) },
  [cap('On a stool on the dais sat a patched and frayed old wizard\'s hat. Then a rip near its brim opened wide like a mouth—and it began to sing.', 44, 34, { w: 460, size: 26 }),
   hat('♪ *Oh, I\'m the Sorting Hat and I\'m okay—I sleep all year and I work one day…* ♪', 400, 640, { w: 480 })], { mood: 'candle', alt: 'The Sorting Hat on its stool, singing through the rip in its brim.' });
ep.panel(560, { cam: { on: ['harry'], fr: 'close' }, bg: () => HG.hallTable('g'), blur: 3, actors: FY({}) },
  [inner('Harry', 'Was the Hat *conscious?* In the sense of being aware of its own awareness? And if so, was it satisfied with only getting to talk to eleven-year-olds once a year?', 400, 110, { w: 560, size: 27 })], { mood: 'candle' });
...
ep.panel(700, { cam: { x: 800, y: 560, w: 1000 }, bg: DAIS, actors: [MCG(), HAT('sleep')] },
  [shout('McGonagall', '"Abbott, Hannah!"', 250, 100, { w: 260, size: 30 })], { mood: 'candle' });
const verdict = (house, col, txt) => (ctx) => rect(0, 0, ctx.w, ctx.h, { fill: col }) + FX.burst(ctx.w, ctx.h, ctx.w / 2, ctx.h / 2, { col: '#fff', op: 0.18 }) + FX.sfxText(ctx.w / 2, ctx.h * 0.62, txt, { size: Math.min(64, ctx.w / (txt.length * 0.62)), fill: '#f6e7c4', font: 'IM Fell English SC', weight: 400, sw: 5 });
ep.multi(440, [
  { x: M, y: 18, w: 240, h: 404, art: verdict('h', '#8a6a1a', 'HUFFLEPUFF!'), border: 'ink' },
  { x: 280, y: 18, w: 240, h: 404, art: verdict('h', '#8a6a1a', 'HUFFLEPUFF!') },
  { x: 536, y: 18, w: 240, h: 404, art: verdict('r', '#243a6a', 'RAVENCLAW!') },
], [note('Abbott, Hannah', 145, 60, { size: 26, color: '#f6e7c4' }), note('Bones, Susan', 400, 60, { size: 26, color: '#f6e7c4' }), note('Boot, Terry', 655, 60, { size: 26, color: '#f6e7c4' })], { alt: 'Abbott, Hannah — HUFFLEPUFF! Bones, Susan — HUFFLEPUFF! Boot, Terry — RAVENCLAW!' });
...
const staff = (o = {}) => [
  ...
  { def: snape, id: 'snape', x: 560, y: 900, turn: 0.2, pose: 'holdOne', expr: 'menace', armF: { sh: 40, el: 90, hand: 'hold', prop: HG.goblet(0, -10, 1, o.crushed || 0) } },
...
ep.bleed(1000, { cam: { x: 880, y: 470, w: 1900 }, bg: STAFF, actors: [...staff(), () => HG.staffTable()] },
  [cap('At the High Table: a wizened ancient with a silver-white beard, in a great golden chair, applauding every student as if freshly delighted by each. To his left, a man with sharp eyes and a dour face, who applauded no-one—and who somehow looked straight back at Harry every time Harry looked at him.', 44, 34, { w: 520, size: 25 }),
   cap('And further along, the pale man from the Leaky Cauldron—whose eyes darted around in panic, and who twitched in his seat. For some reason, Harry kept finding himself staring at him.', 280, 800, { w: 480, size: 25 })], { mood: 'candle', alt: 'The staff at the High Table: Trelawney, Sprout, Snape with a silver goblet, Dumbledore, Quirrell twitching, tiny Flitwick standing on his chair, and the enormous Hagrid.' });
ep.panel(560, { cam: { on: ['quirrell'], fr: 'close' }, bg: STAFF, blur: 2, actors: [...staff(), () => HG.staffTable()], over: (e) => FX.doom(e.w, e.h, 8) },
  [note('mmmmmmmm', 560, 460, { size: 32, color: '#c07a8a' })], { mood: 'candle', alt: 'Quirrell, twitching. The same wrong hum as before.' });
...
ep.panel(760, { cam: { on: ['harry', 'clearwater'], fr: 'bust' }, bg: () => HG.hallTable('r'), actors: [...FY({ he: { expr: { base: 'focus', eyes: { lookX: 1 } } } }).slice(2, 4), RP()] },
  [whisper('Harry', 'Is the man standing on his chair the Head of Ravenclaw?', 250, 100, { w: 300 }),
   whisper('Clearwater', 'That is Professor Filius Flitwick—the most knowledgeable Charms Master alive, and a past Duelling Champion. He does have goblin ancestry—', 560, 560, { w: 360, size: 25 })], { mood: 'candle' });
...
ep.panel(560, { cam: { on: ['hermione'], fr: 'close' }, bg: () => HG.hallTable('r'), blur: 3, actors: FY({ he: { expr: { base: 'focus', eyes: { lookX: 1, lookY: -0.3 } } } }).slice(3, 4) },
  [whisper('Hermione', 'Lithuania.', 400, 100, { w: 180 }), cap('…Hermione whispered absently, her eyes still fixed firmly on the Sorting Hat. Now *she* was getting a smile from the prefect.', 44, 380, { w: 460, size: 25 })], { mood: 'candle' });
ep.panel(420, { cam: { on: ['harry'], fr: 'close' }, bg: () => HG.hallTable('r'), blur: 3, actors: FY({ h: { expr: 'deadpan' } }).slice(2, 3) }, [whisper('Harry', 'Never mind.', 400, 100, { w: 180 })], { mood: 'candle' });

// Hermione
ep.panel(760, { cam: { on: ['hermione'], fr: 'full' }, bg: () => HG.hallTable('r'), blur: 1, actors: FY({ he: { pose: 'tiptoe', expr: 'delight', y: 1060 } }).slice(3, 4) },
  [cap('Hermione was bouncing on her tiptoes so hard her feet were actually leaving the ground.', 44, 34, { w: 400 }), shout('McGonagall', '"Granger, Hermione!"', 560, 620, { w: 300, size: 30, tail: [790, 520] })], { mood: 'candle' });
ep.panel(820, { cam: { x: 1000, y: 620, w: 900 }, bg: DAIS, actors: [() => HG.stool(1000, 900), { def: hermione, id: 'hermione', x: 1000, y: 900, s: 1.1, turn: 0.1, pose: 'sit', seat: 125, expr: 'determined' }, HG.hatOn('hermione', 'amused')], over: (e) => FX.speedLines(e.w, e.h, { n: 20, angle: 90 }) },
...
ep.panel(700, { cam: { on: ['harry'], fr: 'bust' }, bg: () => HG.hallTable('r'), blur: 2, actors: FY({ h: { expr: 'unimpressed', pose: 'crossArms' } }).slice(2, 3) },
  [inner('Harry', 'Talk about your foregone conclusions. In what weird alternative universe would that girl *not* be sorted into Ravenclaw?', 400, 110, { w: 540 }),
   inner('Harry', 'Harry knew pi to 3.141592, because that\'s accurate enough for most purposes. Hermione knew a hundred digits, because that\'s how many were printed in the back of her maths book.', 400, 580, { w: 560, size: 26 })], { mood: 'candle' });
ep.multi(460, [
...
ep.panel(760, { cam: { on: ['harry'], fr: 'bust' }, bg: () => HG.hallTable('r'), blur: 2, actors: FY({ h: { expr: 'smile' } }).slice(2, 3) },
  [inner('Harry', 'Neville in Hufflepuff. Good. A Houseful of reliable friends would do him a whole world of good.', 400, 110, { w: 540 }),
...
ep.panel(760, { cam: { on: ['mcgonagall'], fr: 'bust' }, bg: DAIS, blur: 1, actors: [MCG({ expr: { base: 'stern', eyes: { open: 0.95 } } })] },
  [cap('With a note of trepidation so firmly kept from her voice and face that you\'d have needed to know her very well indeed to notice, Minerva McGonagall inhaled deeply—', 44, 34, { w: 460, size: 26 }),
   shout('McGonagall', '"Potter, Harry!"', 400, 640, { w: 280, size: 34 })], { mood: 'candle' });
ep.bleed(900, { cam: { x: 800, y: 700, w: 1400 }, bg: () => HG.hallTable('s'), actors: [...[1211, 1212, 1213, 1214, 1215, 1216, 1217].map((sd, i) => ({ def: student(sd, ['g', 'r', 'h', 's'][i % 4]), id: 't' + i, x: 150 + i * 220, y: 1150, s: 1.05, turn: i < 3 ? 0.4 : -0.4, expr: { base: 'awe', eyes: { sparkle: false, lookX: i < 3 ? 1 : -1 } } })), () => HG.tableFront()] },
...
ep.bleed(1000, { cam: { x: 1100, y: 640, w: 1300 }, bg: () => HG.hallTable('g'), actors: [{ def: fred, id: 'fred', x: 900, y: 1150, turn: 0.3, pose: 'armsUp', expr: 'laugh' }, { def: george, id: 'george', x: 1150, y: 1150, turn: -0.3, pose: 'armsUp', expr: 'laugh' }, ...[1221, 1222, 1223].map((sd, i) => ({ def: student(sd, 'g'), x: 500 + i * 450 + (i > 0 ? 300 : 0), y: 1160, s: 1.05, turn: 0.2, pose: 'raiseHand', expr: 'bigGrin' })), () => HG.tableFront()] },
  [shout('Fred', '*HARRY POTTER!*', 250, 110, { w: 300, size: 36 }), shout('George', '*HARRY POTTER!*', 560, 260, { w: 300, size: 36 })], { mood: 'candle', alt: 'At the Gryffindor table, the Weasley twins leap up and start chanting.' });
ep.bleed(1200, { cam: { x: 800, y: 440, w: 1500 }, bg: () => HG.greatHallWide() },
  [shout('hall', 'HARRY POTTER! HARRY POTTER! HARRY POTTER!', 400, 600, { w: 600, size: 52, noTail: true })], { mood: 'candle', fadeTop: false, alt: 'The whole Hall chants his name.' });
ep.panel(900, { cam: { on: ['harry'], fr: 'full' }, bg: () => HG.greatHallWide(), blur: 2, actors: [{ def: harryRobes, id: 'harry', x: 800, y: 1150, s: 1.3, turn: 0.1, pose: 'walk', expr: 'grin' }] },
  [cap('And Harry Potter walked forwards—much too slowly, he realised once he\'d begun, but by then it was too late to change pace without looking awkward. So he put on a wide grin, bowed to one side and then the other, and walked on at a grandly measured pace, like a prince inheriting his castle.', 44, 34, { w: 520, size: 25 })], { mood: 'candle' });
...
ep.multi(900, [
  ...
  { x: M, y: 606, w: 752, h: 276, mood: 'candle', art: { cam: { on: ['snape'], fr: 'bust' }, bg: STAFF, blur: 2, actors: [...staff(), () => HG.staffTable()] } },
], [note('fanning herself', 145, 270, { size: 24, color: '#f6e7c4' }), note('clapping along', 400, 270, { size: 24, color: '#f6e7c4' }), note('…vacant', 655, 270, { size: 24, color: '#f6e7c4' }), note('beaming', 200, 570, { size: 26, color: '#f6e7c4' }), note('curious', 590, 570, { size: 26, color: '#f6e7c4' }),
    cap('And Severus Snape, gripping his wine goblet so hard that the silver was slowly deforming.', 44, 630, { w: 400, size: 24 })], { alt: 'Head table reactions: Trelawney fanning herself, Hagrid clapping along, Quirrell gazing at nothing, Dumbledore beaming, Flitwick curious — and Snape gripping his silver goblet until it bends.' });
ep.panel(760, { cam: { x: 1100, y: 700, w: 1100 }, bg: () => HG.hallTable('g'), actors: [{ def: fred, id: 'fred', x: 950, y: 1150, turn: 0.3, pose: 'point', expr: 'bigGrin' }, { def: george, id: 'george', x: 1200, y: 1150, turn: -0.3, pose: 'raiseHand', expr: 'laugh' }, () => HG.tableFront()] },
  [shout('Fred', 'Save us from some more Dark Lords!', 250, 100, { w: 300, size: 28 }), shout('George', 'Especially if they\'re *Professors!*', 560, 250, { w: 300, size: 28 })], { mood: 'candle' });
ep.panel(760, { cam: { on: ['snape'], fr: 'close' }, bg: STAFF, blur: 3, actors: [...staff({ crushed: 1 }).map((a) => a.id === 'snape' ? { ...a, expr: { base: 'smile', eyes: { open: 0.6 } } } : a)] },
  [cap('Severus Snape\'s face had gone beyond rage into a kind of pleasant indifference. A faint smile played about his lips.', 44, 34, { w: 440 }),
...
ep.panel(760, { cam: { x: 1000, y: 620, w: 1000 }, bg: DAIS, actors: [MCG({ x: 560 }), HAT('sleep'), { def: harryRobes, id: 'harry', x: 1250, y: 900, s: 1.1, turn: -0.2, pose: 'bowGrand', expr: 'bigGrin' }] },
...
ep.panel(820, { cam: { x: 1000, y: 470, w: 760 }, bg: DAIS, actors: [() => HG.stool(1000, 900), { def: harryRobes, id: 'harry', x: 1000, y: 900, s: 1.1, turn: 0.1, pose: 'sit', seat: 125, expr: 'determined' }, HG.hatOn('harry', 'sleep')] },
...
ep.bleed(1000, (ctx) => HG.mindscape(ctx.w, ctx.h, 'warm', 3),
  [inner('Harry', '*Don\'t Sort me yet! I have questions! Have I ever been Obliviated? Did you Sort the Dark Lord when he was a child—can you tell me his weaknesses? Why did I get the brother wand to his?*', 400, 150, { w: 580, size: 27 }),
   inner('Harry', '*Is the Dark Lord\'s ghost bound to my scar, and is that why I get so angry sometimes? And if you\'ve got a moment—how do I rediscover the lost magics that created you?*', 400, 520, { w: 580, size: 27 })], { alt: 'Inside Harry\'s head: an endless candlelit library, and his questions tumbling out.' });
...
ep.bleed(1000, (ctx) => HG.mindscape(ctx.w, ctx.h, 'void', 5) + g({ transform: `translate(${ctx.w / 2},${ctx.h * 0.95}) scale(1.95)` }, HG.sortingHat(1, { mood: 'worried' })),
