...
ep.panel(760, { cam: { x: 880, y: 760, w: 820 }, bg: () => HG.hallTable('g'), actors: FY({}) },
  [say('Hermione', 'It\'s bewitched to look like the sky outside!', 520, 100, { w: 320 }), inner('Harry', '*Of course it is.*', 150, 690, { w: 280 })], { mood: 'candle' });
// the Hat's song
const DAIS = () => HG.dais();
...
ep.panel(760, { cam: { on: ['quirrell'], fr: 'close', dy: -0.35 }, bg: STAFF, blur: 2, actors: [...staff(), () => HG.staffTable()], over: (e) => FX.doom(e.w, e.h, 8) },
  [cap('And further along, the pale man from the Leaky Cauldron—whose eyes darted around in panic, and who twitched in his seat. For some reason, Harry kept finding himself staring at him.', 44, 34, { w: 480, size: 25 }),
...
ep.panel(760, { cam: { on: ['harry', 'clearwater'], fr: 'bust' }, bg: () => HG.hallTable('r'), actors: [...FY({ he: { expr: { base: 'focus', eyes: { lookX: 1 } } } }).slice(2, 4), RP()] },
  [whisper('Harry', 'Is the man standing on his chair the Head of Ravenclaw?', 250, 100, { w: 300 }),
   whisper('Clearwater', 'That is Professor Filius Flitwick—the most knowledgeable Charms Master alive, and a past Duelling Champion. He does have goblin ancestry—', 470, 600, { w: 510, size: 25 })], { mood: 'candle' });
...
ep.panel(760, { cam: { on: ['mcgonagall'], fr: 'bust' }, bg: DAIS, blur: 1, actors: [MCG({ expr: { base: 'stern', eyes: { open: 0.95 } } })] },
  [cap('With a note of trepidation so firmly kept from her voice and face that you\'d have needed to know her very well indeed to notice, Minerva McGonagall inhaled deeply—', 44, 34, { w: 560, size: 26 }),
...
ep.panel(1000, { cam: { on: ['snape'], fr: 'bust', dy: -0.1, zoom: 0.9 }, bg: STAFF, blur: 3, actors: [...staff({ crushed: 1 }).map((a) => a.id === 'snape' ? { ...a, expr: { base: 'smile', eyes: { open: 0.6 } }, pose: 'hold', armF: { sh: 22, el: 80, hand: 'fist' }, armB: { sh: 28, el: 75, hand: 'fist' } } : a)] },
...
ep.panel(1100, { cam: { on: ['harry'], fr: 'bust' }, bg: () => HG.greatHallWide(), blur: 3, actors: [{ def: harryRobes, id: 'harry', x: 800, y: 1150, s: 1.3, turn: 0.1, pose: 'walk', expr: { base: 'smile', eyes: { teary: true } } }] },
  [inner('Harry', 'They were cheering him for a job he\'d done when he was one year old. A job he hadn\'t really finished. Somewhere, somehow, the Dark Lord was still alive. Would they cheer so hard if they knew?', 400, 120, { w: 560, size: 27 }),
   ...
   inner('Harry', 'He would fulfil their expectations. And then exceed them—so that people wondered, looking back, that they had once asked so little of him.', 400, 810, { w: 560, size: 27 })], { mood: 'candle' });
...
ep.bleed(840, (ctx) => HG.mindscape(ctx.w, ctx.h, 'warm', 3),
  [inner('Harry', '*Don\'t Sort me yet! I have questions! Have I ever been Obliviated? Did you Sort the Dark Lord when he was a child—can you tell me his weaknesses? Why did I get the brother wand to his?*', 400, 140, { w: 580, size: 27 }),
   inner('Harry', '*Is the Dark Lord\'s ghost bound to my scar, and is that why I get so angry sometimes? And if you\'ve got a moment—how do I rediscover the lost magics that created you?*', 400, 480, { w: 580, size: 27 })], { alt: 'Inside Harry\'s head: an endless candlelit library, and his questions tumbling out.' });
