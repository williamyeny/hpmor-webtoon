...
// the first-years at the table; HAR and HER alone for the close-ups
const HALL = (house) => () => HG.hallTable(house);
const HAR = (o = {}) => ({ def: harryRobes, id: 'harry', x: 700, y: 1080, s: 1.1, turn: 0.2, expr: 'awe', ...o });
const HER = (o = {}) => ({ def: hermione, id: 'hermione', x: 900, y: 1080, s: 1.1, turn: -0.2, expr: 'delight', ...o });
const FY = [{ def: student(1201, 'n'), id: 's1', x: 380, y: 1080, s: 1.05, turn: 0.6, expr: 'awe' }, { def: student(1202, 'n'), id: 's2', x: 1360, y: 1080, s: 1.05, turn: -0.5, expr: 'worried' }, HAR(), HER(), { def: neville, id: 'neville', x: 1100, y: 1080, s: 1.05, turn: -0.3, expr: 'worried' }];
ep.panel(700, { cam: { x: 870, y: 790, w: 700 }, bg: HALL('g'), actors: FY },
...
// the Hat's song
const DAIS = () => HG.dais();
const HAT = (mood = 'talk', x = 1000) => () => HG.stool(x, 900) + g({ transform: `translate(${x},770)` }, HG.sortingHat(0.9, { mood }));
...
ep.panel(560, { cam: { on: ['harry'], fr: 'close' }, bg: HALL('g'), blur: 3, actors: FY },
...
const verdict = (col, txt) => (ctx) => rect(0, 0, ctx.w, ctx.h, { fill: col }) + FX.burst(ctx.w, ctx.h, ctx.w / 2, ctx.h / 2, { col: '#fff', op: 0.18 }) + FX.sfxText(ctx.w / 2, ctx.h * 0.62, txt, { size: Math.min(64, ctx.w / (txt.length * 0.9)), fill: '#f6e7c4', font: 'IM Fell English SC', weight: 400, sw: 5 });
// the Sorting speeds up: the verdicts zigzag down the page on diagonal seams
ep.multi(602, [
  { x: M, y: 18, w: 752, h: 200, shape: 'cut', cutBottom: 40, art: verdict('#8a6a1a', 'HUFFLEPUFF!') },
  { x: M, y: 196, w: 752, h: 210, shape: 'cut', cutTop: 40, cutBottom: -40, art: verdict('#8a6a1a', 'HUFFLEPUFF!') },
  { x: M, y: 384, w: 752, h: 200, shape: 'cut', cutTop: -40, art: verdict('#243a6a', 'RAVENCLAW!') },
...
const staff = [
  ...
  { def: { ...snape, outfit: { ...snape.outfit, handOut: 0.6 } }, id: 'snape', x: 560, y: 900, turn: 0.2, pose: 'holdOne', expr: 'menace', armF: SNAPEF(40, 90) },
...
const snapeEnd = staff.filter((a) => a.id !== 'dumbledore' && a.id !== 'quirrell'); // Snape's close-ups: nobody in front of him
...
ep.panel(780, { cam: { on: ['harry', 'clearwater'], fr: 'bust', dy: -0.6 }, bg: HALL('r'), actors: [HAR(), HER({ expr: { base: 'focus', eyes: { lookX: 1 } } }), RP()] },
...
ep.panel(700, { cam: { on: ['harry'], fr: 'bust' }, bg: HALL('r'), blur: 2, actors: [HAR()] },
...
ep.panel(800, { cam: { on: ['hermione'], fr: 'close', dy: 0.1 }, bg: HALL('r'), blur: 3, actors: [HER({ expr: { base: 'focus', eyes: { lookX: 1, lookY: -0.3 } } })] },
...
ep.multi(880, [
  ...
  { x: 476, y: 250, w: 300, h: 612, mood: 'candle', art: { cam: { head: 'harry', hw: 0.66, hx: 0.5, hy: 0.5 }, bg: HALL('r'), blur: 3, actors: [HAR({ expr: 'wince' })] } },
...
ep.panel(460, verdict('#243a6a', 'RAVENCLAW!'), [], { alt: 'RAVENCLAW!' });
ep.panel(820, { cam: { on: ['harry'], fr: 'bust', zoom: 0.9, dy: -0.1 }, bg: HALL('r'), blur: 2, actors: [HAR({ expr: 'unimpressed', pose: 'crossArms' })] },
...
ep.multi(460, [
  { x: M, y: 18, w: 368, h: 424, art: verdict('#8a6a1a', 'HUFFLEPUFF!') },
  { x: 408, y: 18, w: 368, h: 424, art: verdict('#1f4a35', 'SLYTHERIN!') },
...
ep.panel(760, { cam: { on: ['harry'], fr: 'bust' }, bg: HALL('r'), blur: 2, actors: [HAR({ expr: 'smile' })] },
...
ep.bleed(640, { cam: { x: 800, y: 780, w: 880 }, bg: HALL('s'), actors: [...[1211, 1212, 1213, 1214, 1215, 1216, 1217].map((sd, i) => ({ def: student(sd, ['g', 'r', 'h', 's'][i % 4]), id: 't' + i, x: 150 + i * 220, y: 1050, s: 1.05, turn: i < 3 ? 0.4 : -0.4, expr: { base: 'awe', eyes: { sparkle: false, lookX: i < 3 ? 1 : -1 } } })), () => HG.tableFront()] },
...
ep.panel(620, { cam: { on: ['harry'], fr: 'close' }, bg: HALL('r'), blur: 3, actors: [HAR({ expr: { base: 'worried', sweat: true } })] },
...
ep.panel(900, { cam: { head: 'fred', hw: 0.2, hx: 0.31, hy: 0.1 }, bg: HALL('g'), actors: [{ def: fred, id: 'fred', x: 900, y: 1050, turn: 0.3, pose: 'armsUp', armF: { sh: -166, el: -6, hand: 'splay' }, armB: { sh: 162, el: 10, hand: 'splay' }, expr: 'laugh' }, { def: george, id: 'george', x: 1150, y: 1050, turn: -0.3, pose: 'armsUp', armF: { sh: -162, el: -10, hand: 'splay' }, armB: { sh: 166, el: 6, hand: 'splay' }, expr: 'laugh' }, ...[1221, 1222, 1223].map((sd, i) => ({ def: student(sd, 'g'), x: 500 + i * 450 + (i > 0 ? 600 : 0), y: 1060, s: 1.05, turn: 0.2, pose: 'raiseHand', expr: 'bigGrin' })), () => HG.tableFront()] },
...
ep.multi(1000, [
  ...
  { x: M, y: 606, w: 752, h: 376, mood: 'candle', art: { cam: { on: ['snape'], fr: 'bust', dx: 1.2 }, bg: STAFF, blur: 2, actors: [...snapeEnd, () => HG.staffTable()] } },
...
ep.panel(760, { cam: { x: 1100, y: 640, w: 760 }, bg: HALL('g'), actors: [{ def: fred, id: 'fred', x: 950, y: 1050, turn: 0.3, pose: 'gesture2', expr: 'bigGrin' }, { def: george, id: 'george', x: 1260, y: 1050, turn: -0.3, pose: 'fists', expr: 'laugh' }, () => HG.tableFront()] },
...
ep.panel(1000, { cam: { on: ['snape'], fr: 'bust', dy: -0.1, zoom: 0.9 }, bg: STAFF, blur: 3, actors: [...snapeEnd.map((a) => a.id === 'snape' ? { ...a, expr: { base: 'smile', eyes: { open: 0.6 } }, pose: 'hold', armF: SNAPEF(22, 95, 1), armB: { sh: 8, el: 40, hand: 'fist' } } : a)] },
...
ep.setBg(C.paper);
ep.beat(140, [], { bg: { top: NIGHT, bottom: C.paper } });
ep.end();
export default ep;
