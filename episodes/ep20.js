...
import { snape, student, fred, george, mcgonagall, dad, mum } from '../engine/chars/cast.js';
...
const DCOR = () => CS.corridor({ seed: 43, windows: [], torches: [500, 1500], dim: true }); // the corridor outside it
ep.bleed(900, { cam: { x: 705, y: 625, w: 1100 }, bg: () => DCOR() + rect(-800, -900, 4000, 2200, { fill: '#0c1a14', opacity: 0.35 }), actors: [{ def: harryRaven, id: 'harry', x: 1100, y: 900, s: 1.1, turn: 0.5, pose: 'armsUp', expr: 'rant' }, ...[0, 1, 2].map((i) => ({ def: [padma, terry, anthony][i], id: 'r' + i, x: 690 - i * 210, y: 900, s: 1.05, turn: 0.4, pose: 'walk', expr: 'confused' }))] },
...
// the class on its benches. who: overrides by id, h / he / n: Harry, Hermione, Neville (false leaves that one out)
const CLASS = (o = {}) => {
  const who = { ...o.who, harry: o.h, hermione: o.he, neville: o.n };
  const seats = [SEAT(padma, 'padma', 250), SEAT(student(2011, 'h'), 'hannah', 450), SEAT(harryRaven, 'harry', 700, { s: 1.1, expr: 'neutral' }), SEAT(student(2012, 'h'), 'justin', 920),
    SEAT(hermioneRaven, 'hermione', 1150, { s: 1.1, turn: -0.1 }), SEAT(nevilleHuff, 'neville', 1380, { expr: 'horror' }), SEAT(ernie, 'ernie', 1600), SEAT(terry, 'terry', 1820)];
  return [...seats.filter((a) => who[a.id] !== false).map((a) => ({ ...a, ...who[a.id] })), () => CS.cauldronRow(1120, [700, 1150, 1600], { steam: false })];
};
...
ep.multi(1230, [
  ...
  { x: M, y: 738, w: 368, h: 474, mood: 'candle', art: { cam: { on: ['harry'], fr: 'close', dy: 0.2 }, bg: PR(), blur: 2, actors: CLASS({ h: { expr: 'awe', pose: 'stand', y: 1060, turn: 0.2 } }) } },
...
ep.panel(700, { cam: { x: 942, y: 429, w: 415.6, roll: 4 }, bg: PR(), blur: 2, actors: [SN({ expr: 'cold', turn: -0.15 })], over: SHADE('#050a08', 0.5) },
...
ep.panel(760, { cam: { on: ['snape'], fr: 'close', dy: 0.4, zoom: 1.017, roll: -4 }, bg: PR(), blur: 3, actors: [SN({ turn: 0.1, expr: { base: 'coldSmile', eyes: { open: 0.45 } } })], over: SHADE('#050a08', 0.5) },
...
ep.multi(1222, [
  { x: M, y: 18, w: 752, h: 470, mood: 'candle', art: { cam: { x: 1010, y: 452, w: 532, roll: 5 }, bg: PR(), blur: 3, actors: [SN({ x: 1100, turn: -0.3, expr: { base: 'cold', eyes: { lookX: -0.5, lookY: 0.4 } } })], over: SHADE('#050a08', 0.45) } },
...
ep.panel(1050, { cam: { x: 1000, y: 401, w: 622.1, roll: -4 }, bg: PR(), blur: 2, actors: [SN({ pose: 'stand', turn: -0.15, expr: { base: 'smug', eyes: { lookX: -0.4, lookY: 0.4 } } }), ...SIL([2041, 2042, 2043, 2044], [680, 900, 1120, 1340], 1200, 1.4)], over: SHADE('#050a08', 0.4) },
...
ep.panel(560, { cam: { x: 863, y: 454, w: 564.1, roll: 3 }, bg: PR(), blur: 3, actors: [SN({ turn: -0.7, expr: { base: 'cold', eyes: { open: 0.35 } } })], over: SHADE('#050a08', 0.45) },
...
ep.panel(640, { cam: { x: 1043, y: 447, w: 558.2, roll: -4 }, bg: PR(), blur: 2, actors: [SN({ x: 1150, turn: -0.4, expr: { base: 'coldSmile', eyes: { lookX: -0.5, lookY: 0.5 } } })], over: SHADE('#050a08', 0.45) },
  [say('Snape', 'Detention for one month, Potter.', 230, 240, { w: 300, fixed: true })], { mood: 'candle' });
ep.panel(760, { cam: { x: 623, y: 773, w: 400 }, bg: PR(), blur: 3, actors: CLASS({ who: { hannah: false }, h: { expr: { base: 'cold', eyes: { lookX: 0.4, lookY: -0.4 } }, turn: 0.3 } }), over: (e) => FX.frost(e.w, e.h, 0.62, 97) + rect(0, 0, e.w, e.h, { fill: '#0c1428', opacity: 0.1 }) },
...
ep.panel(480, { cam: { x: 1060, y: 452, w: 494.6, roll: 5 }, bg: PR(), blur: 3, actors: [SN({ x: 1150, turn: -0.4, expr: { base: 'angry', eyes: { lookX: -0.5, lookY: 0.5 } } })], over: SHADE('#050a08', 0.5) },
...
ep.panel(620, { cam: { x: 1030, y: 440, w: 479.7, roll: -4 }, bg: PR(), blur: 3, actors: [SN({ x: 1150, turn: -0.3, expr: { base: 'menace', mouth: { type: 'smirk', w: 0.8 }, eyes: { lookX: -0.5, lookY: 0.4 } } })], over: SHADE('#050a08', 0.5) },
...
ep.panel(960, { cam: { head: 'harry', hw: 0.34, hx: 0.5, hy: 0.54 }, bg: PR(), blur: 3, actors: CLASS({ who: { hannah: false, justin: false }, h: { expr: { base: 'shock', eyes: { lookX: 0.3, lookY: -0.4 } }, turn: 0.25 } }), behind: (e) => FX.burst(e.w, e.h, e.w * 0.5, e.h * 0.52, { n: 70, op: 0.35 }) },
...
// the door
ep.panel(900, { cam: { x: 800, y: 695, w: 1000 }, bg: PR(), actors: [SN({ x: 1200, turn: -0.4, expr: { base: 'angry', eyes: { lookX: -0.6, lookY: 0.4 } } }), ...CLASS({ h: false, who: { padma: { expr: 'gasp', turn: -0.3 }, hannah: { expr: 'gasp', turn: -0.4 }, justin: { expr: 'gasp', turn: -0.4 } }, he: { expr: 'horror', turn: -0.4 } }), { def: harryRaven, id: 'harry', x: 700, y: 1400, s: 1.55, turn: -0.4, pose: 'walk', expr: 'cold' }], over: (e) => FX.frost(e.w, e.h, 0.72, 106) },
...
ep.multi(700, [
  ...
  { x: 366, y: 18, w: 410, h: 664, shape: 'poly', pts: [[0.195, 0], [1, 0], [1, 1], [0, 1]], mood: 'candle', art: { cam: { x: 300, y: 390, w: 340.9, roll: 4 }, bg: PR(), blur: 3, actors: [SN({ x: 300, turn: -0.35, expr: { base: 'smug', eyes: { lookX: -0.6, lookY: 0.5 } } })], over: SHADE('#050a08', 0.5) } },
...
ep.bleed(920, { cam: { x: 850, y: 840, w: 691.4, roll: -5 }, bg: PR(), actors: [SN({ x: 1090, y: 1190, turn: -0.4, expr: 'shock' }), { def: harryRaven, id: 'harry', x: 720, y: 1250, s: 1.5, turn: 0.35, pose: 'stand', poseMod: { armB: { sh: 100, el: 78, hand: 'fist', front: true } }, expr: { base: 'cold', eyes: { lookX: 0.5 } } }], behind: (e) => FX.burst(e.w, e.h, e.w * 0.3, e.h * 0.42, { n: 50, op: 0.18 }), over: (e) => FX.frost(e.w, e.h, 0.85, 109) },
...
ep.panel(900, { cam: { x: 1230, y: 747, w: 520 }, bg: PR(), blur: 2, actors: CLASS({ he: { pose: 'panic', y: 1080, expr: 'yell', turn: -0.3 }, n: { pose: 'cower', y: 1210, expr: 'horror', turn: -0.2 }, who: { ernie: { expr: 'horror' } } }), over: (e) => FX.frost(e.w, e.h, 0.4, 110) },
...
ep.panel(900, { cam: { x: 930, y: 389, w: 614.1, roll: -4 }, bg: PR(), blur: 2, actors: [SN({ x: 1000, turn: -0.4, pose: 'wand', expr: 'angry', armB: { hand: 'hold', prop: wand(110) } })], over: SHADE('#050a08', 0.4) },
...
ep.bleed(1080, { cam: { x: 1000, y: 602, w: 441.4, roll: 6 }, bg: PR(), blur: 3, actors: [SN({ x: 1000, turn: -0.1, expr: { base: 'yell', eyes: { style: 'cold', open: 0.8, lidTilt: -0.5 }, brows: { raise: -0.4, inner: -1.5, outer: 0.3 } } })], over: (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.3, { n: 40, op: 0.25 }) },
...
ep.panel(800, { cam: { x: 850, y: 700, w: 1200 }, bg: PR(), actors: [SN({ x: 1350, turn: -0.4, expr: { base: 'cold', eyes: { lookX: -0.6, lookY: 0.3 } } }), ...CLASS({ h: false, who: { padma: { expr: 'gasp', turn: -0.3 }, hannah: { expr: 'worried', turn: -0.4 }, justin: { expr: 'gasp', turn: -0.4 }, ernie: { expr: 'worried', turn: -0.4 }, terry: { expr: 'gasp', turn: -0.5 } }, he: { expr: 'worried', turn: -0.4 }, n: { expr: 'gasp', turn: -0.4 } }), { def: harryRaven, id: 'harry', x: 420, y: 1400, s: 1.45, turn: 0.5, pose: 'hold', expr: 'cold', armB: { sh: 60, el: 40, hand: 'hold', prop: g({ transform: 'rotate(92) translate(30,-2)' }, P2.sign(["I'M LEAVING", 'DOES ANYONE ELSE', 'NEED TO GET OUT?'], 170, 125)) } }], over: (e) => FX.frost(e.w, e.h, 0.6, 113) },
...
ep.panel(820, { cam: { x: 1330, y: 520, w: 629.6, roll: 4 }, bg: PR({ cupboard: 'open' }), actors: [SN({ x: 1250, y: 920, turn: 0.4, pose: 'reach', expr: { base: 'angry', mouth: { type: 'shout', open: 0.5 } }, armB: { sh: 90, el: 10, hand: 'hold' } })], over: SHADE('#050a08', 0.4) },
...
ep.bleed(1100, { cam: { x: 1000, y: 560, w: 1100 }, bg: () => DCOR() + rect(-800, -900, 4000, 2200, { fill: '#0c1a14', opacity: 0.3 }), actors: [() => CS.invisible(1000, 900, 1.25) + K.glow(1000, 620, 260, '#dfe8f5', 0.12)] },
...
ep.panel(950, { cam: { x: 1000, y: 536, w: 320 }, bg: DCOR, blur: 3, actors: [{ def: harryRaven, id: 'harry', x: 1000, y: 900, s: 1.1, turn: 0, expr: 'horror' }], over: (e) => rect(0, 0, e.w, e.h, { fill: '#dfe8f5', opacity: 0.18 }) },
...
ep.multi(1000, [
  { x: M, y: 18, w: 752, h: 480, shape: 'cloud', seed: 66, mood: 'sepia', art: { cam: { x: 965, y: 525, w: 520 }, bg: () => rect(-500, -500, 3000, 2000, { fill: '#b9a27a' }), actors: [{ def: dad, id: 'dad', x: 850, y: 1000, turn: 0.3, pose: 'fists', expr: 'yell' }, { def: mum, id: 'mum', x: 1050, y: 1010, turn: -0.3, pose: 'stand', expr: 'cry' }] } },
...
ep.panel(1100, { cam: { on: ['harry'], fr: 'bust', dy: -0.1 }, bg: DCOR, blur: 2, actors: [{ def: harryRaven, id: 'harry', x: 1000, y: 900, s: 1.1, turn: 0, pose: 'panic', expr: 'cry' }], over: (e) => rect(0, 0, e.w, e.h, { fill: '#dfe8f5', opacity: 0.18 }) },
...
ep.panel(760, { cam: { head: 'harry', hw: 0.95, hx: 0.5, hy: 0.56 }, bg: DCOR, blur: 3, actors: [{ def: harryRaven, id: 'harry', x: 1000, y: 900, s: 1.1, turn: 0, expr: 'sad' }], over: (e) => rect(0, 0, e.w, e.h, { fill: '#0c1428', opacity: 0.3 }) },
...
ep.bleed(1100, { cam: { x: 1085, y: 651, w: 420 }, bg: DCOR, actors: [{ def: harryRaven, id: 'harry', x: 1000, y: 900, s: 1.1, turn: 0, expr: 'cold' }], over: (e) => FX.frost(e.w, e.h, 0.85, 113) + rect(0, 0, e.w, e.h, { fill: '#0c1428', opacity: 0.25 }) },
...
ep.panel(1300, { cam: { x: 1000, y: 630, w: 440 }, bg: DCOR, blur: 2, actors: [{ def: harryRaven, id: 'harry', x: 1000, y: 900, s: 1.1, turn: 0.2, pose: 'stand', expr: 'coldSmile' }], over: (e) => FX.frost(e.w, e.h, 0.88, 115) + rect(0, 0, e.w, e.h, { fill: '#0c1428', opacity: 0.2 }) },
...
ep.bleed(820, { cam: { on: ['harry'], fr: 'eyes', zoom: 0.8 }, bg: DCOR, actors: [{ def: harryRaven, id: 'harry', x: 1000, y: 900, s: 1.1, turn: 0, expr: { base: 'cold', eyes: { open: 0.5 } } }], over: (e) => FX.frost(e.w, e.h, 0.92, 117) + rect(0, 0, e.w, e.h, { fill: '#0c1428', opacity: 0.35 }) },
