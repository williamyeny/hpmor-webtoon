...
ep.panel(900, { cam: { x: 950, y: 774, w: 620 }, bg: BT, actors: [HB({ expr: { base: 'bigGrin', eyes: { sparkle: true } } }), { def: padma, id: 'padma', x: 1100, y: 1050, s: 1.05, turn: -0.3, expr: 'neutral' }, FRONT] },
...
ep.multi(620, [
  ...
  { x: 536, y: 18, w: 240, h: 584, mood: 'day', art: { cam: { x: 800, y: 690, w: 280 }, bg: BT, actors: [HB({ expr: 'exasperated' })] } },
], [say('S1', 'Pardon me…', 144, 64, { anchor: 'tc', w: 180, fixed: true, tail: 's1' }), say('S2', 'Harry? About Snape…', 400, 64, { anchor: 'tc', w: 190, fixed: true, tail: 's2' }), say('Harry', 'Yes, I\'ll try not to draw his attention…', 656, 64, { anchor: 'tc', w: 200, fixed: true })], { alt: 'Warning after warning.' });
...
ep.panel(1300, { cam: { x: 815, y: 399, w: 720 }, bg: BT, actors: TW({ h: { expr: 'wince' } }) },
  [say('Fred', 'Oh, that\'s hopeless.', 170, 80, { anchor: 'tc', w: 200, fixed: true }),
   say('George', 'Completely hopeless.', 520, 120, { anchor: 'tc', w: 200, fixed: true }),
...
ep.panel(1000, { cam: { x: 815, y: 594, w: 720 }, bg: BT, actors: TW({ f: { expr: 'laugh', pose: 'bow' }, g: { expr: 'laugh', pose: 'bow' }, h: { expr: 'smug' } }) },
...
ep.bleed(900, { cam: { x: 740, y: 625, w: 1100 }, bg: () => CS.corridor({ seed: 43, windows: [], torches: [500, 1500], dim: true }) + rect(-800, -900, 4000, 2200, { fill: '#0c1a14', opacity: 0.35 }), actors: [{ def: harryRaven, id: 'harry', x: 1100, y: 900, s: 1.1, turn: 0.5, pose: 'armsUp', expr: 'rant' }, ...[0, 1, 2].map((i) => ({ def: [padma, terry, anthony][i], id: 'r' + i, x: 700 - i * 180, y: 900, s: 1.05, turn: 0.4, pose: 'walk', expr: 'confused' }))] },
...
ep.panel(760, { cam: { x: 623, y: 773, w: 400 }, bg: PR({}), blur: 3, actors: CLASS({ h: { expr: { base: 'cold', eyes: { lookX: 0.4, lookY: -0.4 } }, turn: 0.3 } }), over: (e) => FX.frost(e.w, e.h, 0.62, 97) + rect(0, 0, e.w, e.h, { fill: '#0c1428', opacity: 0.1 }) },
...
ep.panel(1060, { cam: { x: 810, y: 500, w: 620 }, bg: PR({}), actors: [SN({ x: 940, turn: -0.4, poseMod: { lean: 18, headTilt: 8 }, expr: { base: 'menace', eyes: { lookX: -0.5, lookY: 0.6 } } }), ...CLASS({ h: { expr: { base: 'cold', eyes: { lookX: 0.5, lookY: -0.6 } }, turn: 0.4 } })], over: (e) => FX.frost(e.w, e.h, 0.72, 104) },
  [say('Snape', 'And I find it increasingly unlikely that you were not Sorted into Slytherin. How did you stay out of my House? Ah, yes: the Hat claimed it was *joking.* For the first time in recorded history. What were you *really* chatting about with the Sorting Hat, Potter?', 400, 64, { anchor: 'tc', w: 560, fixed: true })], { mood: 'candle', alt: 'Snape leans down over Harry\'s bench.' });
...
ep.panel(760, { cam: { x: 700, y: 720, w: 420 }, bg: PR({}), blur: 3, actors: CLASS({ h: { expr: { base: 'shock', eyes: { lookX: 0.3, lookY: -0.4 } }, turn: 0.25 } }), over: (e) => FX.burst(e.w, e.h, e.w * 0.5, e.h * 0.66, { n: 70, op: 0.35 }) },
...
// the door
ep.panel(900, { cam: { x: 800, y: 650, w: 1000 }, bg: PR({}), actors: [SN({ x: 1200, turn: -0.4, expr: { base: 'angry', eyes: { lookX: -0.6, lookY: 0.4 } } }), ...CLASS({ h: { x: -900 }, who: { padma: { expr: 'gasp', turn: -0.3 }, hannah: { expr: 'gasp', turn: -0.4 }, justin: { expr: 'gasp', turn: -0.4 } }, he: { expr: 'horror', turn: -0.4 } }), { def: harryRaven, id: 'harry', x: 500, y: 1300, s: 1.3, turn: -0.4, pose: 'walk', expr: 'cold' }], over: (e) => FX.frost(e.w, e.h, 0.72, 106) },
  [cold('Harry', 'I have no intention of letting one unprofessional teacher ruin my time at Hogwarts. I think I\'ll take my leave of this class, and hire a tutor. If any of you decide you don\'t care to be bullied by this man, my sessions will be open to you.', 310, 60, { anchor: 'tc', w: 410, fixed: true })], { mood: 'cold', alt: 'Harry walks out from the benches toward the door. The class stares.' });
ep.panel(900, { cam: { x: -280, y: 640, w: 700 }, bg: PR({}), actors: [{ def: harryRaven, id: 'harry', x: -170, y: 960, s: 1.1, turn: -0.6, pose: 'reach', expr: 'cold', armB: { sh: 118, el: 4, hand: 'hold' } }], over: (e) => FX.frost(e.w, e.h, 0.72, 107) },
...
ep.bleed(1000, tilt(6, { cam: { x: 1000, y: 580, w: 440 }, bg: PR({}), blur: 3, actors: [SN({ x: 1000, turn: -0.1, expr: { base: 'yell', eyes: { style: 'cold', open: 0.8, lidTilt: -0.5 }, brows: { raise: -0.4, inner: -1.5, outer: 0.3 } } })] }, (e) => FX.burst(e.w, e.h, e.w / 2, e.h * 0.3, { n: 40, op: 0.25 })),
  [shout('Snape', 'Ten points from Ravenclaw! Ten points from Ravenclaw! Ten points from Ravenclaw! *FIFTY* points from Ravenclaw! Now sit down and watch the rest of the class take their lesson!', 400, 915, { anchor: 'bc', w: 440, size: 31, fixed: true })], { alt: 'Snape, shouting, taking points in a stream.' });
ep.panel(1000, { cam: { x: 480, y: 742, w: 340 }, bg: PR({}), blur: 2, actors: [{ def: harryRaven, id: 'harry', x: 450, y: 1000, s: 1.1, turn: 0.2, pose: 'hold', expr: { base: 'cold', eyes: { lookY: 0.6 } }, armF: { sh: 34, el: 96, hand: 'hold', prop: g({ transform: 'rotate(-30)' }, P2.marker(1.1)) }, armB: { sh: 30, el: 70, hand: 'hold', prop: g({ transform: 'rotate(8)' }, sheet({ w: 110, h: 80 })) } }], over: (e) => FX.frost(e.w, e.h, 0.72, 112) },
...
ep.setBg(C.paper);
ep.panel(1000, { cam: { x: 1650, y: 560, w: 800 }, bg: PR({ cupboard: 'open' }), actors: [{ def: harryRaven, id: 'harry', x: 1650, y: 900, s: 1.1, turn: 0.1, pose: 'hold', expr: 'focus' }, (e) => g({ transform: 'translate(1650,700)' }, P2.cloak(260, 340, { fade: 0.85 }))] },
...
ep.panel(760, { cam: { on: ['harry'], fr: 'eyes' }, bg: () => CS.corridor({ seed: 43, windows: [], torches: [500, 1500], dim: true }), blur: 3, actors: [{ def: harryRaven, id: 'harry', x: 1000, y: 900, s: 1.1, turn: 0, expr: 'sad' }], over: (e) => rect(0, 0, e.w, e.h, { fill: '#0c1428', opacity: 0.3 }) },
...
ep.bleed(1100, { cam: { x: 1000, y: 651, w: 420 }, bg: () => CS.corridor({ seed: 43, windows: [], torches: [500, 1500], dim: true }), actors: [{ def: harryRaven, id: 'harry', x: 1000, y: 900, s: 1.1, turn: 0, expr: 'cold' }], over: (e) => FX.frost(e.w, e.h, 0.85, 113) + rect(0, 0, e.w, e.h, { fill: '#0c1428', opacity: 0.25 }) },
  [cap('The calming cold washed back through his veins like a wave returning from some breaker. And Harry let out his breath.', 44, 40, { w: 620, fixed: true }),
   cold('Harry', 'Okay. Back to being sane now.', 400, 1030, { anchor: 'bc', w: 360, fixed: true })], { mood: 'cold', alt: 'The frost comes back, all the way.' });
ep.panel(1300, { cam: { x: 1000, y: 630, w: 440 }, bg: () => CS.corridor({ seed: 43, windows: [], torches: [500, 1500], dim: true }), blur: 2, actors: [{ def: harryRaven, id: 'harry', x: 1000, y: 900, s: 1.1, turn: 0.2, pose: 'stand', expr: 'coldSmile' }], over: (e) => FX.frost(e.w, e.h, 0.88, 115) + rect(0, 0, e.w, e.h, { fill: '#0c1428', opacity: 0.2 }) },
  [cold('Harry', 'He was actually a bit disappointed in his non-angry self, for collapsing like that and wanting only to get out of trouble. Snape was *everyone\'s* problem. Normal-Harry had forgotten that, and wished only to protect *himself.* And let all the other victims go hang?', 400, 76, { anchor: 'tc', w: 560, fixed: true }),
   cold('Harry', 'So this is my dark side, is it? Bit of a prejudiced term, that. My light side seems more selfish and cowardly. Not to mention confused and panicky.', 400, 1255, { anchor: 'bc', w: 560, fixed: true })], { mood: 'cold' });
