...
const SET = (d = {}, h = {}, desk = {}) => [() => CS.dumbledoreThrone(1000), DB(d), () => CS.blackDesk(1000, 960, desk), HD(h)];
...
// the reveal: the panel is the doorway at the top of the stairs, so the reader steps into the office with Harry
ep.panel(1180, { cam: { x: 1000, y: 53, w: 1560 }, bg: OF(), actors: [DECOR, ...SET({}, { x: 1360, turn: -0.3, expr: 'awe' })] },
  [cap('And Harry saw the most interesting room he\'d ever seen in his life.', 44, 24, { w: 620, fixed: true })],
  { shape: 'arch', frame: 'stone', spring: 0.34, y: 136, ph: 1026, alt: 'Dumbledore\'s office: shelves of whirring silver instruments, portraits asleep in their frames, a hat rack with the Sorting Hat, a bird on a golden perch, and behind a clean black desk, a cushioned throne.' });
ep.multi(990, [
  { x: M, y: 310, w: 368, h: 290, mood: 'candle', art: { cam: { x: 1700, y: 560, w: 400 }, bg: OF(), actors: [DECOR] } },
  { x: M + 24, y: 612, w: 320, h: 364, mood: 'candle', shape: 'oval', frame: 'gilt', art: { cam: { x: 95, y: -584, w: 166 }, bg: OF(), actors: [DECOR] } },
...
ep.panel(520, { cam: { head: 'dumbledore', hw: 0.7, hx: 0.5, hy: 0.33 }, bg: OF(), blur: 3, actors: SET({ expr: { base: 'smile', eyes: { sparkle: true, open: 1 } } }), over: (e) => { const h = e.anchors.dumbledore.head; return FX.sparkles([[h[0] - 230, h[1] - 60, 26], [h[0] + 240, h[1] - 70, 22], [h[0] + 290, h[1] + 10, 14], [h[0] - 290, h[1] + 20, 12]]); } },
  [cap('His eyes twinkled with a mad intensity.', 400, 494, { anchor: 'bc', w: 560, fixed: true })], { mood: 'candle', shape: 'eye', y: 30, ph: 376, alt: 'Dumbledore\'s eyes, close, twinkling with a mad intensity.' });
...
ep.panel(760, { cam: { head: 'dumbledore', w: 720, hx: 0.235, hy: 0.321 }, bg: OF(), actors: [DECOR, ...SET({}, { expr: 'neutral' })] },
...
ep.panel(760, { cam: { head: 'dumbledore', w: 520, hx: 0.501, hy: 0.555 }, bg: OF(), blur: 2, actors: SET({ pose: 'gesture', expr: { base: 'happy', eyes: { sparkle: true } } }) },
...
ep.panel(520, { cam: { head: 'harry', w: 380, hx: 0.622, hy: 0.581 }, bg: OF(), blur: 3, actors: SET({}, { expr: 'smug' }) },
  [say('Harry', 'I\'ll be sure to, Heh.', 262, 100, { w: 290, fixed: true })], { mood: 'candle' });
ep.panel(760, { cam: { head: 'dumbledore', w: 440, hx: 0.502, hy: 0.721 }, bg: OF(), blur: 2, actors: SET({ expr: 'shock' }) },
...
ep.panel(640, { cam: { head: 'harry', w: 420, hx: 0.703, hy: 0.631 }, bg: OF(), blur: 2, actors: SET({}, { expr: 'horror' }) },
  [say('Harry', 'Ah… I\'m sorry, I, ah, Headmaster, you told me to do it, so I did—', 400, 60, { anchor: 'tc', w: 520, fixed: true })], { mood: 'candle' });
ep.panel(820, { cam: { head: 'dumbledore', w: 520, hx: 0.501, hy: 0.614 }, bg: OF(), blur: 2, actors: SET({ pose: 'gesture', expr: 'warm' }) },
...
ep.panel(900, { cam: { head: 'dumbledore', w: 760, hx: 0.275, hy: 0.313 }, bg: OF(), actors: [DECOR, ...SET({ pose: 'present', expr: 'smile' }, { expr: 'suspicious' }), atAnchor('dumbledore', 'handB', sweets, { dx: 10, dy: -22, k: 1.3 })] },
...
ep.panel(660, { cam: { head: 'harry', w: 460, hx: 0.757, hy: 0.675 }, bg: OF(), blur: 2, actors: SET({}, { expr: 'deadpan', pose: 'gesture' }) },
...
// shot helpers: Dumbledore on the throne / Harry in front of the desk (camera on that one's head, at hx, hy of the panel) / a two-shot
const dbS = (h, w, hx, hy, d, balloons, o = {}) => ep.panel(h, { cam: { head: 'dumbledore', w, hx, hy }, bg: OF(o.of), blur: o.blur ?? 2, actors: [...SET(d, {}, o.desk), ...(o.extra || [])] }, balloons, { mood: 'candle', alt: o.alt });
const hS = (h, w, hx, hy, hh, balloons, o = {}) => ep.panel(h, { cam: { head: 'harry', w, hx, hy }, bg: OF({ rack: false, ...o.of }), blur: o.blur ?? 2, actors: [...SET(o.d, hh, o.desk), ...(o.extra || [])] }, balloons, { mood: 'candle', alt: o.alt });
const two = (h, cam, d, hh, balloons, o = {}) => ep.panel(h, { cam, bg: OF(o.of), actors: [DECOR, ...SET(d, hh, o.desk)] }, balloons, { mood: 'candle', alt: o.alt });
...
const hang = (id, len) => (e) => { const h = e.wa[id].handB, l = [h[0] - 45, h[1]], r = [h[0] + 45, h[1]]; return cloakFill(drapeD(l, r, len), l, r, len); };
const holdOut = { armF: { sh: -40, el: -50, hand: 'hold' }, armB: { sh: 45, el: 50, hand: 'hold' } };
dbS(900, 520, 0.501, 0.685, { pose: 'gesture', expr: 'happy' },
...
hS(460, 380, 0.702, 0.64, { expr: 'suspicious' },
  [say('Harry', 'Why did you call me here, then?', 262, 110, { w: 300, fixed: true })]);
dbS(780, 420, 0.502, 0.601, { pose: 'gesture', expr: { base: 'grin', eyes: { sparkle: true } } },
...
hS(1100, 440, 0.517, 0.8, { expr: 'smug' },
...
dbS(800, 480, 0.502, 0.736, { expr: 'calm' },
...
dbS(900, 520, 0.501, 0.662, { pose: 'gesture', expr: { base: 'smile', eyes: { sparkle: true } } },
...
two(1060, { head: 'dumbledore', w: 700, hx: 0.331, hy: 0.508 }, { pose: 'reach', expr: { base: 'smile', eyes: { sparkle: true } } }, { expr: 'worried' },
...
// the anonymous note's warning, remembered: a scrap of the note itself pinned over Harry's frozen face
const noteScrap = (ctx) => rect(0, 0, ctx.w, ctx.h, { fill: '#f1e6c8' }) + [...Array(6).keys()].map((k) => line(0, 34 + k * 38, ctx.w, 34 + k * 38, { stroke: '#b9c7d8', 'stroke-width': 1.2, opacity: 0.5 })).join('');
ep.multi(960, [
  { x: M, y: 18, w: 752, h: 924, mood: 'candle', art: { cam: { head: 'harry', hw: 0.5, hx: 0.5, hy: 0.6 }, bg: OF({ rack: false }), blur: 3, actors: SET({}, { expr: 'horror' }), over: (e) => FX.memoryEdge(e.w, e.h) } },
  { x: 130, y: 34, w: 540, h: 236, shape: 'torn', frame: 'paper', seed: 8, tear: 12, rotate: -2, shadow: true, grain: false, art: noteScrap },
], [capC('*If Dumbledore saw a chance to possess one of the Deathly Hallows, he would never let it escape his grasp until the day he died.*', 400, 76, { anchor: 'tc', w: 460, fixed: true, bg: 'transparent', border: 'transparent', rot: -2 }),
   cap('This was the most powerful wizard in the world. There was no way he could make it out of the door.', 44, 830, { w: 620, fixed: true })], { alt: 'Harry, frozen, remembering the anonymous note: a torn scrap of it hangs over his face.' });
dbS(840, 460, 0.502, 0.712, { pose: 'sit', expr: 'sad' },
...
hS(480, 400, 0.675, 0.611, { expr: 'shock' },
  [say('Harry', 'I can?', 250, 130, { w: 160, fixed: true })]);
dbS(800, 500, 0.502, 0.736, { expr: 'worried' },
...
dbS(860, 420, 0.502, 0.755, { expr: 'calm' },
...
hS(920, 560, 0.679, 0.432, { pose: 'present', expr: 'wince', armB: { sh: 80, el: 10, hand: 'hold', prop: g({ transform: 'translate(0,20)' }, pouch(0.8)) } },
...
ep.bleed(1040, { cam: { head: 'dumbledore', w: 460, hx: 0.502, hy: 0.577 }, bg: OF(), actors: [() => CS.dumbledoreThrone(1000), DB({ y: 930, pose: 'hold', armF: { sh: -50, el: -55, hand: 'hold' }, armB: { sh: 55, el: 55, hand: 'hold' }, expr: { base: 'awe', eyes: { soft: true } } }), () => CS.blackDesk(1000, 960, {}), drape('dumbledore', 320)] },
...
ep.panel(920, { cam: { head: 'dumbledore', w: 760, hx: 0.288, hy: 0.545 }, bg: OF(), actors: [DECOR, () => CS.dumbledoreThrone(1000), DB({ y: 930, pose: 'hold', ...holdOut, expr: 'stern' }), () => CS.blackDesk(1000, 960, {}), drape('dumbledore', 220), HD({ expr: 'worried' })] },
...
ep.panel(820, { cam: { head: 'dumbledore', w: 760, hx: 0.288, hy: 0.398 }, bg: OF(), actors: [DECOR, () => CS.dumbledoreThrone(1000), DB({ y: 930, pose: 'present', expr: { base: 'sad', eyes: { soft: true } } }), () => CS.blackDesk(1000, 960, {}), hang('dumbledore', 250), HD({ expr: 'shock', pose: 'reach' })] },
...
hS(700, 460, 0.73, 0.68, { expr: 'embarrassed' },
...
dbS(1020, 500, 0.502, 0.734, { expr: { base: 'sad', eyes: { soft: true } } },
...
const away = { x: 1900, y: 960, turn: -0.5, pose: 'stand' }; // for Harry close-ups while Dumbledore is over by the table
ep.panel(1000, { cam: { head: 'dumbledore', w: 640, hx: 0.587, hy: 0.656 }, bg: OF(), actors: ROOM({ expr: { base: 'worried', eyes: { lookX: -0.6, lookY: 0.4 } } }) },
...
hS(780, 460, 0.713, 0.62, { turn: 0.4, pose: 'gesture', expr: 'happy' },
...
ep.panel(460, { cam: { head: 'dumbledore', w: 400, hx: 0.68, hy: 0.666 }, bg: OF(), blur: 2, actors: ROOM({ expr: 'shock', turn: -0.3 }) },
  [say('Dumbledore', 'You\'re *what?*', 200, 150, { w: 220, fixed: true })], { mood: 'candle' });
hS(880, 440, 0.702, 0.712, { turn: 0.4, pose: 'lecture', expr: 'determined' },
...
// Harry's grand cause: he steps out of the office onto the page, fists clenched
ep.cutout(940, { cam: { head: 'harry', hw: 0.28, hx: 0.47, hy: 0.42 }, actors: [HD({ turn: 0.3, pose: 'fists', armB: { sh: 140, el: 10, hand: 'fist' }, legF: { hip: -12 }, legB: { hip: 12 }, expr: { base: 'determined', mouth: { type: 'grin' } } })] },
  [say('Harry', 'Not just for *me!* For all of magical Britain! *And* he\'ll have a happier and mentally healthier life himself!', 400, 44, { anchor: 'tc', w: 500, fixed: true })], { alt: 'Harry, fists clenched, utterly sincere.' });
ep.bleed(1000, { cam: { head: 'dumbledore', w: 520, hx: 0.383, hy: 0.645 }, bg: OF(), actors: ROOM({ pose: 'bow', expr: 'laugh', turn: -0.2 }), over: (e) => { const h = e.anchors.dumbledore.head; return FX.emanata(h[0], h[1], 150, { n: 9, a0: -170, a1: -10 }); } },
...
ep.panel(900, { cam: { head: 'dumbledore', w: 800, hx: 0.749, hy: 0.477 }, bg: OF(), actors: ROOM({ expr: 'happy', turn: -0.4 }, { x: 1650, expr: 'shock', pose: 'point', armB: { sh: 112, el: 0, hand: 'point' } }) },
...
hS(440, 380, 0.714, 0.647, { turn: 0.4, expr: 'shock' },
  [say('Harry', 'You\'re *Muggle-born?*', 262, 120, { w: 340, fixed: true })], { d: away });
ep.panel(800, { cam: { head: 'dumbledore', w: 500, hx: 0.495, hy: 0.71 }, bg: OF(), blur: 2, actors: ROOM({ expr: { base: 'smile', eyes: { sparkle: true } }, pose: 'gesture', turn: -0.3 }) },
...
// Gandalf on the bridge: the wand (and its light) thrusts up out of the panel
ep.panel(1120, { cam: { head: 'dumbledore', w: 620, hx: 0.574, hy: 0.254 }, bg: OF(), mid: [DECOR, () => CS.dumbledoreThrone(1000), () => CS.blackDesk(1000, 960, {})].map((f) => f()).join(''), actors: [DBW({ pose: 'wandUp', expr: 'determined', turn: -0.35, armB: { sh: 160, el: -12, hand: 'hold', prop: wand(240) }, armF: { sh: -70, el: -25, hand: 'splay' }, legF: { hip: -16 }, legB: { hip: 16 } })], behind: (e) => { const h = e.toPanel(e.wa.dumbledore.handB); return FX.burst(e.w, e.h, h[0], h[1] - 120, { n: 24, op: 0.15 }); } },
  [shout('Dumbledore', '*You cannot pass!* How does that look?', 460, 70, { anchor: 'tc', w: 400, fixed: true })], { mood: 'candle', breakout: 'top', y: 270, ph: 830, tile: { over: () => K.glow(150, 200, 80, '#fff3c9', 0.9) + FX.sparkles([[150, 196, 20], [104, 160, 11], [96, 232, 8]], { col: '#e0b040' }) }, alt: 'Dumbledore strikes a pose, wand aloft, in pink pyjamas.' });
hS(460, 380, 0.714, 0.64, { turn: 0.4, expr: 'deadpan' },
  [say('Harry', 'Ah… I think you\'re missing a Balrog.', 262, 130, { w: 320, fixed: true })], { d: away });
ep.panel(900, { cam: { head: 'dumbledore', w: 480, hx: 0.434, hy: 0.705 }, bg: OF(), blur: 2, actors: ROOM({ pose: 'slump', expr: 'sad', turn: -0.3 }) },
...
ep.panel(760, { cam: { head: 'dumbledore', w: 400, hx: 0.496, hy: 0.722 }, bg: OF(), blur: 3, actors: ROOM({ expr: { base: 'smug', eyes: { lookX: 0.5 } }, turn: -0.2 }) },
...
ep.panel(820, { cam: { head: 'dumbledore', w: 800, hx: 0.749, hy: 0.487 }, bg: OF(), actors: ROOM({ pose: 'present', expr: { base: 'smile', eyes: { sparkle: true } }, turn: -0.4 }, { expr: 'hopeful' }) },
...
ep.panel(900, { cam: { head: 'dumbledore', w: 820, hx: 0.389, hy: 0.4 }, bg: OF(), actors: [DECOR, () => CS.dumbledoreThrone(1000), DB({ y: 930, x: 1060, pose: 'reach', armF: { sh: 0, el: 18, hand: 'open' }, armB: { sh: 30, el: 20, hand: 'open' }, expr: 'focus', turn: -0.35 }), () => CS.blackDesk(1000, 960, { rock: true }), HD({ expr: 'deadpan' })], over: (e) => { const r = e.toPanel([940, 640]); return FX.emanata(r[0], r[1], 125, { n: 3, a0: -200, a1: -165 }) + FX.emanata(r[0], r[1], 125, { n: 3, a0: -15, a1: 20 }); } },
...
const RK = { rock: true };
hS(460, 380, 0.702, 0.64, { expr: 'deadpan' },
  [say('Harry', 'This is a joke, right?', 262, 120, { w: 280, fixed: true })], { desk: RK });
dbS(860, 500, 0.502, 0.755, { expr: 'stern' },
...
two(660, { x: 1130, y: 600, w: 760 }, { expr: 'stern' }, { expr: { base: 'suspicious', eyes: { lookX: -0.8, lookY: 0.3 } }, pose: 'point', turn: -0.5, armB: { sh: 128, el: 0, hand: 'point' } },
...
dbS(780, 500, 0.486, 0.644, { expr: 'stern', pose: 'lecture' },
...
dbS(900, 560, 0.501, 0.736, { expr: 'calm', pose: 'present' },
...
dbS(560, 380, 0.715, 0.596, { expr: { base: 'calm', eyes: { soft: true } } },
  [say('Dumbledore', 'It is only our own ignorance which appears to us as insanity.', 60, 50, { anchor: 'tl', w: 300, fixed: true, tail: 'dumbledore' })], { desk: RK, blur: 3 });
hS(500, 400, 0.702, 0.628, { expr: 'think' },
  [say('Harry', 'Reality is always lawful, even if we don\'t know the law.', 262, 60, { anchor: 'tc', w: 300, fixed: true })], { desk: RK });
dbS(640, 480, 0.502, 0.699, { expr: 'happy' },
  [say('Dumbledore', 'Precisely, Harry. To understand this is the essence of wisdom.', 400, 60, { anchor: 'tc', w: 440, fixed: true })], { desk: RK });
two(660, { x: 1130, y: 600, w: 760 }, { expr: 'calm' }, { expr: 'suspicious', pose: 'point', turn: -0.5, armB: { sh: 128, el: 0, hand: 'point' } },
  [say('Harry', 'So… *why* do I have to carry this rock, exactly?', 560, 60, { anchor: 'tc', w: 320, fixed: true })], { desk: RK });
dbS(620, 400, 0.502, 0.672, { expr: { base: 'grin', eyes: { sparkle: true } } },
...
ep.panel(780, (ctx) => { let o = sketchPaper(ctx); const R = rng(4); const x0 = 60, y0 = 290, cw = 42, ch = 40, HC = 10, HR = 4;
  ...
  return o; },
  [say('Harry', 'If there are a million possibilities, most of the evidence you need goes into just *locating* the right one: bringing it to your attention at all.', 400, 60, { anchor: 'tc', w: 520, fixed: true, noTail: true })], { shape: 'torn', frame: 'paper', seed: 14, tear: 12, rotate: -1.2, shadow: true, pad: 26, alt: 'Harry\'s pencil sketch: a grid of boxes, one with a diamond, and a magnifying glass homing in on it.' });
...
ep.panel(860, (ctx) => { let o = sketchPaper(ctx); const R = rng(9);
  ...
  return o; },
  [say('Harry', 'You can\'t just pluck one possibility out of thin air and promote it. Like a detective with no evidence saying, "Have we considered the possibility that Mortimer Snodgrass did it?"', 400, 60, { anchor: 'tc', w: 520, fixed: true, noTail: true })], { frame: 'paper', rotate: 1, shadow: true, pad: 26, alt: 'Harry\'s sketch: a detective in a deerstalker points at random at one bewildered stick figure in a crowd: Mortimer Snodgrass.' });
two(760, { x: 1130, y: 600, w: 760 }, { expr: 'calm' }, { expr: 'rant', pose: 'point', turn: -0.5, armB: { sh: 128, el: 0, hand: 'point' } },
...
dbS(1000, 500, 0.502, 0.684, { expr: 'think', pose: 'crossArms' },
...
hS(580, 400, 0.542, 0.462, { expr: { base: 'shock', mouth: { type: 'o' } } },
  [cap('Harry opened his mouth, and found he had no reply ready.', 44, 450, { w: 440, fixed: true })], { desk: RK });
dbS(920, 500, 0.502, 0.678, { expr: { base: 'smile', eyes: { sparkle: true } }, pose: 'gesture' },
...
ep.panel(920, { cam: { x: 880, y: 569.7, w: 700 }, bg: OF(), actors: [DECOR, () => CS.dumbledoreThrone(1000), () => CS.blackDesk(1000, 960, { rock: true }), eatenLegs, HD({ expr: 'deadpan' })] },
...
const BOOKH = atAnchor('dumbledore', 'handB', P2.potionsBook(1), { dx: 6, dy: -70, rot: -8, k: 0.62 });
dbS(900, 500, 0.478, 0.694, { expr: 'stern', pose: 'holdUp', armB: { sh: 120, el: 20, hand: 'hold' } },
...
hS(700, 440, 0.73, 0.68, { expr: 'suspicious' },
...
hS(720, 440, 0.73, 0.733, { expr: 'determined' },
...
dbS(640, 460, 0.502, 0.666, { expr: 'stern' },
...
// no frame: the open book lies on the page itself, as if Dumbledore had set it down in front of the reader
ep.cutout(1180, (ctx) => { const cx = ctx.w / 2, top = 285, W2 = 340, H2 = 380;
  let o = rect(cx - W2 - 22, top - 4, 2 * W2 + 44, H2 + 34, { rx: 12, fill: '#2a1a12', opacity: 0.3, filter: 'url(#blur3)', transform: 'translate(6,12)' }) +
    rect(cx - W2 - 24, top - 14, 2 * W2 + 48, H2 + 30, { rx: 12, fill: '#6b2f24', stroke: C.ink, 'stroke-width': 3 }) + line(cx, top - 14, cx, top + H2 + 16, { stroke: '#4a1f18', 'stroke-width': 6 });
...
dbS(640, 480, 0.502, 0.666, { expr: 'stern' },
...
hS(480, 400, 0.702, 0.633, { expr: { base: 'grin', sweat: true } },
  [say('Harry', 'Ehhh… sorry… not really…', 262, 120, { w: 260, fixed: true })], { desk: RK });
// he heads for the door
ep.panel(820, { cam: { x: 2100, y: 608.1, w: 800 }, bg: OF(), actors: [DECOR, HD({ x: 1950, y: 1000, turn: 0.6, pose: 'walk', expr: { base: 'grin', sweat: true } })] },
...
ep.panel(640, { cam: { x: 2343.8, y: 598.5, w: 520 }, bg: OF(), actors: [HDR({ x: 2270, y: 845, pose: 'stand', expr: 'horror', armB: { sh: 122, el: -5, hand: 'hold' } })] },
...
ep.panel(760, { cam: { x: 2051.7, y: 383.1, w: 440 }, bg: OF(), blur: 2, actors: [DBD({ pose: 'slump' }), HDR({ turn: -0.5, expr: 'horror' })] },
...
ep.panel(760, { cam: { head: 'dumbledore', w: 400, hx: 0.508, hy: 0.722 }, bg: OF(), blur: 3, actors: [DBD({ expr: { base: 'sad', eyes: { soft: true } } })] },
...
ep.panel(800, { cam: { head: 'dumbledore', w: 700, hx: 0.305, hy: 0.5 }, bg: OF(), actors: [DECOR, DBD({ expr: { base: 'sad', eyes: { soft: true } }, pose: 'shrug' }), HDR({ turn: -0.5, expr: 'worried' })] },
...
ep.panel(560, { cam: { head: 'harry', w: 380, hx: 0.678, hy: 0.481 }, bg: OF(), blur: 2, actors: [HDR({ turn: -0.5, expr: 'determined' })] },
...
ep.panel(300, { cam: { x: 2400, y: 596, w: 300 }, bg: OF() },
  [sfx('click', 560, 150, { size: 60, font: "'Caveat', cursive" })], { mood: 'candle', alt: 'The doorknob clicks.' });
ep.panel(900, { cam: { head: 'dumbledore', w: 820, hx: 0.304, hy: 0.465 }, bg: OF({ doorOpen: 0.8 }), actors: [DECOR, DBD({ pose: 'stand', expr: 'calm' }), HDR({ turn: -0.3, expr: 'worried' })] },
...
ep.panel(900, { cam: { x: 2150, y: 523.7, w: 820 }, bg: OF(), actors: [DECOR, DBD({ pose: 'stand', expr: { base: 'smile', eyes: { sparkle: true } } }), HDR({ x: 2270, y: 845, turn: 0.5, pose: 'stand', expr: 'determined', armB: { sh: 110, el: -5, hand: 'palm' } })], over: (e) => { const d = e.toPanel([2330, 600]); return FX.emanata(d[0] + 40, d[1], 70, { n: 5, a0: -60, a1: 60 }); } },
...
ep.panel(520, { cam: { head: 'harry', w: 380, hx: 0.705, hy: 0.458 }, bg: OF(), blur: 2, actors: [HDR({ turn: -0.5, expr: 'stern' })] },
  [say('Harry', 'Please don\'t do that again. I don\'t like being trapped.', 250, 60, { anchor: 'tc', w: 340, fixed: true })], { mood: 'candle' });
ep.panel(760, { cam: { head: 'dumbledore', w: 440, hx: 0.534, hy: 0.721 }, bg: OF(), blur: 2, actors: [DBD({ expr: 'warm', pose: 'present' })] },
...
hS(760, 520, 0.731, 0.596, { expr: 'deadpan', pose: 'holdOne', armF: { sh: 40, el: 75, hand: 'hold' } },
  [say('Harry', 'Of course. It wasn\'t reasonable of me to expect the door to open before I put the quest items in my inventory.', 300, 60, { anchor: 'tc', w: 460, fixed: true })], { desk: RK, extra: [atAnchor('harry', 'handF', pouch(1), { dy: 27.3 })] });
// insert: the rock goes into the pouch
ep.cutout(640, (ctx) => ellipse(ctx.w / 2 + 40, ctx.h * 0.56 + 186, 140, 16, { fill: '#3a2a1a', opacity: 0.22, filter: 'url(#blur3)' }) +
  g({ transform: `translate(${ctx.w / 2 + 40},${ctx.h * 0.56}) scale(3.2)` }, pouch(1, { open: true })) + g({ transform: `translate(${ctx.w / 2 + 40},${ctx.h * 0.56 - 50}) scale(0.9) rotate(12)` }, P2.rock(1)) +
  g({ transform: `translate(${ctx.w / 2 + 40},${ctx.h * 0.56 + 40}) scale(3.2)` }, path('M-34,-10 Q-44,40 0,48 Q44,40 34,-10 Q0,4 -34,-10Z', { fill: '#8a6a4a', stroke: C.ink, 'stroke-width': 2.2 }), path('M-30,-6 Q-38,30 0,38', { fill: 'none', stroke: '#a9885f', 'stroke-width': 3, 'stroke-dasharray': '2 5' }), path('M-34,-10 Q-20,0 -8,-3 M8,-3 Q20,0 34,-10 M-8,-3 q-6,14 -2,22 M8,-3 q6,14 2,22', { fill: 'none', stroke: '#c9a24a', 'stroke-width': 2.4 })) +
  FX.emanata(ctx.w / 2 + 40, ctx.h * 0.56, 190, { n: 7, a0: -20, a1: 60 }),
...
// the tour
hS(460, 400, 0.702, 0.64, { expr: 'hopeful' },
  [say('Harry', 'As long as I\'m here, I don\'t suppose you\'d give me a bit of a tour?', 262, 60, { anchor: 'tc', w: 340, fixed: true })]);
ep.panel(1300, { cam: { x: 1200, y: 84, w: 1000 }, bg: OF(), actors: [DECOR, () => CS.dumbledoreThrone(1000), () => CS.blackDesk(1000, 960, {}), DBW({ x: 1250, pose: 'present', expr: 'smile', turn: -0.4 }), HW({ x: 1560, turn: -0.4, expr: 'awe' })] },
...
ep.panel(560, { cam: { head: 'harry', w: 460, hx: 0.388, hy: 0.652 }, bg: OF(), blur: 2, actors: [HW({ x: 1560, turn: 0.4, pose: 'point', expr: 'awe', armB: { sh: 110, el: 0, hand: 'point' } })] },
...
ep.panel(1000, { cam: { x: 1760, y: 389.8, w: 640 }, bg: OF(), actors: [DECOR, DBT({ armB: { sh: 60, el: 10, hand: 'point' } })], over: (e) => { const c = e.toPanel([1690, 610]); return text(c[0] - 20, c[1] + 70, 'blorple…', { 'font-family': 'Caveat', 'font-weight': 700, 'font-size': 40, fill: '#dfe7f2', 'text-anchor': 'middle', transform: `rotate(-8 ${c[0]} ${c[1] + 70})` }); } },
...
ep.panel(820, { cam: { x: 1802.1, y: 328.3, w: 560 }, bg: OF(), actors: [DECOR, DBT({ pose: 'present', expr: { base: 'grin', eyes: { sparkle: true } } })], over: (e) => { const c = e.toPanel([1710, 470]); return FX.sparkles([[c[0] - 70, c[1] - 60, 16], [c[0] + 60, c[1] - 30, 12], [c[0] - 40, c[1] + 50, 10]]); } },
...
ep.panel(1000, { cam: { x: 1450, y: 132.4, w: 700 }, bg: OF(), actors: [DECOR, DBH2()] },
...
ep.panel(780, { cam: { x: 1480, y: 316.2, w: 600 }, bg: OF(), actors: [DECOR, DBH2({ pose: 'point', armB: { sh: 125, el: 0, hand: 'point' }, expr: { base: 'smile', eyes: { sparkle: true } } })] },
...
ep.panel(940, { cam: { x: 730, y: 388.9, w: 780 }, bg: OF(), actors: [DECOR, DBP(), HP({ expr: 'confused' })] },
...
ep.panel(1000, { cam: { head: 'harry', w: 360, hx: 0.501, hy: 0.561 }, bg: OF(), blur: 3, actors: [HP({ expr: 'pained' })] },
...
ep.panel(800, { cam: { x: 600, y: 560, w: 620 }, bg: OF(), actors: [DECOR, HP({ expr: { base: 'grin', sweat: true }, pose: 'gesture' })] },
  [say('Harry', 'So, ah, what sort of magic do phœnixes do, then?', 230, 60, { anchor: 'tc', w: 300, fixed: true })], { mood: 'candle' });
ep.panel(800, { cam: { head: 'dumbledore', w: 480, hx: 0.513, hy: 0.71 }, bg: OF(), blur: 2, actors: [DBP({ pose: 'lecture', expr: 'calm', turn: -0.2 })] },
...
ep.panel(620, { cam: { x: 830, y: 385.1, w: 560 }, bg: OF(), actors: [DBP({ expr: { base: 'calm', eyes: { lookX: -0.8, lookY: 0.2 } }, pose: 'stand' })] },
...
// the shock: the panel's edges crack like the blaze
ep.panel(1000, { cam: { x: 720, y: 448.7, w: 800 }, bg: OF({ bird: 'fire' }), actors: [DECOR, DBP({ pose: 'stand', expr: 'calm' }), BLAZE, HP({ expr: 'horror', pose: 'panic' })], behind: (e) => { const c = e.toPanel([700, 280]); return FX.burst(e.w, e.h, c[0], c[1], { n: 26, op: 0.18 }); } },
  [cap('By the time this statement registered fully in Harry\'s mind, the chicken was already on fire.', 50, 44, { w: 620, fixed: true })], { shape: 'jag', jag: 16, seed: 4, alt: 'The chicken bursts into flame.' });
ep.panel(760, { cam: { x: 720, y: 279.7, w: 420 }, bg: OF({ bird: 'ash' }), actors: [() => g({ opacity: 0.5 }, path('M690,300 q-20,-40 10,-80 q10,40 30,20 q-10,-40 20,-60', { fill: 'none', stroke: '#9a9590', 'stroke-width': 5, 'stroke-linecap': 'round' }))] },
...
ep.panel(840, { cam: { x: 720, y: 476.2, w: 700 }, bg: OF({ bird: 'ash' }), actors: [DECOR, DBP({ expr: 'happy' }), HP({ expr: 'horror' })] },
  [say('Dumbledore', 'Don\'t look so horrified, Harry! Fawkes hasn\'t been hurt.', 470, 50, { anchor: 'tc', w: 420, fixed: true })], { mood: 'candle' });
const EGGH = atAnchor('dumbledore', 'handB', P2.egg(1), { dx: 4, dy: -30, k: 2 });
ep.panel(900, { cam: { x: 830, y: 338.1, w: 560 }, bg: OF({ bird: 'ash' }), actors: [DBP({ x: 930, pose: 'holdUp', expr: { base: 'happy', eyes: { sparkle: true } }, armB: { sh: 125, el: 25, hand: 'hold' } }), EGGH], over: (e) => { const h = e.anchors.dumbledore.handB; return FX.sparkles([[h[0] - 40, h[1] - 90, 16], [h[0] + 50, h[1] - 70, 12]]); } },
...
ep.panel(480, { cam: { head: 'harry', w: 380, hx: 0.687, hy: 0.588 }, bg: OF(), blur: 2, actors: [HP({ expr: { base: 'blank', mouth: { type: 'line' } } })] },
  [say('Harry', 'Oh… wow… amazing.', 230, 110, { w: 260, fixed: true })], { mood: 'candle' });
// the apology: back at the desk
const EG = { bird: 'egg' };
dbS(840, 480, 0.502, 0.749, { expr: 'sad' },
...
hS(680, 440, 0.743, 0.716, { expr: 'yell', pose: 'fists' },
  [shout('Harry', 'My step-parents aren\'t wicked! My *parents*, I mean!', 340, 80, { anchor: 'tc', w: 400, fixed: true })], { of: EG });
dbS(640, 440, 0.502, 0.699, { expr: 'confused' },
  [say('Dumbledore', 'They aren\'t? Not even a *little* wicked? That doesn\'t fit the pattern…', 400, 60, { anchor: 'tc', w: 460, fixed: true })], { of: EG });
// the inner Slytherin's scream blows the panel itself apart
ep.panel(900, (ctx) => HG.mindscape(ctx.w, ctx.h, 'warm', 6) + shot({ cam: { head: 'ms', w: 640, hx: 0.485, hy: 0.627 }, actors: [MINI('s', 'yell', 1000, 'panic', 0.1), scarf('s')] })(ctx),
  [shout('ms', '*SHUT UP YOU IDIOT HE\'LL TAKE YOU AWAY FROM THEM!*', 400, 150, { anchor: 'tc', w: 500, fixed: true })], { mood: 'candle', shape: 'burst', points: 20, seed: 5, alt: 'Harry\'s inner Slytherin screams at the top of its mental lungs.' });
hS(840, 460, 0.73, 0.723, { expr: { base: 'grin', sweat: true }, pose: 'shrug' },
...
dbS(900, 500, 0.502, 0.767, { expr: { base: 'warm', eyes: { soft: true } } },
...
hS(900, 580, 0.493, 0.418, { expr: 'angry', pose: 'fists' },
...
dbS(820, 460, 0.502, 0.743, { expr: { base: 'teary', eyes: { lookY: 0.3 } } },
...
ep.panel(860, { cam: { head: 'dumbledore', w: 300, hx: 0.503, hy: 0.34 }, bg: OF(EG), blur: 3, actors: SET({ expr: { base: 'teary', tearDrop: true, mouth: { type: 'line', curve: -0.2, w: 0.7 } } }, {}, {}) },
  [say('Dumbledore', 'that I am truly, truly sorry. For everything that has already happened, and everything that will.', 400, 805, { anchor: 'bc', w: 500, fixed: true })], { mood: 'candle', frame: 'dissolve', feather: 70, alt: 'Moisture glistens in the old wizard\'s eyes.' });
hS(640, 440, 0.73, 0.698, { expr: 'angry' },
...
dbS(900, 500, 0.486, 0.694, { expr: { base: 'calm', eyes: { sparkle: true } }, pose: 'lecture' },
...
// he flees
// the bolt: the panel leans with him as he flees
ep.panel(860, { cam: { x: 820, y: 553.8, w: 720 }, bg: GC({ open: true }), actors: [stairDoor, { def: harryRaven, id: 'harry', x: 700, y: 960, s: 1.1, turn: -0.8, pose: 'run', expr: 'horror' }], behind: (e) => FX.speedLines(e.w, e.h, { n: 40 }), over: (e) => FX.emanata(e.anchors.harry.head[0] + 90, e.anchors.harry.head[1] + 60, 120, { n: 5, a0: -40, a1: 40 }) },
  [cap('Harry spun around and bolted for the exit at top speed. The doorknob turned agreeably. He raced down the spiral stairs even as they turned, and fired out of the stairwell like a cannonball…', 44, 36, { w: 620, fixed: true })], { mood: 'candle', shape: 'slant', slant: -80, alt: 'Harry shoots out of the stairwell at full speed.' });
// McGonagall
...
ep.bleed(820, { cam: { x: 1080, y: 632.5, w: 900 }, bg: GC({ open: true }), actors: [MGF(), HGF(), papers], behind: (e) => { const c = e.toPanel([1080, 800]); return FX.burst(e.w, e.h, c[0], c[1], { n: 26, op: 0.2 }); } },
...
ep.panel(920, { cam: { x: 1080, y: 608.2, w: 760 }, bg: GC({ open: true }), actors: [papers, MGF({ pose: 'sitFloor', expr: 'yell' }), HGF({ pose: 'sitFloor', expr: 'horror' })] },
...
ep.panel(760, { cam: { head: 'mcgonagall', w: 420, hy: 0.756 }, bg: GC({ open: true }), blur: 2, actors: [MGF({ pose: 'sitFloor', expr: { base: 'yell', brows: { raise: -0.6, inner: -0.8 } } })] },
  [shout('McGonagall', '*Were you talking about the Defence Professor?*', 400, 84, { anchor: 'tc', w: 380, fixed: true })], { mood: 'candle' });
ep.panel(760, { cam: { head: 'harry', w: 420, hx: 0.553, hy: 0.693 }, bg: GC({ open: true }), blur: 2, actors: [HGF({ pose: 'sitFloor', expr: 'pleading' })] },
...
ep.panel(900, { cam: { x: 1040, y: 546.4, w: 640 }, bg: GC({}), actors: [MG({ expr: 'worried' }), HM({ expr: 'sad' })] },
...
ep.panel(760, { cam: { head: 'harry', w: 420, hx: 0.659, hy: 0.748 }, bg: GC({}), blur: 2, actors: [HM({ expr: { base: 'teary', eyes: { lookY: -0.6, lookX: 0.4 } } })] },
...
ep.panel(820, { cam: { head: 'mcgonagall', w: 440, hx: 0.493, hy: 0.716 }, bg: GC({}), blur: 2, actors: [MG({ expr: 'stern', turn: -0.2 })] },
...
ep.panel(780, { cam: { head: 'harry', w: 440, hx: 0.659, hy: 0.728 }, bg: GC({}), blur: 2, actors: [HM({ expr: 'hopeful', pose: 'gesture' })] },
...
ep.panel(1080, { cam: { head: 'mcgonagall', w: 380, hx: 0.576, hy: 0.739 }, bg: GC({}), actors: [MG({ expr: { base: 'stern', eyes: { lookX: -0.4 } }, pose: 'lecture', turn: -0.2 }), atAnchor('mcgonagall', 'handB', K.glow(0, 0, 70, '#f7dc8c', 0.5), { dy: -40 }), atAnchor('mcgonagall', 'handB', P2.ring(1), { dy: -44, k: 1.4 })] },
...
ep.panel(640, { cam: { head: 'mcgonagall', w: 380, hx: 0.493, hy: 0.665 }, bg: GC({}), blur: 3, actors: [MG({ expr: 'confused', turn: -0.1 })] },
  [say('McGonagall', 'Did the Headmaster *really…*', 400, 40, { anchor: 'tc', w: 400, fixed: true })], { mood: 'candle' });
ep.panel(820, { cam: { x: 1060, y: 483, w: 800 }, bg: GC({ open: true }), actors: [stairDoor, MG({ x: 1080, expr: { base: 'stern', eyes: { soft: true } }, turn: -0.4 }), HM({ x: 760, expr: 'worried' })] },
...
ep.panel(1030, { cam: { x: 960, y: 466.1, w: 900 }, bg: GC({ open: true }), actors: [stairDoor, MG({ x: 1000, y: 260, turn: 0.1 }), HM({ x: 640, turn: 0.4, pose: 'point', expr: 'yell', armB: { sh: 110, el: 0, hand: 'point' } })] },
...
ep.panel(620, { cam: { x: 1000, y: 476.2, w: 700 }, bg: GC({}), over: (e) => { const c = e.toPanel([1000, 700]); return FX.emanata(c[0] - 190, c[1] - 10, 60, { n: 3, a0: 160, a1: 200 }) + FX.emanata(c[0] + 190, c[1] - 10, 60, { n: 3, a0: -20, a1: 20 }); } },
