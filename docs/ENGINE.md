...
- **Faces:** `glasses: false` on an actor hides their glasses. Mouth `'grimace'` = `'grit'`. The blue "cold" skin tint only applies to Harry (his dark side); set `coldTint: true` on another character's def to allow it. A deep bow no longer turns the face sideways.
- **Drawing with a character's position:** `withActor('harry', (a, e) => svg)` draws only if Harry is in the shot, handing you his anchors (`a.head`, `a.handB`, `a.hr` head radius, `a.s` scale…). World space in `actors`/`fg`; `{ space: 'panel' }` in `over`.
- **Props at a hand:** `atAnchor('harry', 'handB', svg, { dx, dy, rot, k })` draws something at an actor's anchor (hands, head, mouth, neck, hip). Put it in the shot's `actors` list or `fg`; with `{ space: 'panel' }` it works in `over`. Offsets and size follow the actor's scale and the zoom. For a prop gripped in the hand, `armF: { prop }` on the actor is usually simpler.
- **Handwriting and notes:** `FX.scrawl(x, y, text, size, anchor, colour)` is Harry's pencil hand (Caveat); `FX.notebook(w, h, { step, margin… })` is lined notebook paper.
- **Fading a cut-off figure into the page:** `over: FX.fadeOut(0.62, 0.97)` fades the panel into the page colour between those fractions of its height (a cut-out figure cropped above the feet).
